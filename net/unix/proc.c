...
int unix_get_info(char *buffer)
{
    ...
    for (i = 0; i < NSOCKETS; i++) {
        if (unix_datas[i].refcnt > 0) {
            pos += sprintf(pos, "%2d: %08X %08X %08lX %04X %02X", i, unix_datas[i].refcnt,
                           unix_datas[i].protocol, unix_datas[i].socket->flags,
                           unix_datas[i].socket->type, unix_datas[i].socket->state);
