...
static int sock_read(struct inode *inode, struct file *file, char *buf, int size);
static int sock_write(struct inode *inode, struct file *file, char *buf, int size);
...
static struct file_operations socket_file_ops = {sock_lseek,  sock_read,  sock_write, sock_readdir,
                                                 sock_select, sock_ioctl, NULL, /* mmap */
    ...
    while (1) {
        ...
        for (sock = sockets; sock <= last_socket; ++sock) {
             ...
             */
                ...
                DPRINTF(
                    (net_debug, "NET: sock_alloc: sk 0x%x, ino 0x%x\n", sock, SOCK_INODE(sock)));
                return (sock);
            }
        }
        sti();
...
int sock_awaitconn(struct socket *mysock, struct socket *servsock)
{
    ...
    DPRINTF((net_debug, "NET: sock_awaitconn: trying to connect socket 0x%x to 0x%x\n", mysock,
             servsock));
    if (!(servsock->flags & SO_ACCEPTCON)) {
        DPRINTF((net_debug, "NET: sock_awaitconn: server not accepting connections\n"));
        return (-EINVAL);
    }
...
static int sock_socket(int family, int type, int protocol)
{
    ...
    DPRINTF((net_debug, "NET: sock_socket: family = %d, type = %d, protocol = %d\n", family, type,
             protocol));
   ...
   * protocol later.
   */
    if ((type != SOCK_STREAM && type != SOCK_DGRAM && type != SOCK_SEQPACKET && type != SOCK_RAW &&
         type != SOCK_PACKET) ||
        protocol < 0)
        return (-EINVAL);
...
static int sock_socketpair(int family, int type, int protocol, unsigned long usockvec[2])
{
    ...
    DPRINTF((net_debug, "NET: sock_socketpair: family = %d, type = %d, protocol = %d\n", family,
             type, protocol));
...
static int sock_accept(int fd, struct sockaddr *upeer_sockaddr, int *upeer_addrlen)
{
    ...
    if (!(sock->flags & SO_ACCEPTCON)) {
        DPRINTF((net_debug, "NET: sock_accept: socket not accepting connections!\n"));
        return (-EINVAL);
    }
...
static int sock_connect(int fd, struct sockaddr *uservaddr, int addrlen)
{
    ...
    default:
        DPRINTF((net_debug, "NET: sock_connect: socket not unconnected\n"));
        return (-EINVAL);
    }
...
static int sock_send(int fd, void *buff, int len, unsigned flags)
{
    ...
    DPRINTF((net_debug, "NET: sock_send(fd = %d, buff = %X, len = %d, flags = %X)\n", fd, buff, len,
             flags));
...
static int sock_sendto(int fd, void *buff, int len, unsigned flags, struct sockaddr *addr,
                       int addr_len)
{
    struct socket *sock;
    struct file *file;
    ...
    return (
        sock->ops->sendto(sock, buff, len, (file->f_flags & O_NONBLOCK), flags, addr, addr_len));
}
...
static int sock_recv(int fd, void *buff, int len, unsigned flags)
{
    ...
    DPRINTF((net_debug, "NET: sock_recv(fd = %d, buff = %X, len = %d, flags = %X)\n", fd, buff, len,
             flags));
...
static int sock_recvfrom(int fd, void *buff, int len, unsigned flags, struct sockaddr *addr,
                         int *addr_len)
{
    struct socket *sock;
    struct file *file;
    ...
    return (
        sock->ops->recvfrom(sock, buff, len, (file->f_flags & O_NONBLOCK), flags, addr, addr_len));
}
...
asmlinkage int sys_socketcall(int call, unsigned long *args)
{
    ...
    case SYS_BIND:
        ...
        if (er)
            return er;
        return (sock_bind(get_fs_long(args + 0), (struct sockaddr *)get_fs_long(args + 1),
                          get_fs_long(args + 2)));
    case SYS_CONNECT:
        er = verify_area(VERIFY_READ, args, 3 * sizeof(long));
        if (er)
            return er;
        return (sock_connect(get_fs_long(args + 0), (struct sockaddr *)get_fs_long(args + 1),
                             get_fs_long(args + 2)));
    case SYS_LISTEN:
        er = verify_area(VERIFY_READ, args, 2 * sizeof(long));
        if (er)
            return er;
        return (sock_listen(get_fs_long(args + 0), get_fs_long(args + 1)));
    case SYS_ACCEPT:
        er = verify_area(VERIFY_READ, args, 3 * sizeof(long));
        if (er)
            return er;
        return (sock_accept(get_fs_long(args + 0), (struct sockaddr *)get_fs_long(args + 1),
                            (int *)get_fs_long(args + 2)));
    case SYS_GETSOCKNAME:
        er = verify_area(VERIFY_READ, args, 3 * sizeof(long));
        if (er)
            return er;
        return (sock_getsockname(get_fs_long(args + 0), (struct sockaddr *)get_fs_long(args + 1),
                                 (int *)get_fs_long(args + 2)));
    case SYS_GETPEERNAME:
        er = verify_area(VERIFY_READ, args, 3 * sizeof(long));
        if (er)
            return er;
        return (sock_getpeername(get_fs_long(args + 0), (struct sockaddr *)get_fs_long(args + 1),
                                 (int *)get_fs_long(args + 2)));
    case SYS_SOCKETPAIR:
        er = verify_area(VERIFY_READ, args, 4 * sizeof(long));
        if (er)
            return er;
        return (sock_socketpair(get_fs_long(args + 0), get_fs_long(args + 1), get_fs_long(args + 2),
                                (unsigned long *)get_fs_long(args + 3)));
    case SYS_SEND:
        er = verify_area(VERIFY_READ, args, 4 * sizeof(unsigned long));
        if (er)
            return er;
        return (sock_send(get_fs_long(args + 0), (void *)get_fs_long(args + 1),
                          get_fs_long(args + 2), get_fs_long(args + 3)));
    case SYS_SENDTO:
        er = verify_area(VERIFY_READ, args, 6 * sizeof(unsigned long));
        if (er)
            return er;
        return (sock_sendto(get_fs_long(args + 0), (void *)get_fs_long(args + 1),
                            get_fs_long(args + 2), get_fs_long(args + 3),
                            (struct sockaddr *)get_fs_long(args + 4), get_fs_long(args + 5)));
    case SYS_RECV:
        er = verify_area(VERIFY_READ, args, 4 * sizeof(unsigned long));
        if (er)
            return er;
        return (sock_recv(get_fs_long(args + 0), (void *)get_fs_long(args + 1),
                          get_fs_long(args + 2), get_fs_long(args + 3)));
    case SYS_RECVFROM:
        er = verify_area(VERIFY_READ, args, 6 * sizeof(unsigned long));
        if (er)
            return er;
        return (sock_recvfrom(get_fs_long(args + 0), (void *)get_fs_long(args + 1),
                              get_fs_long(args + 2), get_fs_long(args + 3),
    ...
    case SYS_SHUTDOWN:
        ...
        if (er)
            return er;
        return (sock_shutdown(get_fs_long(args + 0), get_fs_long(args + 1)));
    case SYS_SETSOCKOPT:
        er = verify_area(VERIFY_READ, args, 5 * sizeof(unsigned long));
        if (er)
            return er;
        return (sock_setsockopt(get_fs_long(args + 0), get_fs_long(args + 1), get_fs_long(args + 2),
                                (char *)get_fs_long(args + 3), get_fs_long(args + 4)));
    case SYS_GETSOCKOPT:
        er = verify_area(VERIFY_READ, args, 5 * sizeof(unsigned long));
        if (er)
            return er;
        return (sock_getsockopt(get_fs_long(args + 0), get_fs_long(args + 1), get_fs_long(args + 2),
                                (char *)get_fs_long(args + 3), (int *)get_fs_long(args + 4)));
    default:
        return (-EINVAL);
    }
}

static int net_ioctl(unsigned int cmd, unsigned long arg)
{
    int er;
    switch (cmd) {
    case DDIOCSDBG:
 ...
 */
static int net_fioctl(struct inode *inode, struct file *file, unsigned int cmd, unsigned long arg)
{
...
int sock_register(int family, struct proto_ops *ops)
{
    ...
    for (i = 0; i < NPROTO; i++) {
        ...
        sti();
        DPRINTF(
            (net_debug, "NET: Installed protocol %d in slot %d (0x%X)\n", family, i, (long)ops));
        return (i);
    }
    sti();
    return (-ENOMEM);
