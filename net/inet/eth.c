...
char *eth_print(unsigned char *ptr)
{
    ...
    if (ptr == NULL)
        return ("[NONE]");
    sprintf(buff, "%02X:%02X:%02X:%02X:%02X:%02X", (ptr[0] & 255), (ptr[1] & 255), (ptr[2] & 255),
...
int eth_header(unsigned char *buff, struct device *dev, unsigned short type, unsigned long daddr,
               unsigned long saddr, unsigned len)
{
    ...
    if (arp_find(eth->h_dest, daddr, dev, dev->pa_addr)) {
        ...
        memcpy(eth->h_source, dev->dev_addr,
               dev->addr_len); /* This was missing causing chaos if the
