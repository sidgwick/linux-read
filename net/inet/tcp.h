...
#define TCP_TIMEWAIT_LEN                                                                           \
    (60 * HZ)             /* how long to wait to sucessfully
                  * close the socket, about 60 seconds    */
#define TCP_ACK_TIME 3000 /* time to delay before sending an ACK    */
#define TCP_DONE_TIME                                                                              \
    250 /* maximum time to wait before actually
                 * destroying a socket            */
#define TCP_WRITE_TIME                                                                             \
    3000                      /* initial time to wait for an ACK,
                     * after last transmit            */
#define TCP_CONNECT_TIME 2000 /* time to retransmit first SYN        */
#define TCP_SYN_RETRIES                                                                            \
    5 /* number of times to retry openning a
                 * connection                 */
#define TCP_PROBEWAIT_LEN                                                                          \
    100 /* time to wait between probes when
                 * I've got something to write and
                 * there is no window            */

#define TCP_NO_CHECK                                                                               \
    0 /* turn to one if you want the default
                 * to be no checksum            */
...
{
    return (state == TCP_ESTABLISHED || state == TCP_CLOSE_WAIT || state == TCP_FIN_WAIT1 ||
            state == TCP_FIN_WAIT2 || state == TCP_SYN_RECV);
}

extern struct proto tcp_prot;

extern void tcp_err(int err, unsigned char *header, unsigned long daddr, unsigned long saddr,
                    struct inet_protocol *protocol);
extern void tcp_shutdown(struct sock *sk, int how);
extern int tcp_rcv(struct sk_buff *skb, struct device *dev, struct options *opt,
