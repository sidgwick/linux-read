...
static char *arp_cmds[] = {"0x%04X", "REQUEST", "REPLY", "REVERSE REQUEST", "REVERSE REPLY", NULL};
#define ARP_MAX_CMDS (sizeof(arp_cmds) / sizeof(arp_cmds[0]))

static struct {
    char *name;
    char *(*print)(unsigned char *ptr, int len);
} arp_types[] = {{"0x%04X", unk_print},
...
static void arp_print(struct arphdr *arp)
{
    int len, idx;
    unsigned char *ptr;
...
static int arp_response(struct arphdr *arp1, struct device *dev, int addrtype)
{
    ...
    skb = alloc_skb(sizeof(struct sk_buff) + sizeof(struct arphdr) + (2 * arp1->ar_hln) +
    ...
    skb->mem_addr = skb;
    skb->len =
        sizeof(struct arphdr) + (2 * arp1->ar_hln) + (2 * arp1->ar_pln) + dev->hard_header_len;
...
int arp_rcv(struct sk_buff *skb, struct device *dev, struct packet_type *pt)
{
    ...
    if (((arp->ar_pro != NET16(0x00CC) && dev->type == 3) ||
         (arp->ar_pro != NET16(ETH_P_IP) && dev->type != 3)) ||
        arp->ar_pln != 4) {
...
void arp_send(unsigned long paddr, struct device *dev, unsigned long saddr)
{
    ...
    skb = alloc_skb(sizeof(struct sk_buff) + sizeof(struct arphdr) + (2 * dev->addr_len) +
                        dev->hard_header_len + (2 * 4 /* arp->plen */),
                    GFP_ATOMIC);
    if (skb == NULL) {
...
int arp_find(unsigned char *haddr, unsigned long paddr, struct device *dev, unsigned long saddr)
{
     ...
     */
        if ((apt->flags & ATF_PERM) || (apt->last_used < jiffies + ARP_TIMEOUT && apt->hlen != 0)) {
            apt->last_used = jiffies;
    ...
    for (i = 0; i < ARP_TABLE_SIZE; i++) {
        ...
        while (apt != NULL) {
            if (pos < (buffer + 4000)) {
                ...
                memcpy((char *)req->arp_ha.sa_data, (char *)&apt->ha, apt->hlen);
...
static int arp_req_set(struct arpreq *req)
{
   ...
   */
    ...
    if (apt == NULL) {
        apt = arp_create(si->sin_addr.s_addr, (unsigned char *)r.arp_ha.sa_data, hlen, htype);
        if (apt == NULL)
            return (-ENOMEM);
    }
