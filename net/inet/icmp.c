...
void icmp_send(struct sk_buff *skb_in, int type, int code, struct device *dev)
{
    ...
    len = sizeof(struct sk_buff) + dev->hard_header_len + sizeof(struct iphdr) +
          sizeof(struct icmphdr) + sizeof(struct iphdr) + 8; /* amount of header to return */
    ...
    offset = ip_build_header(skb, dev->pa_addr, iph->saddr, &dev, IPPROTO_ICMP, NULL, len,
    ...
    icmph->checksum =
        ip_compute_csum((unsigned char *)icmph, sizeof(struct icmphdr) + sizeof(struct iphdr) + 8);
...
static void icmp_unreach(struct icmphdr *icmph, struct sk_buff *skb)
{
    ...
    while (ipprot != NULL) {
        ...
        if (iph->protocol == ipprot->protocol && ipprot->err_handler) {
            ipprot->err_handler(err, (unsigned char *)(icmph + 1), iph->daddr, iph->saddr, ipprot);
        }
...
static void icmp_redirect(struct icmphdr *icmph, struct sk_buff *skb, struct device *dev)
{
    ...
    case ICMP_REDIR_NET:
        ...
        rt_add((RTF_DYNAMIC | RTF_MODIFIED | RTF_GATEWAY), ip, 0, icmph->un.gateway, dev);
        break;
#endif
    case ICMP_REDIR_HOST:
        rt_add((RTF_DYNAMIC | RTF_MODIFIED | RTF_HOST | RTF_GATEWAY), ip, 0, icmph->un.gateway,
               dev);
        break;
                      ...
                      unsigned long saddr, unsigned long daddr, int len, struct options *opt)
{
    ...
    /* Build Layer 2-3 headers for message back to source */
    offset =
        ip_build_header(skb2, daddr, saddr, &dev, IPPROTO_ICMP, opt, len, skb->ip_hdr->tos, 255);
    if (offset < 0) {
             ...
             unsigned short len, unsigned long saddr, int redo, struct inet_protocol *protocol)
{
    ...
    case ICMP_ADDRESSREPLY:
        ...
        return (0);
    default:
        DPRINTF((DBG_ICMP, "ICMP: Unsupported ICMP from %s, type = 0x%X\n", in_ntoa(saddr),
                 icmph->type));
        skb1->sk = NULL;
        kfree_skb(skb1, FREE_READ);
        return (0);
