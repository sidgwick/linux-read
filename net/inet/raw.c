...
void raw_err(int err, unsigned char *header, unsigned long daddr, unsigned long saddr,
 ...
 */
int raw_rcv(struct sk_buff *skb, struct device *dev, struct options *opt, unsigned long daddr,
...
static int raw_sendto(struct sock *sk, unsigned char *from, int len, int noblock, unsigned flags,
                      struct sockaddr_in *usin, int addr_len)
{
    ...
    DPRINTF((DBG_RAW,
             "raw_sendto(sk=%X, from=%X, len=%d, noblock=%d, flags=%X,\n"
             "            usin=%X, addr_len = %d)\n",
             sk, from, len, noblock, flags, usin, addr_len));
    ...
    while (skb == NULL) {
        ...
        skb = sk->prot->wmalloc(sk, len + sizeof(*skb) + sk->prot->max_header, 0, GFP_KERNEL);
        if (skb == NULL) {
            int tmp;
    ...
    tmp = sk->prot->build_header(skb, sk->saddr, sin.sin_addr.s_addr, &dev, sk->protocol, sk->opt,
 ...
 */
int raw_recvfrom(struct sock *sk, unsigned char *to, int len, int noblock, unsigned flags,
                 struct sockaddr_in *sin, int *addr_len)
{
    ...
    DPRINTF((DBG_RAW,
             "raw_recvfrom (sk=%X, to=%X, len=%d, noblock=%d, flags=%X,\n"
             "              sin=%X, addr_len=%X)\n",
...
struct proto raw_prot = {sock_wmalloc,
                         sock_rmalloc,
                         sock_wfree,
                         sock_rfree,
                         sock_rspace,
