...
extern int eth_header(unsigned char *buff, struct device *dev, unsigned short type,
                      unsigned long daddr, unsigned long saddr, unsigned len);
extern int eth_rebuild_header(void *buff, struct device *dev);
extern void eth_add_arp(unsigned long addr, struct sk_buff *skb, struct device *dev);
