...
static void __print_th(struct tcphdr *th)
{
    ...
    printk("    fin=%d, syn=%d, rst=%d, psh=%d, ack=%d, urg=%d res1=%d res2=%d\n", th->fin, th->syn,
           th->rst, th->psh, th->ack, th->urg, th->res1, th->res2);
    printk("    window = %d, check = %d urg_ptr = %d\n", ntohs(th->window), ntohs(th->check),
           ntohs(th->urg_ptr));
    printk("    doff = %d\n", th->doff);
 ...
 */
void tcp_err(int err, unsigned char *header, unsigned long daddr, unsigned long saddr,
...
static int tcp_readable(struct sock *sk)
{
    ...
    do {
        ...
        sum = skb->len -
              (counted -
               skb->h.th
                   ->seq); /* Length - header but start from where we are up to (avoid overlaps) */
        if (skb->h.th->syn)
            sum++;
...
static int tcp_select(struct sock *sk, int sel_type, select_table *wait)
{
    ...
    case SEL_OUT:
        ...
        if (sk->shutdown & SEND_SHUTDOWN) {
            DPRINTF((DBG_TCP, "write select on shutdown socket.\n"));
        ...
        DPRINTF((DBG_TCP,
                 ...
                 sk->wmem_alloc, sk->packets_out, sk->wback, sk->wfront, sk->write_seq,
                 sk->window_seq));
...
void tcp_send_check(struct tcphdr *th, unsigned long saddr, unsigned long daddr, int len,
                    struct sock *sk)
{
    th->check = 0;
...
static void tcp_send_skb(struct sock *sk, struct sk_buff *skb)
{
    ...
    if (after(skb->h.seq, sk->window_seq) || (sk->retransmits && sk->timeout == TIME_WRITE) ||
        sk->packets_out >= sk->cong_window) {
        DPRINTF((DBG_TCP, "sk->cong_window = %d, sk->packets_out = %d\n", sk->cong_window,
                 sk->packets_out));
        DPRINTF(
            (DBG_TCP, "sk->write_seq = %d, sk->window_seq = %d\n", sk->write_seq, sk->window_seq));
        skb->next = NULL;
        ...
        sk->wback = skb;
        if (before(sk->window_seq, sk->wfront->h.seq) && sk->send_head == NULL &&
            sk->ack_backlog == 0)
...
static void tcp_send_ack(unsigned long sequence, unsigned long ack, struct sock *sk,
 ...
 */
static int tcp_write(struct sock *sk, unsigned char *from, int len, int nonblock, unsigned flags)
{
    int copied = 0;
    int copy;
    int tmp;
 ...
 */
       ...
       * mss is always <= mtu */
        ...
        if (skb == NULL) {
            ...
            if (tmp <= sk->wmem_alloc &&
                (sk->state == TCP_ESTABLISHED || sk->state == TCP_CLOSE_WAIT) && sk->err == 0) {
                interruptible_sleep_on(sk->sleep);
                if (current->signal & ~current->blocked) {
                    sti();
     ...
     */
        tmp = prot->build_header(skb, sk->saddr, sk->daddr, &dev, IPPROTO_TCP, sk->opt,
                                 skb->mem_len, sk->ip_tos, sk->ip_ttl);
        if (tmp < 0) {
...
static int tcp_sendto(struct sock *sk, unsigned char *from, int len, int nonblock, unsigned flags,
...
static void tcp_read_wakeup(struct sock *sk)
{
   ...
   */
    ...
    /* Put in the IP header and routing stuff. */
    tmp = sk->prot->build_header(buff, sk->saddr, sk->daddr, &dev, IPPROTO_TCP, sk->opt,
                                 MAX_ACK_SIZE, sk->ip_tos, sk->ip_ttl);
    if (tmp < 0) {
        buff->free = 1;
        sk->prot->wfree(sk, buff->mem_addr, buff->mem_len);
        return;
...
static int tcp_read_urg(struct sock *sk, int nonblock, unsigned char *to, int len, unsigned flags)
{
    ...
    while (len > 0) {
        ...
        if ((sk->urg_data & URG_NOTYET) && sk->err == 0 && !(sk->shutdown & RCV_SHUTDOWN))
            schedule();
...
static int tcp_read(struct sock *sk, unsigned char *to, int len, int nonblock, unsigned flags)
{
    ...
    found_ok_skb:
        ...
        /* Copy it */
        memcpy_tofs(to, ((unsigned char *)skb->h.th) + skb->h.th->doff * 4 + offset, used);
        copied += used;
        len -= used;
        to += used;
        *seq += used;
...
void tcp_shutdown(struct sock *sk, int how)
{
   ...
   */
    ...
    /* Put in the IP header and routing stuff. */
    tmp = prot->build_header(buff, sk->saddr, sk->daddr, &dev, IPPROTO_TCP, sk->opt,
...
static int tcp_recvfrom(struct sock *sk, unsigned char *to, int to_len, int nonblock,
                        unsigned flags, struct sockaddr_in *addr, int *addr_len)
{
    struct sockaddr_in sin;
    int len;
    int err;
                      ...
                      struct proto *prot, struct options *opt, struct device *dev, int tos, int ttl)
{
   ...
   */
    ...
    /* Put in the IP header and routing stuff. */
    tmp = prot->build_header(buff, saddr, daddr, &dev, IPPROTO_TCP, opt, sizeof(struct tcphdr), tos,
                             ttl);
    if (tmp < 0) {
        buff->free = 1;
 ...
 * listening.
 */
static void tcp_conn_request(struct sock *sk, struct sk_buff *skb, unsigned long daddr,
                             unsigned long saddr, struct options *opt, struct device *dev)
{
   ...
   */
    ...
    /* Put in the IP header and routing stuff. */
    tmp = sk->prot->build_header(buff, newsk->saddr, newsk->daddr, &dev, IPPROTO_TCP, NULL,
...
static void tcp_write_xmit(struct sock *sk)
{
    ...
    while (sk->wfront != NULL && before(sk->wfront->h.seq, sk->window_seq + 1) &&
           (sk->retransmits == 0 || sk->timeout != TIME_WRITE ||
...
static int tcp_ack(struct sock *sk, struct tcphdr *th, unsigned long saddr, int len)
{
   ...
   */
    ...
    if (sk->timeout == TIME_WRITE && sk->cong_window < 2048 && after(ack, sk->rcv_ack_seq)) {
        /*
   ...
   */
    if (sk->wfront != NULL) {
        if (after(sk->window_seq + 1, sk->wfront->h.seq) &&
            ...
            flag |= 1;
            tcp_write_xmit(sk);
        } else if (before(sk->window_seq, sk->wfront->h.seq) && sk->send_head == NULL &&
                   sk->ack_backlog == 0 && sk->state != TCP_TIME_WAIT) {
            reset_timer(sk, TIME_PROBE0, sk->rto);
        }
    } else {
    ...
    if (sk->packets_out == 0 && sk->partial != NULL && sk->wfront == NULL &&
        sk->send_head == NULL) {
        flag |= 1;
        tcp_send_partial(sk);
    }
 ...
 */
    ...
    if (((!flag) || (flag & 4)) && sk->send_head != NULL &&
        (((flag & 2) && sk->retransmits) || (sk->send_head->when + sk->rto < jiffies))) {
 ...
 */
static int tcp_data(struct sk_buff *skb, struct sock *sk, unsigned long saddr, unsigned short len)
{
    ...
    if (sk->shutdown & RCV_SHUTDOWN) {
        ...
        tcp_reset(sk->saddr, sk->daddr, skb->h.th, sk->prot, NULL, skb->dev, sk->ip_tos,
                  sk->ip_ttl);
        sk->state = TCP_CLOSE;
        sk->err = EPIPE;
        sk->shutdown = SHUTDOWN_MASK;
   ...
   */
    ...
    if ((!dup_dumped && (skb1 == NULL || skb1->acked)) || before(th->seq, sk->acked_seq + 1)) {
        if (before(th->seq, sk->acked_seq + 1)) {
            ...
            for (skb2 = (struct sk_buff *)skb->next; skb2 != (struct sk_buff *)sk->rqueue;
...
static inline int tcp_urg(struct sock *sk, struct tcphdr *th, unsigned long saddr,
                          unsigned long len)
{
    unsigned long ptr;
...
static int tcp_sequence(struct sock *sk, struct tcphdr *th, short len, struct options *opt,
...
int tcp_rcv(struct sk_buff *skb, struct device *dev, struct options *opt, unsigned long daddr,
  ...
  if (!opt) {    /* FIXME: it's ok for opt to be NULL */
    ...
    case TCP_SYN_SENT:
        ...
        case TCP_SYN_SENT:
            ...
            sk->fin_seq = th->seq;
            tcp_send_ack(sk->sent_seq, th->seq + 1, sk, th, sk->daddr);
...
struct proto tcp_prot = {sock_wmalloc,
                         sock_rmalloc,
                         sock_wfree,
                         sock_rfree,
                         sock_rspace,
