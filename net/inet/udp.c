 ...
 */
void udp_err(int err, unsigned char *header, unsigned long daddr, unsigned long saddr,
...
sport=%d,dport=%d",
             err, header, daddr, saddr, protocol, (int)th->source, (int)th->dest));
...
static unsigned short udp_check(struct udphdr *uh, int len, unsigned long saddr,
                                unsigned long daddr)
{
    unsigned long sum;

    DPRINTF(
        (DBG_UDP, "UDP: check(uh=%X, len = %d, saddr = %X, daddr = %X)\n", uh, len, saddr, daddr));
...
static void udp_send_check(struct udphdr *uh, unsigned long saddr, unsigned long daddr, int len,
                           struct sock *sk)
{
    uh->check = 0;
    if (sk && sk->no_check)
...
static int udp_send(struct sock *sk, struct sockaddr_in *sin, unsigned char *from, int len)
{
    ...
    DPRINTF((DBG_UDP, "UDP: >> IP_Header: %X -> %X dev=%X prot=%X len=%d\n", saddr,
             sin->sin_addr.s_addr, dev, IPPROTO_UDP, skb->mem_len));
    tmp = sk->prot->build_header(skb, saddr, sin->sin_addr.s_addr, &dev, IPPROTO_UDP, sk->opt,
...
static int udp_sendto(struct sock *sk, unsigned char *from, int len, int noblock, unsigned flags,
 ...
 */
int udp_recvfrom(struct sock *sk, unsigned char *to, int len, int noblock, unsigned flags,
...
int udp_read(struct sock *sk, unsigned char *buff, int len, int noblock, unsigned flags)
{
...
int udp_rcv(struct sk_buff *skb, struct device *dev, struct options *opt, unsigned long daddr,
...
struct proto udp_prot = {sock_wmalloc,
                         sock_rmalloc,
                         sock_wfree,
                         sock_rfree,
                         sock_rspace,
