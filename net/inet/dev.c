...
static struct packet_type ipx_8023_type = {NET16(ETH_P_802_3), 0, ipx_rcv, NULL, NULL};

static struct packet_type ipx_packet_type = {NET16(ETH_P_IPX), 0, ipx_rcv, NULL, &ipx_8023_type};

#endif

#ifdef CONFIG_AX25

static struct packet_type ax25_packet_type = {NET16(ETH_P_AX25), 0, ax25_rcv, NULL,
#ifdef CONFIG_IPX
                                              &ipx_packet_type
#else
                                              NULL
...
static struct packet_type arp_packet_type = {NET16(ETH_P_ARP), 0, /* copy */
                                             arp_rcv, NULL,
#ifdef CONFIG_IPX
#ifndef CONFIG_AX25
                                             &ipx_packet_type
...
int dev_rint(unsigned char *buff, long len, int flags, struct device *dev)
{
    ...
    if (flags & IN_SKBUFF) {
        ...
        len2 = len;
        while (len2 > 0) {
            amount = min(len2, (unsigned long)dev->rmem_end - (unsigned long)buff);
            ...
            if (ptype->type == type || ptype->type == NET16(ETH_P_ALL)) {
                ...
                if (ptype->copy || nitcount) { /* copy if we need to    */
                    ...
                    skb2->h.raw = (unsigned char *)((unsigned long)skb2 +
                                                    (unsigned long)skb->h.raw - (unsigned long)skb);
                    skb2->free = 1;
                } else {
                    skb2 = skb;
                }
...
int dev_get_info(char *buffer)
{
    ...
    pos +=
        sprintf(pos, "Inter-|   Receive                  |  Transmit\n"
...
static int dev_ifsioc(void *arg, unsigned int getset)
{
    ...
    case SIOCSIFFLAGS: {
        ...
        dev->flags =
            ifr.ifr_flags & (IFF_UP | IFF_BROADCAST | IFF_DEBUG | IFF_LOOPBACK | IFF_POINTOPOINT |
        ...
        if ((old_flags & IFF_UP) && ((dev->flags & IFF_UP) == 0)) {
            ...
            ret = (!(old_flags & IFF_UP) && (dev->flags & IFF_UP)) ? dev_open(dev) : 0;
            if (ret < 0)
