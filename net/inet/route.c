 ...
 */
void rt_add(short flags, unsigned long dst, unsigned long mask, unsigned long gw,
            struct device *dev)
{
    struct rtable *r, *rt;
    struct rtable **rp;
...
int rt_get_info(char *buffer)
{
    ...
    for (r = rt_base; r != NULL; r = r->rt_next) {
        pos +=
            sprintf(pos, "%s\t%08lX\t%08lX\t%02X\t%d\t%lu\t%d\t%08lX\n", r->rt_dev->name, r->rt_dst,
                    r->rt_gateway, r->rt_flags, r->rt_refcnt, r->rt_use, r->rt_metric, r->rt_mask);
    }
    return (pos - buffer);
}
