...
static int do_options(struct iphdr *iph, struct options *opt)
{
    ...
    if (opt->record_route.route_size == 0) {
        if (opt->strict_route.route_size != 0) {
            memcpy(&(opt->record_route), &(opt->strict_route), sizeof(opt->record_route));
        } else if (opt->loose_route.route_size != 0) {
            memcpy(&(opt->record_route), &(opt->loose_route), sizeof(opt->record_route));
        }
    }
...
static void ip_expire(unsigned long arg)
{
    ...
    if (qp->fragments != NULL)
        icmp_send(qp->fragments->skb, ICMP_TIME_EXCEEDED, ICMP_EXC_FRAGTIME, qp->dev);
...
void ip_fragment(struct sock *sk, struct sk_buff *skb, struct device *dev, int is_frag)
{
    ...
    DPRINTF((DBG_IP, "    DEV=%s, MTU=%d, LEN=%d SRC=%s", dev->name, dev->mtu, left,
             in_ntoa(iph->saddr)));
...
static void ip_forward(struct sk_buff *skb, struct device *dev, int is_frag)
{
   ...
   */
    ...
    if (dev2->flags & IFF_UP) {
        skb2 = (struct sk_buff *)alloc_skb(
            sizeof(struct sk_buff) + dev2->hard_header_len + skb->len, GFP_ATOMIC);
        if (skb2 == NULL) {
...
int ip_rcv(struct sk_buff *skb, struct device *dev, struct packet_type *pt)
{
   ...
   */
    ...
    for (ipprot = (struct inet_protocol *)inet_protos[hash]; ipprot != NULL;
    ...
    */
        if (ipprot->copy) {
            ...
            skb2->mem_addr = skb2;
            skb2->ip_hdr = (struct iphdr *)((unsigned long)skb2 + (unsigned long)skb->ip_hdr -
                                            (unsigned long)skb);
            skb2->h.raw = (unsigned char *)((unsigned long)skb2 + (unsigned long)skb->h.raw -
                                            (unsigned long)skb);
            skb2->free = 1;
        } else {
            skb2 = skb;
    ...
    */
        ipprot->handler(skb2, dev, opts_p ? &opt : 0, iph->daddr,
                        (ntohs(iph->tot_len) - (iph->ihl * 4)), iph->saddr, 0, ipprot);
    }
