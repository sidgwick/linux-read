...
static char *version = "at1700.c:v0.06 3/3/94  Donald Becker (becker@super.org)\n";
...
int at1700_probe1(struct device *dev, short ioaddr)
{
    ...
    printk("%s: AT1700 found at %#3x, IRQ %d, address ", dev->name, ioaddr, irq);
       ...
       */
    {
        ...
        printk(" %s interface (%04x).\n", porttype[(dev->if_port >> 3) & 3], setup_value);
    }
...
static int net_send_packet(struct sk_buff *skb, struct device *dev)
{
    ...
    if (dev->tbusy) {
        ...
        if (tickssofar < 10)
            return 1;
        printk("%s: transmit timed out with status %04x, %s?\n", dev->name, inw(ioaddr + STATUS),
               inb(ioaddr + TX_STATUS) & 0x80 ? "IRQ conflict" : "network cable problem");
        printk("%s: timeout registers: %04x %04x %04x %04x %04x %04x %04x %04x.\n", dev->name,
               inw(ioaddr + 0), inw(ioaddr + 2), inw(ioaddr + 4), inw(ioaddr + 6), inw(ioaddr + 8),
               inw(ioaddr + 10), inw(ioaddr + 12), inw(ioaddr + 14));
        lp->stats.tx_errors++;
