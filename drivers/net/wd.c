...
static char *version = "wd.c:v0.99-14 11/21/93 Donald Becker (becker@super.org)\n";
...
static void wd_block_output(struct device *dev, int count, const unsigned char *buf,
                            const start_page);
...
int wdprobe1(int ioaddr, struct device *dev)
{
    ...
    if (inb(ioaddr + 0) == 'P' && inb(ioaddr + 1) == 'D') {
        ...
        if (!ancient && (inb(ioaddr + 1) & 0x01) != (word16 & 0x01))
            printk("\nWD80?3: Bus width conflict, %d (probe) != %d (reg report).", word16 ? 16 : 8,
    ...
    printk(" %s, IRQ %d, shared memory at %#lx-%#lx.\n", model_name, dev->irq, dev->mem_start,
           dev->mem_end - 1);
    if (ei_debug > 0)
        printk(version);
