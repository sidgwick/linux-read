...
static char *version = "hp.c:v0.99.15k 3/3/94 Donald Becker (becker@super.org)\n";
...
static int hp_block_input(struct device *dev, int count, char *buf, int ring_offset);
static void hp_block_output(struct device *dev, int count, const unsigned char *buf,
                            const start_page);
...
int hpprobe1(struct device *dev, int ioaddr)
{
    ...
    if (inb(ioaddr) != 0x08 || inb(ioaddr + 1) != 0x00 || inb(ioaddr + 2) != 0x09 ||
        inb(ioaddr + 14) == 0x57)
        return ENODEV;
...
static void hp_init_card(struct device *dev)
{
    int irq = dev->irq;
    NS8390_init(dev, 0);
    outb_p(irqmap[irq & 0x0f] | HP_RUN, dev->base_addr - NIC_OFFSET + HP_CONFIGURE);
    return;
}
