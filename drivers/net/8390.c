...
static char *version = "8390.c:v0.99-15e 2/16/94 Donald Becker (becker@super.org)\n";
...
static void NS8390_trigger_send(struct device *dev, unsigned int length, int start_page);
...
void ei_interrupt(int reg_ptr)
{
    ...
    if (dev->interrupt || ei_local->irqlock) {
        ...
        printk(ei_local->irqlock
               ...
               dev->name, inb_p(e8390_base + EN0_ISR), inb_p(e8390_base + EN0_IMR));
        return;
    }
...
static void ei_receive(struct device *dev)
{
    ...
    while (++rx_pkt_count < 10) {
        ...
        /* Check for bogosity warned by 3c503 book: the status byte is never
           ...
           cleaned up someday. */
        if (rx_frame.next != next_frame && rx_frame.next != next_frame + 1 &&
