...
static char *version = "3c503.c:v0.99.15k 3/3/93 Donald Becker (becker@super.org)\n";
...
static void el2_block_output(struct device *dev, int count, const unsigned char *buf,
                             const start_page);
static int el2_block_input(struct device *dev, int count, char *buf, int ring_offset);
...
int el2probe1(int ioaddr, struct device *dev)
{
    ...
    if ((membase_reg & 0xf0) == 0) {
        ...
        dev->mem_start =
            ((membase_reg & 0xc0) ? 0xD8000 : 0xC8000) + ((membase_reg & 0xA0) ? 0x4000 : 0);
...
static void el2_reset_8390(struct device *dev)
{
    if (ei_debug > 1) {
        ...
        printk("%#x=%#02x %#x=%#02x %#x=%#02x...", E33G_IDCFR, inb(E33G_IDCFR), E33G_CNTRL,
               inb(E33G_CNTRL), E33G_GACFR, inb(E33G_GACFR));
    }
                             ...
                             const start_page)
{
    ...
    if (dev->mem_start) { /* Shared memory transfer */
        void *dest_addr = (void *)(dev->mem_start + ((start_page - ei_status.tx_start_page) << 8));
    ...
    outb_p((ei_status.interface_num ? ECNTRL_AUI : ECNTRL_THIN) | ECNTRL_OUTPUT | ECNTRL_START,
           E33G_CNTRL);
...
static int el2_block_input(struct device *dev, int count, char *buf, int ring_offset)
{
    ...
    outb_p((ei_status.interface_num == 0 ? ECNTRL_THIN : ECNTRL_AUI) | ECNTRL_INPUT | ECNTRL_START,
           E33G_CNTRL);
