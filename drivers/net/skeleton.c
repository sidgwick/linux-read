...
static char *version = "skeleton.c:v0.05 11/16/93 Donald Becker (becker@super.org)\n";
...
int netcard_probe1(struct device *dev, short ioaddr)
{
    ...
    if (station_addr[0] != SA_ADDR0 || station_addr[1] != SA_ADDR1 || station_addr[2] != SA_ADDR2) {
        return ENODEV;
    }

    printk("%s: %s found at %#3x, IRQ %d.\n", dev->name, "network card", dev->base_addr, dev->irq);
...
static void net_rx(struct device *dev)
{
    ...
    do {
        ...
        if (status & 0x40) { /* There was an error. */
            ...
            memcpy(skb->data, (void *)dev->rmem_start, pkt_len);
            /* or */
