...
int el3_probe(struct device *dev)
{
    ...
    if (dev->base_addr != 0 && dev->base_addr != (unsigned short)ioaddr) {
        return -ENODEV;
    }
...
found:
...

    {
        ...
        printk("%s: 3c509 at %#3.3x tag %d, %s port, address ", dev->name, dev->base_addr,
               current_tag, if_names[dev->if_port]);
    }
    ...
    { /* Error-checking code, delete for 1.00. */
        ...
        if (status & 0x0001                    /* IRQ line active, missed one. */
            ...
            printk("%s: Missed interrupt, status then %04x now %04x"
                   "  Tx %2.2x Rx %4.4x.\n",
                   dev->name, status, inw(ioaddr + EL3_STATUS), inb(ioaddr + TX_STATUS),
...
static int el3_rx(struct device *dev)
{
    ...
    while ((rx_status = inw(ioaddr + RX_STATUS)) > 0) {
        ...
        if ((!(rx_status & 0x4000)) || !(rx_status & 0x1000)) { /* Dribble bits are OK. */
