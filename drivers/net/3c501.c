...
static char *version = "3c501.c: 3/3/94 Donald Becker (becker@super.org).\n";
...
int el1_probe(struct device *dev)
{
    ...
    printk("%s: 3c501 EtherLink at %#x, using %sIRQ %d, melting ethernet.\n", dev->name,
           dev->base_addr, autoirq ? "auto" : "assigned ", dev->irq);
    ...
    if (dev->tbusy) {
        ...
        if (el_debug)
            printk("%s: transmit timed out, txsr %#2x axsr=%02x rxsr=%02x.\n", dev->name,
                   inb(TX_STATUS), inb(AX_STATUS), inb(RX_STATUS));
    ...
    if (dev->tbusy) {
        ...
        if ((axsr & 0x80) && (txsr & TX_READY) == 0) {
            printk("%s: Unusual interrupt during Tx, txsr=%02x axsr=%02x"
                   " gp=%03x rp=%03x.\n",
                   dev->name, txsr, axsr, inw(ioaddr + EL1_DATAPTR), inw(ioaddr + EL1_RXPTR));
            dev->tbusy = 0;
            mark_bh(INET_BH);
