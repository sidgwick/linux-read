...
unsigned long lance_init(unsigned long mem_start, unsigned long mem_end)
{
    ...
    for (port = &ports[0]; *port; port++) {
        ...
        if (check_region(ioaddr, LANCE_TOTAL_SIZE) == 0 && inb(ioaddr + 14) == 0x57 &&
            inb(ioaddr + 15) == 0x57) {
...
unsigned long lance_probe1(short ioaddr, unsigned long mem_start)
{
    ...
    dev = init_etherdev(
        0, sizeof(struct lance_private) + PKT_BUF_SZ * (RX_RING_SIZE + TX_RING_SIZE), &mem_start);
...
static int lance_open(struct device *dev)
{
    ...
    if (lance_debug > 1)
        printk("%s: lance_open() irq %d dma %d tx/rx rings %#x/%#x init %#x.\n", dev->name,
               dev->irq, dev->dma, (int)lp->tx_ring, (int)lp->rx_ring, (int)&lp->init_block);
    ...
    if (lance_debug > 2)
        printk("%s: LANCE open after %d ticks, init block %#x csr0 %4.4x.\n", dev->name, i,
               (int)&lp->init_block, inw(ioaddr + LANCE_DATA));
...
static int lance_start_xmit(struct sk_buff *skb, struct device *dev)
{
    ...
    if (dev->tbusy) {
        ...
        {
            int i;
            printk(" Ring data dump: dirty_tx %d cur_tx %d cur_rx %d.", lp->dirty_tx, lp->cur_tx,
                   lp->cur_rx);
            for (i = 0; i < RX_RING_SIZE; i++)
                printk("%s %08x %04x %04x", i & 0x3 ? "" : "\n ", lp->rx_ring[i].base,
                       -lp->rx_ring[i].buf_length, lp->rx_ring[i].msg_length);
            for (i = 0; i < TX_RING_SIZE; i++)
                printk(" %s%08x %04x %04x", i & 0x3 ? "" : "\n ", lp->tx_ring[i].base,
                       -lp->tx_ring[i].length, lp->tx_ring[i].misc);
            printk("\n");
        }
#endif
        lance_init_ring(dev);
    ...
    if (lp->old_lance) {
        lp->tx_ring[entry].length = -(ETH_ZLEN < skb->len ? skb->len : ETH_ZLEN);
    } else
    ...
    if ((int)(skb->data) + skb->len > 0x01000000) {
        ...
        lp->tx_ring[entry].base = (int)(lp->tx_bounce_buffs + entry) | 0x83000000;
        if (skb->free)
...
struct netdev_entry lance_drv = {"lance", lance_probe1, LANCE_TOTAL_SIZE, lance_portlist};
#endif
