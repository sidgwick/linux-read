...
static char *version = "Net2Debugged PLIP 1.01 (from plip.c:v0.15 for 0.99pl12+, 8/11/93)\n";
...
static int plip_header(unsigned char *buff, struct device *dev, unsigned short type,
                       unsigned long h_dest, unsigned long h_source, unsigned len);
...
static int plip_rebuild_enethdr(struct device *dev, struct ethhdr *eth, unsigned char h_dest,
                                unsigned char h_source, unsigned short type);
...
static int get_byte(struct device *dev)
{
    ...
    val = inb(dev->base_addr + PAR_STATUS);
    PRINTK2(("%02x %s ", low_nibble | ((val << 1) & 0xf0), error ? "t" : ""));
...
static int plip_send_packet(struct device *dev, unsigned char *buf, int length)
{
    ...
    PRINTK2(("%s: plip_send_packet(%d) %02x %02x %02x %02x %02x...", dev->name, length, buf[0],
...
static int plip_addrcmp(struct ethhdr *eth)
{
    ...
    PRINTK2(("h_dest = %08x%04x h_source = %08x%04x\n", *(long *)&eth->h_dest[2],
             *(short *)&eth->h_dest[0], *(long *)&eth->h_source[2], *(short *)&eth->h_source[0]));
    return 0;
}
...
static int plip_rebuild_enethdr(struct device *dev, struct ethhdr *eth, unsigned char dest,
                                unsigned char source, unsigned short type)
{
    eth->h_proto = type;
...
static void double_timeoutfactor()
{
    timeoutfactor *= 2;
