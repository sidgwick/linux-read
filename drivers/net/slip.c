...
static void ip_dump(unsigned char *ptr, int len)
{
#ifdef SL_DUMP
    struct iphdr *ip;
    struct tcphdr *th;
    ...
    printk("%s\n", in_ntoa(ip->daddr));
    printk(" len %u ihl %u ver %u ttl %u prot %u", dlen, ip->ihl, ip->version, ip->ttl,
           ip->protocol);
...
static void sl_initialize(struct slip *sl, struct device *dev)
{
    sl->inuse = 0;
    sl->sending = 0;
    sl->escape = 0;
...
static struct slip *sl_find(struct tty_struct *tty)
{
    struct slip *sl;
    int i;
...
static inline struct slip *sl_alloc(void)
{
    unsigned long flags;
    struct slip *sl;
    int i;
...
/* Free a SLIP channel. */
static inline void sl_free(struct slip *sl)
{
    unsigned long flags;
...
static inline void sl_enqueue(struct slip *sl, unsigned char c)
{
    unsigned long flags;
...
static inline void sl_dequeue(struct slip *sl, int i)
{
    unsigned long flags;
...
static unsigned short sl_type_trans(struct sk_buff *skb, struct device *dev)
{
#ifdef CONFIG_AX25
...
static int sl_header(unsigned char *buff, struct device *dev, unsigned short type,
...
static void sl_add_arp(unsigned long addr, struct sk_buff *skb, struct device *dev)
{
#ifdef CONFIG_AX25
    struct slip *sl = &sl_ctrl[dev->base_addr];
...
static int sl_rebuild_header(void *buff, struct device *dev)
{
#ifdef CONFIG_AX25
    struct slip *sl = &sl_ctrl[dev->base_addr];
 ...
 */
static void slip_recv(struct tty_struct *tty)
{
    ...
    if (sl->mtu !=
        sl->dev->mtu) /* Argh! mtu change time! - costs us the packet part received at the change */
        sl_changedmtu(sl);
...
static struct enet_statistics *sl_get_stats(struct device *dev)
{
...
#ifdef NOTDEF
                printk("rbuff %02x %02x %02x %02x\n", sl->rbuff[0], sl->rbuff[1], sl->rbuff[2],
                       sl->rbuff[3]);
#endif
                sl_bump(sl);
            }
...
int slip_init(struct device *dev)
{
    ...
    int i;
#ifdef CONFIG_AX25
    static char ax25_bcast[7] = {'Q' << 1, 'S' << 1, 'T' << 1, ' ' << 1,
                                 ' ' << 1, ' ' << 1, '0' << 1};
    static char ax25_test[7] = {'L' << 1, 'I' << 1, 'N' << 1, 'U' << 1,
                                'X' << 1, ' ' << 1, '1' << 1};
#endif
    ...
    if (already++ == 0) {
        ...
        sl_ldisc.write = NULL;
        sl_ldisc.ioctl =
            (int (*)(struct tty_struct *, struct file *, unsigned int, unsigned long))slip_ioctl;
