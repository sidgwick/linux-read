...
int depca_probe(struct device *dev)
{
    ...
    if (base_addr > 0x1ff) { /* Check a single specified location. */
        ...
        for (status = -ENODEV, port = &ports[0]; *port && (num_depcas < MAX_NUM_DEPCAS); port++) {
            int ioaddr = *port;
        ...
        */
                    if ((dev->next != (struct device *)NULL) && (num_eth > 0) && (num_eth < 9999)) {
        ...
        if (eth0)
            dev = eth0; /* restore the first device */
    }
...
static int depca_probe1(struct device *dev, short ioaddr)
{
    ...
    */
            ...
            mem_start =
                ((((unsigned long)dev->priv + sizeof(struct depca_private)) + (unsigned long)0x07) &
                 (unsigned long)~0x07);
    ...
    */
            lp->ringSize = j;
            if (lp->ringSize > RING_SIZE)
                lp->ringSize = RING_SIZE;
...
static int depca_open(struct device *dev)
{
    ...
    */
    ...
    if (depca_debug > 1) {
        ...
        for (i = 0; i < lp->ringSize; i++) {
            printk("\t0x%8.8lx  0x%8.8lx\n", (long)lp->rx_ring[i].base, (long)lp->tx_ring[i].base);
        }
...
static void depca_init_ring(struct device *dev)
{
    ...
    for (i = 0; i < lp->ringSize; i++) {
        ...
        lp->tx_ring[i].base =
            (lp->dma_buffs + (i + lp->ringSize) * PKT_BUF_SZ) & (unsigned long)(0x00ffffff);
    }
    ...
    */
    { /* Fill in a Tx ring entry */
      ...
      */
        ...
        if (depca_debug > 4) {
            unsigned char *pkt = (unsigned char *)(lp->tx_ring[entry].base & 0x00ffffff);

            printk("%s: tx ring[%d], %#lx, sk_buf %#lx len %d.\n", dev->name, entry,
                   (unsigned long)&lp->tx_ring[entry], lp->tx_ring[entry].base,
                   -lp->tx_ring[entry].length);
            printk("%s:  Tx %2.2x %2.2x %2.2x ... %2.2x  %2.2x %2.2x %2.2x...%2.2x len %2.2x %2.2x "
                   " %2.2x %2.2x.\n",
                   dev->name, pkt[0], pkt[1], pkt[2], pkt[5], pkt[6], pkt[7], pkt[8], pkt[11],
...
static int depca_rx(struct device *dev)
{
    ...
    for (; lp->rx_ring[entry].base >= 0; entry = (++lp->cur_rx) & lp->rmask) {
        ...
        if (status & R_ERR) {      /* There was an error. */
            ...
            skb->len = pkt_len;
            skb->dev = dev;
            memcpy(skb->data, (unsigned char *)(lp->rx_ring[entry].base & 0x00ffffff), pkt_len);
            /*
    ...
    if (i != 100) {
        ...
        if (depca_debug > 2) {
            printk("%s: DEPCA open after %d ticks, init block %#lx csr0 %4.4x.\n", dev->name, i,
                   (long)&lp->init_block, inw(DEPCA_DATA));
        }
    } else {
        status = -1;
