...
static char *version = "eexpress.c:v0.07 1/19/94 Donald Becker (becker@super.org)\n";
...
static short init_words[] = {
    ...
    0x0000,
    0x0000, /* Set control mailbox (SCB) addr. */
    0,
    0,      /* pad to 0x000000. */
...
#endif
        if (sum == 0xbaba && eexp_probe1(dev, *port) == 0)
            return 0;
    }
...
static int eexp_open(struct device *dev)
{
    ...
    if (irq2dev_map[dev->irq] != 0
        ...
        || (irq2dev_map[dev->irq] = dev) == 0 || request_irq(dev->irq, &eexp_interrupt)) {
        return -EAGAIN;
    }
    ...
    if (dev->tbusy) {
        ...
        if (net_debug > 1)
            printk("%s: transmit timed out, %s?  ", dev->name,
                   inw(ioaddr + SCB_STATUS) & 0x8000 ? "IRQ conflict" : "network cable problem");
        lp->stats.tx_errors++;
...
static void hardware_send_packet(struct device *dev, void *buf, short length)
{
    ...
    if (net_debug > 4) {
        printk("%s: EExp @%x send length = %d, tx_block %3x, next %3x, "
               "reap %4x status %4.4x.\n",
               dev->name, ioaddr, length, tx_block, lp->tx_head, lp->tx_reap,
...
static void eexp_rx(struct device *dev)
{
    ...
    while ((frame_status = inw(ioaddr)) < 0) { /* Command complete */
        ...
        if (next_rx_frame != rx_head + RX_BUF_SIZE && next_rx_frame != RX_BUF_START) {
        ...
        if (inw(ioaddr) != rx_head) {
            printk("%s: Rx buf link mismatch, at %04x link %04x instead of %04x.\n", dev->name,
                   rx_tail, (outw(rx_tail + 4, ioaddr + READ_PTR), inw(ioaddr)), rx_head);
            outw(rx_head, ioaddr);
        }
#endif
