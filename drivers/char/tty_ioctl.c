...
static void unset_locked_termios(struct termios *termios, struct termios *old,
...
static int set_termios_2(struct tty_struct *tty, struct termios *termios)
{
    ...
    *tty->termios = *termios;
    ...
    if (tty->link && tty->link->packet) {
        int old_flow = ((old_termios.c_iflag & IXON) && (old_termios.c_cc[VSTOP] == '\023') &&
                        (old_termios.c_cc[VSTART] == '\021'));
        int new_flow = (I_IXON(tty) && STOP_CHAR(tty) == '\023' && START_CHAR(tty) == '\021');
    ...
    unset_locked_termios(tty->termios, &old_termios, termios_locked[tty->line]);
...
static int set_window_size(struct tty_struct *tty, struct winsize *ws)
{
    ...
    if (memcmp(&tmp_ws, &tty->winsize, sizeof(struct winsize)) && tty->pgrp > 0)
...
static int tty_set_ldisc(struct tty_struct *tty, int ldisc)
{
    if ((ldisc < N_TTY) || (ldisc >= NR_LDISCS) || !(ldiscs[ldisc].flags & LDISC_FLAG_DEFINED))
        return -EINVAL;
...
static unsigned long inq_canon(struct tty_struct *tty)
{
    ...
    /* Skip EOF-chars.. */
    while (head != tail) {
        if (test_bit(tail, &tty->secondary_flags) && tty->secondary.buf[tail] == __DISABLED_CHAR)
            nr--;
        INC(tail);
    }
    return nr;
...
int tty_ioctl(struct inode *inode, struct file *file, unsigned int cmd, unsigned long arg)
{
    ...
    switch (cmd) {
    case TCGETS:
        retval = verify_area(VERIFY_WRITE, (void *)arg, sizeof(struct termios));
        if (retval)
            return retval;
        memcpy_tofs((struct termios *)arg, termios_tty->termios, sizeof(struct termios));
        return 0;
    case TCSETSF:
    case TCSETSW:
    case TCSETS:
    ...
    case TIOCSCTTY:
        ...
        if (tty->session > 0) {
            ...
            if ((arg == 1) && suser()) {
                ...
                for_each_task(p) if (p->tty == dev) p->tty = -1;
            } else
                return -EPERM;
        }
        current->tty = dev;
        ...
        return 0;
    case TIOCGPGRP:
        retval = verify_area(VERIFY_WRITE, (void *)arg, sizeof(pid_t));
        if (retval)
            return retval;
    ...
    case TIOCSPGRP:
        ...
        if (retval)
            return retval;
        if ((current->tty < 0) || (current->tty != termios_dev) ||
        ...
        return 0;
    case TIOCOUTQ:
        retval = verify_area(VERIFY_WRITE, (void *)arg, sizeof(unsigned long));
        if (retval)
            return retval;
    ...
    case TIOCINQ:
        ...
        if (retval)
            return retval;
        if (L_ICANON(tty))
            put_fs_long(inq_canon(tty), (unsigned long *)arg);
        else
    ...
    case TIOCSTI:
        ...
        return 0;
    case TIOCGWINSZ:
        retval = verify_area(VERIFY_WRITE, (void *)arg, sizeof(struct winsize));
        if (retval)
            return retval;
    ...
    case TIOCGLCKTRMIOS:
        ...
        if (retval)
            return retval;
        memcpy_tofs((struct termios *)arg, &termios_locked[termios_dev], sizeof(struct termios));
        return 0;
    case TIOCSLCKTRMIOS:
        if (!suser())
            return -EPERM;
        arg = get_fs_long((unsigned long *)arg);
        memcpy_fromfs(&termios_locked[termios_dev], (struct termios *)arg, sizeof(struct termios));
        return 0;
    case TIOCPKT:
        if (!IS_A_PTY_MASTER(dev))
            return -ENOTTY;
        retval = verify_area(VERIFY_READ, (void *)arg, sizeof(unsigned long));
        if (retval)
            return retval;
        if (get_fs_long(arg)) {
            if (!tty->packet) {
    ...
    default:
        ...
        if (ldiscs[tty->disc].ioctl) {
            retval = (ldiscs[tty->disc].ioctl)(tty, file, cmd, arg);
            return retval;
        }
        return -EINVAL;
    }
