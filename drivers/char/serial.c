 ...
 */
static int baud_table[] = {0,    50,   75,   110,  134,   150,   200,   300,    600, 1200,
                           1800, 2400, 4800, 9600, 19200, 38400, 57600, 115200, 0};
...
static void rs_start(struct tty_struct *tty)
{
    ...
    info->IER = (UART_IER_MSI | UART_IER_RLSI | UART_IER_THRI | UART_IER_RDI);
#ifdef ISR_HACK
    serial_out(info, UART_IER, info->IER);
#endif
}
...
static inline void receive_chars(struct async_struct *info, int *status)
{
...
#define VLEFT ((tail - head - 1) & (TTY_BUF_SIZE - 1))
    ...
    queue->head = head;
    if ((VLEFT < RQ_THRESHOLD_LW) && !set_bit(TTY_RQ_THROTTLED, &info->tty->flags))
...
static inline int check_modem_status(struct async_struct *info)
{
    ...
    if ((status & UART_MSR_DDCD) && !C_CLOCAL(info->tty)) {
        ...
        printk("ttys%d CD now %s...", info->line, (status & UART_MSR_DCD) ? "on" : "off");
#endif
...
static void rs_interrupt(int irq)
{
    ...
    while (info) {
        if (info->tty && info->tty->termios &&
            (!pass_number || !(serial_inp(info, UART_IIR) & UART_IIR_NO_INT))) {
            done = 0;
        ...
        recheck_write:
            ...
            if (check_modem_status(info) && (recheck_count++ <= 64))
                goto recheck_write;
#ifdef SERIAL_DEBUG_INTR
            if (recheck_count > 16)
...
static int startup(struct async_struct *info, int get_irq)
{
    ...
    if (info->flags & ASYNC_FOURPORT)
        ...
        serial_outp(info, UART_MCR, UART_MCR_DTR | UART_MCR_RTS | UART_MCR_OUT2);
    ...
    if (info->tty && info->tty->termios && I_INPCK(info->tty))
        info->read_status_mask = (UART_LSR_OE | UART_LSR_BI | UART_LSR_FE | UART_LSR_PE);
    else
        info->read_status_mask = (UART_LSR_OE | UART_LSR_BI | UART_LSR_FE);
...
#ifdef SERIAL_DEBUG_OPEN
    printk("Shutting down serial port %d (irq %d)....", info->line, info->irq);
#endif
...
static void change_speed(unsigned int line)
{
    ...
    if (quot) {
        info->timeout = ((info->xmit_fifo_size * HZ * 15 * quot) / info->baud_base) + 2;
...
static void rs_throttle(struct tty_struct *tty, int status)
{
    ...
    printk("throttle tty%d: %d (%d, %d)....\n", DEV_TO_SL(tty->line), status, LEFT(&tty->read_q),
           LEFT(&tty->secondary));
#endif
    switch (status) {
...
static int set_serial_info(struct async_struct *info, struct serial_struct *new_info)
{
    ...
    if (!suser()) {
        if (change_irq || change_port || (new_serial.baud_base != info->baud_base) ||
            (new_serial.type != info->type) || (new_serial.close_delay != info->close_delay) ||
    ...
    if ((new_serial.irq > 15) || (new_serial.port > 0xffff) || (new_serial.type < PORT_UNKNOWN) ||
    ...
    for (i = 0; i < NR_PORTS; i++)
        if ((info != &rs_table[i]) && (rs_table[i].port == new_serial.port) && rs_table[i].type)
            return -EADDRINUSE;
...
static int set_modem_info(struct async_struct *info, unsigned int cmd, unsigned int *value)
{
    ...
    case TIOCMSET:
        control = (control & ~(UART_MCR_RTS | UART_MCR_DTR)) |
                  ((arg & TIOCM_RTS) ? UART_MCR_RTS : 0) | ((arg & TIOCM_DTR) ? UART_MCR_DTR : 0);
        break;
    default:
        return -EINVAL;
    }
...
static int check_wild_interrupts(int doprint)
{
    ...
    for (i = 0, mask = 1; i < 16; i++, mask <<= 1) {
        if ((rs_triggered & (1 << i)) && (irq_lines & (1 << i))) {
...
static void rs_set_termios(struct tty_struct *tty, struct termios *old_termios)
{
    ...
    if (!(old_termios->c_cflag & CLOCAL) && (tty->termios->c_cflag & CLOCAL))
        wake_up_interruptible(&info->open_wait);
...
static int block_til_ready(struct tty_struct *tty, struct file *filp, struct async_struct *info)
{
    ...
    if (MAJOR(filp->f_rdev) == TTYAUX_MAJOR) {
        if (info->flags & ASYNC_NORMAL_ACTIVE)
            return -EBUSY;
        if ((info->flags & ASYNC_CALLOUT_ACTIVE) && (info->flags & ASYNC_SESSION_LOCKOUT) &&
            (info->session != current->session))
            return -EBUSY;
        if ((info->flags & ASYNC_CALLOUT_ACTIVE) && (info->flags & ASYNC_PGRP_LOCKOUT) &&
    ...
    while (1) {
        ...
        if (!(info->flags & ASYNC_CALLOUT_ACTIVE) && !(info->flags & ASYNC_CLOSING) &&
            (do_clocal || (serial_in(info, UART_MSR) & UART_MSR_DCD)))
            break;
...
long rs_init(long kmem_start)
{
    ...
    for (i = 0, info = rs_table; i < NR_PORTS; i++, info++) {
        ...
        printk("tty%02d%s at 0x%04x (irq = %d)", info->line,
               (info->flags & ASYNC_FOURPORT) ? " FourPort" : "", info->port, info->irq);
        switch (info->type) {
        case PORT_8250:
            printk(" is a 8250\n");
            break;
