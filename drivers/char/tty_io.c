...
static int hung_up_tty_select(struct inode *inode, struct file *filp, int sel_type,
                              select_table *wait)
{
    return 1;
}
...
static struct file_operations tty_fops = {tty_lseek,  tty_read,   tty_write, NULL, /* tty_readdir */
                                          tty_select, tty_ioctl,  NULL,            /* tty_mmap */
                                          tty_open,   tty_release};
...
void disassociate_ctty(int priv)
{
    ...
    for_each_task(p) if (p->session == current->session) p->tty = -1;
}
...
void complete_change_console(unsigned int new_console)
{
    ...
    if (vt_cons[new_console].vt_mode.mode == VT_PROCESS) {
         ...
         * is awry
         */
        if (kill_proc(vt_cons[new_console].vt_pid, vt_cons[new_console].vt_mode.acqsig, 1) != 0) {
            /*
...
void change_console(unsigned int new_console)
{
    ...
    if (vt_cons[fg_console].vt_mode.mode == VT_PROCESS) {
         ...
         * is awry
         */
        if (kill_proc(vt_cons[fg_console].vt_pid, vt_cons[fg_console].vt_mode.relsig, 1) == 0) {
            /*
...
static void copy_to_cooked(struct tty_struct *tty)
{
    ...
    while (1) {
        ...
        if (tty->throttle && (c < SQ_THRESHOLD_LW) && !set_bit(TTY_SQ_THROTTLED, &tty->flags))
        ...
        if (!EMPTY(&tty->read_q)) {
            ...
            special_flag = clear_bit(tty->read_q.tail, &tty->readq_flags);
            INC(tty->read_q.tail);
            restore_flags(flags);
        } else {
            restore_flags(flags);
        ...
        if (L_ICANON(tty) && !tty->lnext) {
            ...
            if (c == REPRINT_CHAR(tty) && L_ECHO(tty) && L_IEXTEN(tty)) {
                ...
                opost('\n', tty);
                while (tail != tty->secondary.head) {
                    echo_char(tty->secondary.buf[tail], tty);
                    INC(tail);
                }
                continue;
            }
        ...
        if (I_IXON(tty) && !tty->lnext) {
            if ((tty->stopped && I_IXANY(tty) && L_IEXTEN(tty)) || c == START_CHAR(tty)) {
                start_tty(tty);
                continue;
            }
            if (c == STOP_CHAR(tty)) {
        ...
        if (c == '\n' && !tty->lnext) {
            ...
            if (c != EOF_CHAR(tty) || !L_ICANON(tty) || tty->lnext) {
...
static int read_chan(struct tty_struct *tty, struct file *file, unsigned char *buf, unsigned int nr)
{
    ...
    if (file->f_inode->i_rdev != CONSOLE_DEV && current->tty == tty->line) {
        if (tty->pgrp <= 0)
            ...
            if (!nr) {
                ...
                if (eol) {
                    if (c == __DISABLED_CHAR) {
                        ...
                        set_bit(tty->secondary.tail, &tty->secondary_flags);
                    }
                }
                sti();
                break;
...
static int write_chan(struct tty_struct *tty, struct file *file, unsigned char *buf,
                      unsigned int nr)
{
    struct wait_queue wait = {current, NULL};
    int c;
...
repeat:
    ...
    if (!termios_locked[dev] && !ltp) {
        ltp = (struct termios *)kmalloc(sizeof(struct termios), GFP_KERNEL);
        if (!ltp)
            goto end_init;
    ...
    if (IS_A_PTY(dev)) {
        ...
        if (!termios_locked[o_dev] && !o_ltp) {
            o_ltp = (struct termios *)kmalloc(sizeof(struct termios), GFP_KERNEL);
            if (!o_ltp)
                goto end_init;
...
retry_open:
    ...
    if (!noctty && current->leader && current->tty < 0 && tty->session == 0) {
        current->tty = minor;
...
static int tty_select(struct inode *inode, struct file *filp, int sel_type, select_table *wait)
{
    ...
    if (ldiscs[tty->disc].select)
        return (ldiscs[tty->disc].select)(tty, inode, filp, sel_type, wait);
    return 0;
}

static int normal_select(struct tty_struct *tty, struct inode *inode, struct file *file,
...
void do_SAK(struct tty_struct *tty)
{
    ...
    for (p = &LAST_TASK; p > &FIRST_TASK; --p) {
        if (!(*p))
            continue;
        if (((*p)->tty == line) || ((session > 0) && ((*p)->session == session)))
            send_sig(SIGKILL, *p, 1);
        else {
            for (i = 0; i < NR_OPEN; i++) {
                filp = (*p)->filp[i];
                if (filp && (filp->f_op == &tty_fops) && (MINOR(filp->f_rdev) == line)) {
                    send_sig(SIGKILL, *p, 1);
                    break;
                }
            }
 ...
 */
int tty_write_data(struct tty_struct *tty, char *bufp, int buflen, void (*callback)(void *data),
                   void *callarg)
{
    int head, tail, count;
    unsigned long flags;
...
static void initialize_termios(int line, struct termios *tp)
{
    ...
    if (IS_A_CONSOLE(line) || IS_A_PTY_SLAVE(line)) {
        ...
        tp->c_lflag = ISIG | ICANON | ECHO | ECHOE | ECHOK | ECHOCTL | ECHOKE | IEXTEN;
    } else if (IS_A_SERIAL(line)) {
        tp->c_iflag = ICRNL | IXON;
