...
int vt_ioctl(struct tty_struct *tty, struct file *file, unsigned int cmd, unsigned long arg)
{
    ...
    case KDSKBSENT: {
        ...
        for (p = a->kb_string; get_fs_byte(p); p++)
            delta++;
        first_free = func_table[NR_FUNC - 1] + strlen(func_table[NR_FUNC - 1]) + 1;
        if (delta > 0 && first_free + delta > func_buf + FUNC_BUFSIZE)
            return -EINVAL;
        if (i < NR_FUNC - 1) {
            memmove(func_table[i + 1] + delta, func_table[i + 1], first_free - func_table[i + 1]);
    ...
    case KDGKBDIACR: {
        ...
        memcpy_tofs(a->kbdiacr, accent_table, accent_table_size * sizeof(struct kbdiacr));
        return 0;
    }
