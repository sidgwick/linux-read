...
#if !defined(TAPE_QIC02_PORT) || !defined(TAPE_QIC02_IRQ) || !defined(TAPE_QIC02_DMA)
...
static struct exception_list_type {
    ...
    {-1, TP_ST0 | TP_CNI | TP_USL | TP_WRP, "Drive not online" /* 2 */},
    {~(TP_ST1 | TP_BOM), TP_ST0 | TP_WRP, "Write protected cartridge" /* 3 */},
    {~(TP_ST1 | TP_EOR), TP_ST0 | TP_EOM, "End of media" /* 4 */},
     ...
     */
    ,
    {-1, TP_ST1, "Hmm, this must be Cypher drive... Aaargh" /* extra */}
#endif
};
...
static int tp_sense(int ignore)
{
    ...
    if (((err & TP_ST0) && (err & REPORT_ERR0)) || ((err & TP_ST1) && (err & REPORT_ERR1)))
        return TE_ERR;
    return TE_OK;
} /* tp_sense */
...
static void tape_qic02_interrupt(int unused)
{
    ...
    if (status_expect_int) {
        ...
        if (((stat & (AR_STAT_DMADONE)) == 0) && ((stat & (QIC_STAT_EXCEPTION)) != 0)) {
            TIMERCONT;
...
static int tape_qic02_ioctl(struct inode *inode, struct file *filp, unsigned int iocmd,
                            unsigned long ioarg)
{
    int error;
    short i;
 ...
 */
static struct sigaction tape_qic02_sigaction = {tape_qic02_interrupt, 0, SA_INTERRUPT, NULL};
