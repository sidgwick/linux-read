...
static unsigned char *translations[] = {
    ...
    (unsigned char *)"\000\001\002\003\004\005\006\007\000\011\000\013\000\000\000\000"
    ...
    (unsigned char *)"\000\001\002\003\004\005\006\007\010\011\000\013\000\000\016\017"
                     "\020\021\022\023\024\025\026\027\030\031\032\000\034\035\036\037"
                     "\040\041\042\043\044\045\046\047\050\051\052\053\054\055\056\057"
                     "\060\061\062\063\064\065\066\067\070\071\072\073\074\075\076\077"
                     "\100\101\102\103\104\105\106\107\110\111\112\113\114\115\116\117"
    ...
    while (!tty->stopped && (c = get_tty_queue(&tty->write_q)) >= 0) {
        ...
        case EShash:
            ...
            if (c == '8') {
                ...
                video_erase_char = (video_erase_char & 0xff00) | 'E';
                csi_J(currcons, 2);
                video_erase_char = (video_erase_char & 0xff00) | ' ';
            }
            continue;
        case ESsetG0:
            if (c == '0')
...
long con_init(long kmem_start)
{
    ...
    printk("Console: %s %s %ldx%ld, %d virtual consoles\n", can_do_color ? "colour" : "mono",
           display_desc, video_num_columns, video_num_lines, NR_CONSOLES);
...
static void set_scrmem(int currcons)
{
    ...
    if (video_type == VIDEO_TYPE_MDA) {
        if (vcmode == KD_TEXT) {
            ...
            int i;
            static char herc_txt_tbl[12] = {0x61, 0x50, 0x52, 0x0f, 0x19, 6,
                                            0x19, 0x19, 2,    0x0d, 0x0b, 0x0c};
...
int set_selection(const int arg)
{
    ...
    case 1: /* word-by-word selection */
        ...
        for (new_sel_start = ps;; ps -= 2) {
            if ((spc && !isspace(*(off + ps))) || (!spc && !inword(*(off + ps))))
                break;
            new_sel_start = ps;
        ...
        for (new_sel_end = pe;; pe += 2) {
            if ((spc && !isspace(*(off + pe))) || (!spc && !inword(*(off + pe))))
                break;
            new_sel_end = pe;
    ...
    case 2: /* line-by-line selection */
        ...
        new_sel_end = pe + video_size_row - pe % video_size_row - 2;
        break;
    }
    /* select to end of line if on trailing space */
    if (new_sel_end > new_sel_start && !atedge(new_sel_end) && isspace(*(off + new_sel_end))) {
...
int con_set_trans(char *arg)
{
    ...
    for (i = 0; i < E_TABSZ; i++)
        USER_TRANS[i] = get_fs_byte(arg + i);
    USER_TRANS[012] = 0;
    USER_TRANS[014] = 0;
    USER_TRANS[015] = 0;
    USER_TRANS[033] = 0;
...
int con_get_trans(char *arg)
{
    ...
    for (i = 0; i < E_TABSZ; i++)
        put_fs_byte(USER_TRANS[i], arg + i);
    return 0;
}
