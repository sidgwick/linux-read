...
static int internal_command(unsigned char target, unsigned char lun, const void *cmnd, void *buff,
                            int bufflen, int reselect);
...
static unsigned char controller_type =
...
#ifndef OVERRIDE
static const char *seagate_bases[] = {(char *)0xc8000, (char *)0xca000, (char *)0xcc000,
...
int seagate_st0x_detect(int hostnum)
{
    ...
    int i, j;
#endif
    static struct sigaction seagate_sigaction = {&seagate_reconnect_intr, 0, SA_INTERRUPT, NULL};
 ...
 */
        ...
        for (i = 0; i < (sizeof(seagate_bases) / sizeof(char *)); ++i)
            for (j = 0; !base_address && j < NUM_SIGNATURES; ++j)
                if (!memcmp((void *)(seagate_bases[i] + signatures[j].offset),
    ...
    if (base_address) {
        st0x_cr_sr = (void *)(((unsigned char *)base_address) +
                              (controller_type == SEAGATE ? 0x1a00 : 0x1c00));
        st0x_dr = (void *)(((unsigned char *)base_address) +
                           (controller_type == SEAGATE ? 0x1c00 : 0x1e00));
#ifdef DEBUG
        printk("ST0x detected. Base address = %x, cr = %x, dr = %x\n", base_address, st0x_cr_sr,
               st0x_dr);
#endif
        /*
...
#endif
            "\n",
            hostno, (controller_type == SEAGATE) ? "seagate" : "FD TMC-8xx", irq, base_address);
    return buffer;
}
...
static void seagate_reconnect_intr(int unused)
{
    ...
    if (!should_reconnect)
        ...
        temp = internal_command(current_target, current_lun, current_cmnd, current_data,
                                current_bufflen, RECONNECT_NOW);

        if (msg_byte(temp) != DISCONNECT) {
            if (done_fn) {
#if (DEBUG & PHASE_RESELECT)
                printk("scsi%d : done_fn(%d,%08x)", hostno, hostno, temp);
#endif
                if (!SCint)
 ...
 */
        ...
        if (linked_connected) {
            ...
            printk("scsi%d : using linked commands, current I_T_L nexus is ", hostno);
#endif
...
int seagate_st0x_command(Scsi_Cmnd *SCpnt)
{
    return internal_command(SCpnt->target, SCpnt->lun, SCpnt->cmnd, SCpnt->request_buffer,
                            SCpnt->request_bufflen, (int)NO_RECONNECT);
}
...
#if (((DEBUG & PHASE_ETC) == PHASE_ETC) || (DEBUG & PRINT_COMMAND) || (DEBUG & PHASE_EXIT))
    int i;
#endif
 ...
 */
        ...
        for (clock = jiffies + 10, temp = 0; (jiffies < clock) && !(STATUS & STAT_IO);)
            ;
 ...
 */
        ...
        sti();
        while (!((status_read = STATUS) & (STAT_ARB_CMPL | STAT_SEL)) && (jiffies < clock) &&
               !st0x_aborted)
            ;
 ...
 */
        ...
        CONTROL = BASE_CMD | CMD_DRVR_ENABLE | CMD_SEL | (reselect ? CMD_ATTN : 0);
        sti();
...
#if (DEBUG & DEBUG_SG)
            {
                ...
                for (i = 0; i < nobuffs; ++i)
                    printk("scsi%d : buffer %d address = %08x length = %d\n", hostno, i,
                           buffer[i].address, buffer[i].length);
            }
#endif
...
*/
            ...
            case REQ_DATAIN:
                ...
                if (borken) {
                    ...
                    transfered += len;
#endif
                    for (; len && (STATUS & (REQ_MASK | STAT_REQ)) == (REQ_DATAIN | STAT_REQ);
                         --len) {
                        *data++ = DATA;
                        borken_wait();
                    }
 ...
 */
            ...
            formatted_sectors = (data[4 + 1] << 16) | (data[4 + 2] << 8) | data[4 + 3];
