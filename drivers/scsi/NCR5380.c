...
static struct {
    ...
    char *name;
               ...
               {SR_REQ, "REQ"},    {SR_MSG, "MSG"}, {SR_CD, "CD"},
               {SR_IO, "IO"},      {SR_SEL, "SEL"}, {0, NULL}},
  basrs[] = {{BASR_ATN, "ATN"}, {BASR_ACK, "ACK"}, {0, NULL}},
  icrs[] = {{ICR_ASSERT_RST, "ASSERT RST"},
  ...
  mrs[] = {{MR_BLOCK_DMA_MODE, "MODE BLOCK DMA"},      {MR_TARGET, "MODE TARGET"},
           {MR_ENABLE_PAR_CHECK, "MODE PARITY CHECK"}, {MR_ENABLE_PAR_INTR, "MODE PARITY INTR"},
           {MR_MONITOR_BSY, "MODE MONITOR BSY"},       {MR_DMA_MODE, "MODE DMA"},
           {MR_ARBITRATE, "MODE ARBITRATION"},         {0, NULL}};
...
static struct {
    unsigned char value;
    char *name;
} phases[] = {{PHASE_DATAOUT, "DATAOUT"}, {PHASE_DATAIN, "DATAIN"}, {PHASE_CMDOUT, "CMDOUT"},
...
static void NCR5380_print_phase(struct Scsi_Host *instance)
{
    ...
    if (!(status & SR_REQ))
        ...
        for (i = 0;
             (phases[i].value != PHASE_UNKNOWN) && (phases[i].value != (status & PHASE_MASK)); ++i)
            ;
...
void NCR5380_timer_fn(void)
{
    ...
    for (; expires_first && expires_first->time_expires >= jiffies;) {
        instance = ((NCR5380_hostdata *)expires_first->host_data)->expires_next;
        ((NCR5380_hostdata *)expires_first->host_data)->expires_next = NULL;
        ((NCR5380_hostdata *)expires_first->host_data)->time_expires = 0;
...
static struct sigaction probe_sigaction = {probe_intr, 0, SA_INTERRUPT, NULL};
...
static int NCR5380_probe_irq(struct Scsi_Host *instance, int possible)
{
    ...
    for (trying_irqs = i = 0, mask = 1; i < 16; ++i, mask <<= 1)
        if ((mask & possible) && (irqaction(i, &probe_sigaction) == 0))
            trying_irqs |= mask;
...
#endif
    int
    NCR5380_queue_command(Scsi_Cmnd *cmd, void (*done)(Scsi_Cmnd *))
{
    struct NCR5380_hostdata *hostdata = (struct NCR5380_hostdata *)cmd->host->hostdata;
    Scsi_Cmnd *tmp;
    ...
    do {
         ...
         */
             ...
             */
                        ...
                        printk("scsi%d : main() : command for target %d lun %d removed from "
                               "issue_queue\n",
 ...
 */
             ...
             */
                        ...
                        if ((NCR5380_read(MODE_REG) & MR_DMA) &&
                            ((basr & BASR_END_DMA_TRANSFER) || !(basr & BASR_PHASE_MATCH))) {
                            int transfered;

                            if (!hostdata->connected)
                                panic("scsi%d : recieved end of DMA interrupt with no connected "
                                      "cmd\n",
                                      instance->hostno);
...
static int NCR5380_transfer_pio(struct Scsi_Host *instance, unsigned char *phase, int *count,
                                unsigned char **data)
{
    NCR5380_local_declare();
...
#endif
                NCR5380_write(INITIATOR_COMMAND_REG,
                              ICR_BASE | ICR_ASSERT_DATA | ICR_ASSERT_ATN | ICR_ASSERT_ACK);
            }
        } else {
#if (NDEBUG & NDEBUG_PIO)
            NCR5380_print(instance);
...
static int NCR5380_transfer_dma(struct Scsi_Host *instance, unsigned char *phase, int *count,
                                unsigned char **data)
{
    NCR5380_local_declare();
    register int c = *count;
    ...
    printk("scsi%d : initializing DMA channel %d for %s, %d bytes %s %0x\n", instance->host_no,
           instance->dma_channel, (p & SR_IO) ? "reading" : "writing", c,
           (p & SR_IO) ? "to" : "from", (unsigned)d);
#endif
    hostdata->dma_len = (p & SR_IO) ? NCR5380_dma_read_setup(instance, d, c)
         ...
         */
            if (!(hostdata->flags & FLAG_HAS_LAST_BYTE_SENT)) {
                ...
                if (NCR5380_read(STATUS_REG) & SR_REQ) {
                    for (; timeout && !(NCR5380_read(BUS_AND_STATUS_REG) & BASR_ACK); --timeout)
                        ;
                    for (; timeout && (NCR5380_read(STATUS_REG) & SR_REQ); --timeout)
                        ;
                }
#endif
    ...
    while (1) {
         ...
         */
...
#else
                if (!scsi_devices[cmd->index].borken && (transfersize = cmd->transfersize) &&
                    cmd->SCp.this_residual && !(cmd->SCp.this_residual % transfersize)) {
#endif
                    len = transfersize;
                    if (NCR5380_transfer_dma(instance, &phase, &len,
             ...
             */
                    ...
                    NCR5380_transfer_pio(instance, &phase, (int *)&cmd->SCp.this_residual,
                                         (unsigned char **)&cmd->SCp.ptr);
                break;
            case PHASE_MSGIN:
                /*
         ...
         */
             ...
             */
                    ...
                    if (!cmd->next_link) {
                        printk("scsi%d : target %d lun %d linked command complete, no "
                               "next_link\n" instance->host_no,
                               cmd->target, cmd->lun);
                        msgout = ABORT;
                        NCR5380_write(INITIATOR_COMMAND_REG, ICR_BASE | ICR_ASSERT_ATN);
 ...
 */
         ...
         */
            ...
            if (!disconnect && hostdata->time_expires && jiffies > hostdata->time_expires) {
                hostdata->time_expires = jiffies + USLEEP_SLEEP;
#if (NDEBUG & NDEBUG_USLEEP)
...
static void NCR5380_reselect(struct Scsi_Host *instance)
{
     ...
     */
        ...
        for (tmp = (Scsi_Cmnd *)hostdata->disconnected_queue, prev = NULL; tmp;
             prev = tmp, tmp = (Scsi_Cmnd *)tmp->host_scribble)
...
#ifdef SCSI2
            printk(
                "scsi%d : warning : target bitmask %02x lun %d tag %d not in disconnect_queue.\n",
...
#endif
    int
    NCR5380_abort(Scsi_Cmnd *cmd, int code)
{
    NCR5380_local_declare();
 ...
 */
    for (prev = (Scsi_Cmnd **)&(hostdata->issue_queue), tmp = (Scsi_Cmnd *)hostdata->issue_queue;
         tmp; prev = (Scsi_Cmnd **)&(tmp->host_scribble), tmp = (Scsi_Cmnd *)tmp->host_scribble)
        if (cmd == tmp) {
