...
const char *const scsi_device_types[MAX_SCSI_DEVICE_CODE] = {
    ...
    "WORM             ", "CD-ROM           ", "Scanner          ", "Optical Device   ",
    "Medium Changer   ", "Communications   "};
...
static int blacklisted(unsigned char *response_data)
{
    ...
    unsigned char *pnt;
    for (i = 0; 1; i++) {
        if (blacklist[i].vendor == NULL)
            return 0;
        pnt = &response_data[8];
        while (*pnt && *pnt == ' ')
            pnt++;
        if (memcmp(blacklist[i].vendor, pnt, strlen(blacklist[i].vendor)))
            continue;
        pnt = &response_data[16];
        while (*pnt && *pnt == ' ')
            pnt++;
        if (memcmp(blacklist[i].model, pnt, strlen(blacklist[i].model)))
            continue;
        return 1;
    };
};
 ...
 */
                    ...
                    scsi_do_cmd(&SCmd, (void *)scsi_cmd, (void *)scsi_result, 256, scan_scsis_done,
                                SCSI_TIMEOUT + 400, 5);
         ...
         */
                    ...
                    if (!the_result) {
                        ...
                        scsi_devices[NR_SCSI_DEVICES].changed = 0;
                        scsi_devices[NR_SCSI_DEVICES].access_count = 0;
                        scsi_devices[NR_SCSI_DEVICES].busy = 0;
                        /*
                        ...
                        if (type != -1) {
                            ...
                            case TYPE_MOD:
                                printk("Detected scsi disk sd%c at scsi%d, id %d, lun %d\n",
                                       'a' + MAX_SD, shpnt->host_no, dev, lun);
                                if (NR_SD != -1)
                                    ++MAX_SD;
                                break;
                            default:
                            ...
                            scsi_devices[NR_SCSI_DEVICES].scsi_level = scsi_result[2] & 0x07;
            ...
            if(memcmp("INSITE", &scsi_result[8], 6) == 0 &&
               (memcmp("Floptical   F*8I", &scsi_result[16], 16) == 0
                || memcmp("I325VM", &scsi_result[16], 6) == 0)) {
                                ...
                                scsi_do_cmd(&SCmd, (void *)scsi_cmd, (void *)scsi_result, 0x2a,
...
Scsi_Cmnd *request_queueable(struct request *req, int index)
{
    ...
    while (SCpnt) {
        if (SCpnt->target == scsi_devices[index].id && SCpnt->lun == scsi_devices[index].lun)
    ...
    if (scsi_devices[index].host->hostt->can_queue &&
        scsi_devices[index].host->host_busy >= scsi_devices[index].host->hostt->can_queue)
        return NULL;
        ...
        if (!SCpnt || SCpnt->request.dev >= 0) /* Might have changed */
        {
            ...
            if (!SCwait) {
                printk("Attempt to allocate device index %d, target %d, lun %d\n", index,
                       scsi_devices[index].id, scsi_devices[index].lun);
...
*/
    ...
    printk("internal_cmnd (host = %d, target = %d, command = %08x, buffer =  %08x, \n"
           "bufflen = %d, done = %08x)\n",
           SCpnt->host->host_no, SCpnt->target, SCpnt->cmnd, SCpnt->buffer, SCpnt->bufflen,
           SCpnt->done);
#endif
...
static void scsi_request_sense(Scsi_Cmnd *SCpnt)
{
    ...
    memcpy((void *)SCpnt->cmnd, (void *)generic_sense, sizeof(generic_sense));
...
void scsi_do_cmd(Scsi_Cmnd *SCpnt, const void *cmnd, void *buffer, unsigned bufflen,
                 void (*done)(Scsi_Cmnd *), int timeout, int retries)
{
...
#ifdef DEBUG
    {
        ...
        printk("scsi_do_cmd (host = %d, target = %d, buffer =%08x, "
               ...
               "command : ",
               host->host_no, target, buffer, bufflen, done, timeout, retries);
        for (i = 0; i < 10; ++i)
...
*/
    ...
    while (1 == 1) {
        cli();
        if (host->hostt->can_queue && host->host_busy >= host->hostt->can_queue) {
            sti();
            SCSI_SLEEP(&host->host_wait, (host->host_busy >= host->hostt->can_queue));
        } else {
            host->host_busy++;
            sti();
            break;
...
#define PENDING 4
...
#ifdef DEBUG
                        printk("In scsi_done, GOOD status, COMMAND COMPLETE, parsing sense "
                               "information.\n");
#endif
    ...
    case MAYREDO:
        ...
        if ((++SCpnt->retries) < SCpnt->allowed) {
            if ((SCpnt->retries >= (SCpnt->allowed >> 1)) && !(SCpnt->flags & WAS_RESET)) {
    ...
    case REDO:
        if (SCpnt->flags & WAS_SENSE)
            ...
            memcpy((void *)SCpnt->cmnd, (void *)SCpnt->data_cmnd, sizeof(SCpnt->data_cmnd));
    ...
    while (1) {
        ...
        if (SCpnt->internal_timeout & IN_RESET) {
            ...
            if (host->host_busy) {
                ...
                while (SCpnt1) {
                    if ((SCpnt1->request.dev > 0) && !(SCpnt1->flags & IS_RESETTING) &&
...
int scsi_free(void *obj, unsigned int len)
{
    ...
    if ((mask << sector) > 0xffff)
        panic("Bad memory alignment");
...
unsigned long scsi_dev_init(unsigned long memory_start, unsigned long memory_end)
{
    ...
    for (i = 0; i < NR_SCSI_DEVICES; ++i) {
        ...
        if (scsi_devices[i].type != TYPE_TAPE)
            dma_sectors += ((host->sg_tablesize * sizeof(struct scatterlist) + 511) >> 9) *
                           host->hostt->cmd_per_lun;

        if (host->unchecked_isa_dma && memory_end > ISA_DMA_THRESHOLD &&
            scsi_devices[i].type != TYPE_TAPE) {
            dma_sectors += (PAGE_SIZE >> 9) * host->sg_tablesize * host->hostt->cmd_per_lun;
            need_isa_buffer++;
        };
    };
    ...
    if (memory_start & 1)
        memory_start++; /* Some host adapters require
...
static void print_inquiry(unsigned char *data)
{
    ...
    printk("  Type:   %s ", i < MAX_SCSI_DEVICE_CODE ? scsi_device_types[i] : "Unknown          ");
