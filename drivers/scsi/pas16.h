...
#define MV_PAS16                                                                                   \
    {                                                                                              \
        "Pro Audio Spectrum-16 SCSI", pas16_detect, pas16_info, NULL, pas16_queue_command,         \
            pas16_abort, pas16_reset, NULL, pas16_biosparam, /* can queue */ CAN_QUEUE,            \
            /* id */ 7, SG_ALL, /* cmd per lun */ CMD_PER_LUN, 0, 0                                \
    }
...
#define NCR5380_read(reg)                                                                          \
    (((unsigned char)printk("scsi%d : read register %d at io_port %04x\n", instance->hostno,       \
                            (reg), PAS16_io_port(reg))),                                           \
     inb(PAS16_io_port(reg)))
