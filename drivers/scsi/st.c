...
static int st_int_ioctl(struct inode *inode, struct file *file, unsigned int cmd_in,
                        unsigned long arg);
...
static int st_chk_result(Scsi_Cmnd *SCpnt)
{
    ...
    /*  if ((sense[0] & 0x70) == 0x70 &&
       ((sense[2] & 0x80) ))
    return 0; */
    if ((sense[0] & 0x70) == 0x70 && sense[2] == RECOVERED_ERROR
#ifdef ST_RECOVERED_WRITE_FATAL
        && SCpnt->cmnd[0] != WRITE_6 && SCpnt->cmnd[0] != WRITE_FILEMARKS
#endif
    ) {
...
static void st_sleep_done(Scsi_Cmnd *SCpnt)
{
    ...
    if ((st_nbr = SCpnt->request.dev) < NR_ST && st_nbr >= 0) {
        ...
        if ((STp->buffer)->writing && (SCpnt->sense_buffer[0] & 0x70) == 0x70 &&
            (SCpnt->sense_buffer[2] & 0x40)) {
            /* EOM at write-behind, has all been written? */
            if ((SCpnt->sense_buffer[0] & 0x80) != 0)
                remainder = (SCpnt->sense_buffer[3] << 24) | (SCpnt->sense_buffer[4] << 16) |
                            (SCpnt->sense_buffer[5] << 8) | SCpnt->sense_buffer[6];
            else
                remainder = 0;
            if ((SCpnt->sense_buffer[2] & 0x0f) == VOLUME_OVERFLOW || remainder > 0)
...
static void write_behind_check(int dev)
{
    ...
    if (STbuffer->writing < STbuffer->buffer_bytes)
        memcpy(STbuffer->b_data, STbuffer->b_data + STbuffer->writing,
...
static int flush_write_buffer(int dev)
{
    ...
    if (STp->dirty == 1) {
        ...
        transfer = ((offset + STp->block_size - 1) / STp->block_size) * STp->block_size;
#ifdef DEBUG
        ...
        scsi_do_cmd(SCpnt, (void *)cmd, (STp->buffer)->b_data, transfer, st_sleep_done, ST_TIMEOUT,
                    MAX_RETRIES);

        if (SCpnt->request.dev == dev)
            sleep_on(&(STp->waiting));

        if ((STp->buffer)->last_result_fatal != 0) {
            printk("st%d: Error on flush.\n", dev);
            if ((SCpnt->sense_buffer[0] & 0x70) == 0x70 && (SCpnt->sense_buffer[2] & 0x40) &&
...
static int scsi_tape_open(struct inode *inode, struct file *filp)
{
    ...
    SCpnt->request.dev = dev;
    scsi_do_cmd(SCpnt, (void *)cmd, (void *)(STp->buffer)->b_data, ST_BLOCK_SIZE, st_sleep_done,
                ST_LONG_TIMEOUT, MAX_READY_RETRIES);
    ...
    if ((STp->buffer)->last_result_fatal != 0) {
        if ((SCpnt->sense_buffer[0] & 0x70) == 0x70 && (SCpnt->sense_buffer[2] & 0x0f) == NO_TAPE)
            printk("st%d: No tape.\n", dev);
        else
            printk("st%d: Error %x.\n", dev, SCpnt->result);
        (STp->buffer)->in_use = 0;
    ...
    SCpnt->request.dev = dev;
    scsi_do_cmd(SCpnt, (void *)cmd, (void *)(STp->buffer)->b_data, ST_BLOCK_SIZE, st_sleep_done,
                ST_TIMEOUT, MAX_READY_RETRIES);

    if (SCpnt->request.dev == dev)
        sleep_on(&(STp->waiting));

    if (!SCpnt->result && !SCpnt->sense_buffer[0]) {
        STp->max_block = ((STp->buffer)->b_data[1] << 16) | ((STp->buffer)->b_data[2] << 8) |
                         (STp->buffer)->b_data[3];
        STp->min_block = ((STp->buffer)->b_data[4] << 8) | (STp->buffer)->b_data[5];
#ifdef DEBUG
        printk("st%d: Block limits %d - %d bytes.\n", dev, STp->min_block, STp->max_block);
#endif
    } else {
...
#endif
        (STp->buffer)->b_data[2] = (STp->buffer)->b_data[3] = 0;
    }
    SCpnt->request.dev = -1; /* Mark as not busy */

#ifdef DEBUG
    printk("st%d: Mode sense. Length %d, medium %x, WBS %x, BLL %d\n", dev,
           (STp->buffer)->b_data[0], (STp->buffer)->b_data[1], (STp->buffer)->b_data[2],
           (STp->buffer)->b_data[3]);
#endif
    ...
    if ((STp->buffer)->b_data[3] >= 8) {
        ...
        STp->block_size = (STp->buffer)->b_data[9] * 65536 + (STp->buffer)->b_data[10] * 256 +
                          (STp->buffer)->b_data[11];
#ifdef DEBUG
        printk("st%d: Density %x, tape length: %x, blocksize: %d, drv buffer: %d\n", dev,
               STp->density,
               (STp->buffer)->b_data[5] * 65536 + (STp->buffer)->b_data[6] * 256 +
                   (STp->buffer)->b_data[7],
    ...
    if (STp->block_size > 0) {
        ...
        (STp->buffer)->buffer_size = (STp->buffer)->buffer_blocks * STp->block_size;
    } else {
...
static void scsi_tape_close(struct inode *inode, struct file *filp)
{
    ...
    if (STp->rw == ST_WRITING) {
        ...
        if (result == 0 || result == (-ENOSPC)) {
            ...
            cmd[4] = 1;
            SCpnt->request.dev = dev;
            scsi_do_cmd(SCpnt, (void *)cmd, (void *)(STp->buffer)->b_data, ST_BLOCK_SIZE,
...
/* Write command */
static int st_write(struct inode *inode, struct file *filp, char *buf, int count)
{
    int dev;
...
#endif
    {
        ...
        memcpy_fromfs((STp->buffer)->b_data + (STp->buffer)->buffer_bytes, b_point, do_count);

        if (STp->block_size == 0)
            blks = do_count;
        else
            blks = ((STp->buffer)->buffer_bytes + do_count) / STp->block_size;
        cmd[2] = blks >> 16;
        cmd[3] = blks >> 8;
        cmd[4] = blks;
        SCpnt->sense_buffer[0] = 0;
        SCpnt->request.dev = dev;
        scsi_do_cmd(SCpnt, (void *)cmd, (STp->buffer)->b_data, (STp->buffer)->buffer_size,
                    st_sleep_done, ST_TIMEOUT, MAX_RETRIES);
        ...
        if ((STp->buffer)->last_result_fatal != 0) {
            ...
            if ((SCpnt->sense_buffer[0] & 0x70) == 0x70 && (SCpnt->sense_buffer[2] & 0x40)) {
                if (STp->block_size != 0 && (SCpnt->sense_buffer[0] & 0x80) != 0)
                    transfer = (SCpnt->sense_buffer[3] << 24) | (SCpnt->sense_buffer[4] << 16) |
                               (SCpnt->sense_buffer[5] << 8) | SCpnt->sense_buffer[6];
                else if (STp->block_size == 0 && (SCpnt->sense_buffer[2] & 0x0f) == VOLUME_OVERFLOW)
                    transfer = do_count;
                else
                    transfer = 0;
                if (STp->block_size != 0)
    ...
    if (count != 0) {
        STp->dirty = 1;
        memcpy_fromfs((STp->buffer)->b_data + (STp->buffer)->buffer_bytes, b_point, count);
        filp->f_pos += count;
...
#if ST_WRITE_THRESHOLD_BLOCKS < ST_BUFFER_BLOCKS
    if ((STp->buffer)->buffer_bytes >= ST_WRITE_THRESHOLD || STp->block_size == 0) {
        /* Schedule an asynchronous write */
        if (STp->block_size == 0)
            (STp->buffer)->writing = (STp->buffer)->buffer_bytes;
        else
            (STp->buffer)->writing =
                ((STp->buffer)->buffer_bytes / STp->block_size) * STp->block_size;
        STp->dirty = 0;
        ...
        SCpnt->sense_buffer[0] = 0;
        SCpnt->request.dev = dev;
        scsi_do_cmd(SCpnt, (void *)cmd, (STp->buffer)->b_data, (STp->buffer)->writing,
                    st_sleep_done, ST_TIMEOUT, MAX_RETRIES);
    } else
#endif
        SCpnt->request.dev = -1; /* Mark as not busy */
...
/* Read command */
static int st_read(struct inode *inode, struct file *filp, char *buf, int count)
{
    int dev;
    int total;
...
#endif
    if (((STp->buffer)->buffer_bytes == 0) && STp->eof == ST_EOM_OK) /* EOM or Blank Check */
        return (-EIO);
    ...
    for (total = 0; total < count;) {
        ...
        if ((STp->buffer)->buffer_bytes == 0 && STp->eof == ST_NOEOF) {
            ...
            if ((STp->buffer)->last_result_fatal) {
                ...
                printk("st%d: Sense: %2x %2x %2x %2x %2x %2x %2x %2x\n", dev,
                       SCpnt->sense_buffer[0], SCpnt->sense_buffer[1], SCpnt->sense_buffer[2],
                       SCpnt->sense_buffer[3], SCpnt->sense_buffer[4], SCpnt->sense_buffer[5],
                ...
                if ((SCpnt->sense_buffer[0] & 0x70) == 0x70) { /* extended sense */
                    ...
                    if ((SCpnt->sense_buffer[2] & 0xe0) != 0) { /* EOF, EOM, or ILI */
                        ...
                        if ((SCpnt->sense_buffer[0] & 0x80) != 0)
                            ...
                            transfer = 0;
                        if (STp->block_size == 0 && (SCpnt->sense_buffer[2] & 0x0f) == MEDIUM_ERROR)
                            transfer = bytes;
                        ...
                        if (SCpnt->sense_buffer[2] & 0x20) {
                            ...
                            if (STp->block_size == 0)
                                ...
                                (STp->buffer)->buffer_bytes =
                                    ((STp->buffer)->buffer_blocks - transfer) * STp->block_size;
#ifdef DEBUG
                            ...
                            if (STp->block_size == 0)
                                ...
                                (STp->buffer)->buffer_bytes =
                                    ((STp->buffer)->buffer_blocks - transfer) * STp->block_size;
#ifdef DEBUG
                            printk("st%d: EOF detected (%d bytes read, transferred %d bytes).\n",
        ...
        if ((STp->buffer)->buffer_bytes > 0) {
            ...
            if (STp->eof != ST_NOEOF)
                printk("st%d: EOF up. Left %d, needed %d.\n", dev, (STp->buffer)->buffer_bytes,
                       count - total);
#endif
            transfer = (STp->buffer)->buffer_bytes < count - total ? (STp->buffer)->buffer_bytes
                                                                   : count - total;
            memcpy_tofs(buf, (STp->buffer)->b_data + (STp->buffer)->read_pointer, transfer);
            filp->f_pos += transfer;
            buf += transfer;
            total += transfer;
                        ...
                        unsigned long arg)
{
    ...
    case MTSETDRVBUFFER: /* Set drive buffering */
        ...
        if (cmd_in == MTSETBLK && arg != 0 &&
            (arg < STp->min_block || arg > STp->max_block || arg > ST_BUFFER_SIZE)) {
        ...
        if (cmd_in == MTSETBLK)
            printk("st%d: Setting block size to %d bytes.\n", dev,
                   (STp->buffer)->b_data[9] * 65536 + (STp->buffer)->b_data[10] * 256 +
                       (STp->buffer)->b_data[11]);
        else if (cmd_in == MTSETDENSITY)
            printk("st%d: Setting density code to %x.\n", dev, (STp->buffer)->b_data[4]);
        else
...
static int st_ioctl(struct inode *inode, struct file *file, unsigned int cmd_in, unsigned long arg)
{
    ...
    if (cmd == (MTIOCTOP & IOCCMD_MASK)) {
        ...
        i = flush_buffer(inode, file,
                         mtc.mt_op == MTSEEK || mtc.mt_op == MTREW || mtc.mt_op == MTOFFL ||
        ...
        memcpy_tofs((char *)arg, (char *)(STp->buffer)->mt_status, sizeof(struct mtget));
        return 0;
        ...
        SCpnt->request.dev = dev;
        SCpnt->sense_buffer[0] = 0;
        scsi_do_cmd(SCpnt, (void *)scmd, (void *)(STp->buffer)->b_data, ST_BLOCK_SIZE,
                    st_sleep_done, ST_TIMEOUT, MAX_READY_RETRIES);
        ...
        if ((STp->buffer)->last_result_fatal != 0) {
            ...
            if ((STp->device)->scsi_level < SCSI_2)
                mt_pos.mt_blkno = ((STp->buffer)->b_data[0] << 16) +
                                  ((STp->buffer)->b_data[1] << 8) + (STp->buffer)->b_data[2];
            else
                mt_pos.mt_blkno = ((STp->buffer)->b_data[4] << 24) +
                                  ((STp->buffer)->b_data[5] << 16) +
                                  ((STp->buffer)->b_data[6] << 8) + (STp->buffer)->b_data[7];
        }
