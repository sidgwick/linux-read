...
/* This asm is fragile: it doesn't work without the casts and it may
   ...
   to gcc.  --jfc  */
static inline unsigned char xchgb(unsigned char reg, volatile unsigned char *mem)
{
    asm("xchgb %0,%1"
...
static void log_ultrastor_abort(register struct ultrastor_config *config, int command)
{
...
static int ultrastor_14f_detect(int hostnum)
{
    ...
    for (i = 0; i < ARRAY_SIZE(ultrastor_ports_14f); i++) {
        if (check_region(ultrastor_ports_14f[i], 0x0c))
            continue;
...
printk("US14F: detect: detect succeeded\n"
       ...
       "  H/A SCSI ID: %u\n"
       "  Subversion: %u\n",
       config.port_address, config.bios_segment, config.interrupt, config.dma_channel,
       config.ha_scsi_id, config.subversion);
#endif
...
static int ultrastor_24f_detect(int hostnum)
{
    ...
    for (i = 1; i < 15; i++) {
        ...
        if (inb(addr) != US24F_PRODUCT_ID_0 && inb(addr + 1) != US24F_PRODUCT_ID_1 &&
        ...
        printk("US24F: detect: detect succeeded\n"
               ...
               "  Interrupt: %u\n"
               "  H/A SCSI ID: %u\n",
               config.port_address, config.bios_segment, config.interrupt, config.ha_scsi_id);
#endif
        config.host_number = hostnum;
        scsi_hosts[hostnum].this_id = config.ha_scsi_id;
...
const char *ultrastor_info(void)
{
    ...
    if (config.slot)
        ...
        sprintf(buf, "UltraStor 34F SCSI @ Port %03X BIOS %05X IRQ%u\n", config.port_address,
                (int)config.bios_segment, config.interrupt);
    else
        sprintf(buf, "UltraStor 14F SCSI @ Port %03X BIOS %05X IRQ%u DMA%u\n", config.port_address,
                (int)config.bios_segment, config.interrupt, config.dma_channel);
    return buf;
}
...
retry:
    ...
    while ((inb(LCL_DOORBELL_INTR(config.doorbell_address)) & (config.slot ? 2 : 1)) &&
...
int ultrastor_abort(Scsi_Cmnd *SCpnt, int code)
{
    ...
    if (config.slot)
        return 0; /* Do not attempt an abort for the 24f */
...
#else
        for (i = 0; i < ULTRASTOR_MAX_CMDS; i++) {
            if (!(config.mscp_free & (1 << i)) && config.mscp[i].done && config.mscp[i].SCint) {
