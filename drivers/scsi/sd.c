...
#endif
                if (SCpnt->request.cmd == READ)
                    memcpy(SCpnt->request.buffer, SCpnt->buffer, SCpnt->bufflen);
...
*/
                panic("sd.c: linked page request (%lx %x)", SCpnt->request.sector, this_count);
            }
        }
...
*/
            if rscsi_disks
                [DEVICE_NR(SCpnt->request.dev)].remap
                {
                    result = 0;
                }
            else
...
static void do_sd_request(void)
{
   ...
   otherwise the kernel might try and slip in a request inbetween somewhere. */
        ...
        if (!SCpnt && NR_SD > 1) {
            ...
            while (req) {
                SCpnt =
                    request_queueable(req, rscsi_disks[DEVICE_NR(MINOR(req->dev))].device->index);
                if (SCpnt)
                    break;
                req1 = req;
                req = req->next;
 ...
 */
    ...
    if (!SCpnt->request.bh || (SCpnt->request.nr_sectors == SCpnt->request.current_nr_sectors)) {
    ...
    } else if (SCpnt->host->sg_tablesize == 0 || (need_isa_buffer && dma_free_sectors < 10)) {
   ...
   operation */
                    ...
                    if (SCpnt->request.cmd == WRITE)
                        memcpy(sgpnt[count].address, sgpnt[count].alt_address, sgpnt[count].length);
                };
            }; /* for loop */
    ...
    printk("sd%d : %s %d/%d 512 byte blocks.\n", MINOR(SCpnt->request.dev),
           (SCpnt->request.cmd == WRITE) ? "writing" : "reading", this_count,
 ...
 */
    ...
    scsi_do_cmd(SCpnt, (void *)cmd, buff, this_count * rscsi_disks[dev].sector_size, rw_intr,
...
static int sd_init_onedisk(int i)
{
    ...
    if (current == task[0]) {
        do {
            ...
            scsi_do_cmd(SCpnt, (void *)cmd, (void *)buffer, 512, sd_init_done, SD_TIMEOUT,
                        MAX_RETRIES);
    ...
    do {
        ...
        scsi_do_cmd(SCpnt, (void *)cmd, (void *)buffer, 8, sd_init_done, SD_TIMEOUT, MAX_RETRIES);
   ...
   */
    ...
    if (the_result) {
        printk("sd%d : READ CAPACITY failed.\n"
               ...
               i, i, status_byte(the_result), msg_byte(the_result), host_byte(the_result),
     ...
     will not report this properly. */
        if (rscsi_disks[i].device->removable && SCpnt->sense_buffer[2] == NOT_READY)
        ...
        rscsi_disks[i].sector_size =
            (buffer[4] << 24) | (buffer[5] << 16) | (buffer[6] << 8) | buffer[7];

        if (rscsi_disks[i].sector_size != 512 && rscsi_disks[i].sector_size != 1024 &&
