...
static const char *group_0_commands[] = {
    ...
    "Mode Sense",
    "Start/Stop Unit",
    /* 1c-1d */ "Receive Diagnostic",
    "Send Diagnostic",
    /* 1e-1f */ "Prevent/Allow Medium Removal",
    unknown,
};
...
static const char **commands[] = {group_0_commands,
                                  group_1_commands,
                                  group_2_commands,
                                  (const char **)RESERVED_GROUP,
                                  (const char **)RESERVED_GROUP,
                                  (const char **)NOTEXT_GROUP,
...
void print_sense(char *devclass, Scsi_Cmnd *SCpnt)
{
    ...
    if (sense_class == 7) {
        ...
        if (sense_buffer[7] + 7 < 13 || (sense_buffer[12] == 0 && sense_buffer[13] == 0))
            goto done;

#if (CONSTANTS & CONST_XSENSE)
        for (i = 0; additional[i].text; i++)
            if (additional[i].code1 == sense_buffer[12] && additional[i].code2 == sense_buffer[13])
...
#define NO_TWO_BYTE_MSGS (sizeof(two_byte_msgs) / sizeof(const char *))
                                                        ...
                                                        static const char *
                                                        extended_msgs[] = {
                                                            /* 0x00 */ "Modify Data Pointer",
                                                            "Synchronous Data Transfer Request",
                                                            /* 0x02 */ "SCSI-I Extended Identify",
                                                            "Wide Data Transfer Reqeust"};
