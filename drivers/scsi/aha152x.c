...
static void *addresses[] = {
    ...
    (void *)0xc8000, (void *)0xcc000, (void *)0xd0000, (void *)0xd4000,
    (void *)0xd8000, (void *)0xe0000, (void *)0xf0000,
};
#define ADDRESS_COUNT (sizeof(addresses) / sizeof(unsigned))

/* possible i/o adresses for the AIC-6260 */
static unsigned short ports[] = {0x340, /* default first */
                                 0x140};
...
All Rights Reserved\r\n \r\n \r\n",
     0x102e, 101}, /* Adaptec 152x */
    {"Adaptec ASW-B626 BIOS\r\n\0\
Version 1.0      \r\n\0\
Copyright 1990 Adaptec, Inc.\r\n\
All Rights Reserved\r\n\0 \r\n \r\n",
     0x1029, 102},                            /* on-board controller */
...
static inline Scsi_Cmnd *remove_SC(Scsi_Cmnd **SC, int target, int lun)
{
    ...
    for (ptr = *SC, prev = NULL; ptr && ((ptr->target != target) || (ptr->lun != lun));
        ...
        for (i = 0; i < ADDRESS_COUNT && !ok; i++)
            for (j = 0; (j < SIGNATURE_COUNT) && !ok; j++)
                ok = !memcmp((void *)addresses[i] + signatures[j].sig_offset,
                             (void *)signatures[j].signature, (int)signatures[j].sig_length);
...
#if defined(DEBUG_QUEUE)
    printk("SCpnt (target = %d lun = %d cmnd = 0x%02x pieces = %d size = %u), ", SCpnt->target,
           SCpnt->lun, *(unsigned char *)SCpnt->cmnd, SCpnt->use_sg, SCpnt->request_bufflen);
    disp_ports();
#endif
...
int aha152x_abort(Scsi_Cmnd *SCpnt, int code)
{
    ...
    for (ptr = issue_SC, prev = NULL; ptr && ptr != SCpnt;
         prev = ptr, ptr = (Scsi_Cmnd *)ptr->host_scribble)
        ;
    ...
    for (ptr = disconnected_SC, prev = NULL; ptr && ptr != SCpnt;
         prev = ptr, ptr = (Scsi_Cmnd *)ptr->host_scribble)
        ;
...
void aha152x_intr(int irqno)
{
  ...
  */
    if (TESTHI(SSTAT0, SELDI) && disconnected_SC &&
    ...
    current_SC->SCp.phase = (current_SC->SCp.phase & ~((P_MASK | 1) << 16)) | (phase << 16);
    ...
    case P_MSGO: /* MESSAGE OUT */
    {
        ...
        if (current_SC->SCp.phase & aborted) {
            ...
            if (!(current_SC->SCp.phase & sent_ident)) {
                ...
                printk("IDENTIFY (reconnect=%s;lun=%d), ", can_disconnect ? "enabled" : "disabled",
                       current_SC->lun);
#endif
            } else {
    ...
    case P_CMD: /* COMMAND phase */
        ...
        if (!(current_SC->SCp.sent_command)) {
...
#if defined(DEBUG_CMD)
            printk("DFIFOEMP, outsw (%d words), ", COMMAND_SIZE(current_SC->cmnd[0]) >> 1);
            disp_ports();
#endif

            outsw(DATAPORT, &current_SC->cmnd, COMMAND_SIZE(current_SC->cmnd[0]) >> 1);
            ...
            printk("sent %d/%d command bytes, ", GETSTCNT(), COMMAND_SIZE(current_SC->cmnd[0]));
#endif
    ...
    case P_DATAI: /* DATA IN phase */
    {
        ...
        if (current_SC->SCp.buffers_residual || current_SC->SCp.this_residual)
            printk("left buffers (buffers=%d, bytes=%d), ", current_SC->SCp.buffers_residual,
                   current_SC->SCp.this_residual);
#endif
        /* transfer can be considered ended, when SCSIEN reads back zero */
        CLRBITS(SXFRCTL0, SCSIEN | DMAEN);
    ...
    case P_DATAO: /* DATA OUT phase */
    {
        ...
        printk("got data to send (bytes=%d, buffers=%d), ", current_SC->SCp.this_residual,
               current_SC->SCp.buffers_residual);
#endif
        ...
        while (TESTLO(SSTAT1, PHASEMIS) &&
            ...
            data_count = current_SC->SCp.this_residual > 128 ? 128 : current_SC->SCp.this_residual;
...
#if defined(DEBUG_DATAO)
            printk("fifo (%d bytes), transfered (%d bytes), ", GETPORT(FIFOSTAT), GETSTCNT());
#endif

            /* if this buffer is empty and there are more buffers left */
            if (TESTLO(SSTAT1, PHASEMIS) && !current_SC->SCp.this_residual &&
                current_SC->SCp.buffers_residual) {
                /* advance to next buffer */
                current_SC->SCp.buffers_residual--;
                current_SC->SCp.buffer++;
...
#if defined(DEBUG_DATAO)
            printk("left data (bytes=%d, buffers=%d), fifos (bytes=%d), transfer incomplete, "
                   "resetting fifo, ",
                   current_SC->SCp.this_residual, current_SC->SCp.buffers_residual, data_count);
#endif
            SETPORT(DMACNTRL0, WRITE_READ | RSTFIFO);
            CLRBITS(SXFRCTL0, SCSIEN | DMAEN);
            CLRBITS(DMACNTRL0, ENDMA);
...
#if defined(DEBUG_DATAO)
            printk("left data (bytes=%d, buffers=%d) ", current_SC->SCp.this_residual,
                   current_SC->SCp.buffers_residual);
#endif
            CLRBITS(SXFRCTL0, SCSIEN | DMAEN);
    ...
    if (done) {
        ...
        aha152x_done((current_SC->SCp.Status & 0xff) | ((current_SC->SCp.Message & 0xff) << 8) |
                     (DID_OK << 16));
...
static void show_command(Scsi_Cmnd *ptr)
{
    ...
    printk("0x%08x: target=%d; lun=%d; cmnd=( ", (unsigned long)ptr, ptr->target, ptr->lun);

    for (i = 0; i < COMMAND_SIZE(ptr->cmnd[0]); i++)
        printk("%02x ", ptr->cmnd[i]);

    printk("); residual=%d; buffers=%d; phase |", ptr->SCp.this_residual,
