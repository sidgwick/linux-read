...
static int sg_ioctl(struct inode *inode, struct file *file, unsigned int cmd_in, unsigned long arg)
{
...
static int sg_write(struct inode *inode, struct file *filp, char *buf, int count)
{
    ...
    buf += sizeof(struct sg_header);
    bsize = (device->header.pack_len > device->header.reply_len) ? device->header.pack_len
                                                                 : device->header.reply_len;
    ...
    printk("do cmd\n");
#endif
    scsi_do_cmd(SCpnt, (void *)cmnd, (void *)device->buff, amt, sg_command_done, device->timeout,
                SG_DEFAULT_RETRIES);
#ifdef DEBUG
    printk("done cmd\n");
#endif
