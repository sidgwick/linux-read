...
#define VERIFY1_DEBUG(RW)                                                                          \
    ...
    start = 0;                                                                                     \
    if ((SCpnt->request.dev & 0xf) != 0)                                                           \
        start = starts[(SCpnt->request.dev & 0xf) - 1];                                            \
    if (bh) {                                                                                      \
        if (bh->b_size != 1024)                                                                    \
            panic("Wrong bh size");                                                                \
...
#define VERIFY_DEBUG(RW)                                                                           \
    if (bufflen != 1024 && (!SCpnt->use_sg)) {                                                     \
        printk("%x %d\n ", bufflen, SCpnt->use_sg);                                                \
        panic("Bad bufflen");                                                                      \
    };                                                                                             \
    start = 0;                                                                                     \
    if ((SCpnt->request.dev & 0xf) > npart)                                                        \
        panic("Bad partition");                                                                    \
    if ((SCpnt->request.dev & 0xf) != 0)                                                           \
        start = starts[(SCpnt->request.dev & 0xf) - 1];                                            \
    if (SCpnt->request.cmd != RW)                                                                  \
        panic("Wrong  operation");                                                                 \
    if (SCpnt->request.sector + start != block)                                                    \
        panic("Wrong block.");                                                                     \
    if (SCpnt->request.current_nr_sectors != 2 && (!SCpnt->use_sg))                                \
        panic("Wrong # blocks");                                                                   \
    if (SCpnt->request.bh) {                                                                       \
        if (SCpnt->request.bh->b_size != 1024)                                                     \
            panic("Wrong bh size");                                                                \
        if ((SCpnt->request.bh->b_blocknr << 1) + start != block) {                                \
            printk("Wrong bh block# %d %d ", SCpnt->request.bh->b_blocknr, block);                 \
            panic("Wrong bh block#");                                                              \
        };                                                                                         \
        if (SCpnt->request.bh->b_dev != SCpnt->request.dev)                                        \
            panic("Bad bh target");                                                                \
    };

static volatile void (*do_done[SCSI_DEBUG_MAILBOXES])(Scsi_Cmnd *) = {
    NULL,
};
static int scsi_debug_host = 0;
extern void scsi_debug_interrupt();

volatile Scsi_Cmnd *SCint[SCSI_DEBUG_MAILBOXES] = {
    NULL,
};
static volatile unsigned int timeout[SCSI_DEBUG_MAILBOXES] = {
    0,
};
...
int scsi_debug_queuecommand(Scsi_Cmnd *SCpnt, void (*done)(Scsi_Cmnd *))
{
    ...
    case REQUEST_SENSE:
        ...
        {
            int i;
            printk("scsi_debug: Requesting sense buffer (%x %x %x %d):", SCpnt, buff, done,
                   bufflen);
            for (i = 0; i < 12; i++)
    ...
    case ALLOW_MEDIUM_REMOVAL:
        if (cmd[4])
            printk("Medium removal inhibited...");
        else
    ...
    case READ_CAPACITY:
        ...
        if (NR_REAL < 0)
            NR_REAL = (SCpnt->request.dev >> 4) & 0x0f;
        memset(buff, 0, bufflen);
        buff[0] = (CAPACITY >> 24);
...
repeat:
    ...
    if (i == SCSI_DEBUG_MAILBOXES) {
        ...
        for (i = 0; i < SCSI_DEBUG_MAILBOXES; i++) {
            ...
            if (timeout[i] > jiffies) {
                if (pending > timeout[i])
                    pending = timeout[i];
                continue;
            };
        };
        if (pending && pending != INT_MAX) {
            timer_table[SCSI_DEBUG_TIMER].expires = (pending <= jiffies ? jiffies + 1 : pending);
            timer_active |= 1 << SCSI_DEBUG_TIMER;
        };
        sti();
        return;
