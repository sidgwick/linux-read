...
static void rw_intr(Scsi_Cmnd *SCpnt)
{
    ...
    if (!result) { /* No error */
        if (SCpnt->use_sg == 0) {
            if (SCpnt->buffer != SCpnt->request.buffer) {
                ...
                memcpy((char *)SCpnt->request.buffer, (char *)SCpnt->buffer + offset,
                       this_count << 9);
               ...
               it. */
                if ((offset == 0) && this_count == 2 && SCpnt->request.nr_sectors > this_count &&
                    SCpnt->request.bh && SCpnt->request.bh->b_reqnext &&
                    SCpnt->request.bh->b_reqnext->b_size == 1024) {
                    memcpy((char *)SCpnt->request.bh->b_reqnext->b_data,
                           (char *)SCpnt->buffer + 1024, 1024);
                    this_count += 2;
                };
...
#ifdef DEBUG
        printk("(%x %x %x) ", SCpnt->request.bh, SCpnt->request.nr_sectors, this_count);
#endif
        if (SCpnt->request.nr_sectors > this_count) {
            SCpnt->request.errors = 0;
            if (!SCpnt->request.bh)
                panic("sr.c: linked page request (%lx %x)", SCpnt->request.sector, this_count);
        }
    ...
    if (result) {
        printk("SCSI CD error : host %d id %d lun %d return code = %03x\n",
               ...
               scsi_CDs[DEVICE_NR(SCpnt->request.dev)].device->lun, result);
...
{
    if (MINOR(inode->i_rdev) >= NR_SR || !scsi_CDs[MINOR(inode->i_rdev)].device)
...
static void do_sr_request(void)
{
   ...
   otherwise the kernel might try and slip in a request inbetween somewhere. */
        ...
        if (!SCpnt && NR_SR > 1) {
            ...
            req = CURRENT;
            while (req) {
                SCpnt = request_queueable(req, scsi_CDs[DEVICE_NR(MINOR(req->dev))].device->index);
                if (SCpnt)
                    break;
                req1 = req;
                req = req->next;
...
are any multiple of 512 bytes long.  */
    ...
    if (SCpnt->host->sg_tablesize > 0 && (!need_isa_buffer || dma_free_sectors >= 10)) {
       ...
       a padding record at the start */
        ...
        if (!sgpnt) {
            ...
            for (bh = SCpnt->request.bh; count < SCpnt->use_sg; count++, bh = bh->b_reqnext) {
                ...
                if (((int)sgpnt[count].address) + sgpnt[count].length > ISA_DMA_THRESHOLD &
   ...
   operation */
...
#ifdef DEBUG
            printk("SG: %d %d %d %d %d *** ", SCpnt->use_sg, SCpnt->request.sector, this_count,
                   SCpnt->request.current_nr_sectors, SCpnt->request.nr_sectors);
            for (count = 0; count < SCpnt->use_sg; count++)
                printk("SGlist: %d %x %x %x\n", count, sgpnt[count].address,
                       sgpnt[count].alt_address, sgpnt[count].length);
#endif
    ...
    if (SCpnt->use_sg == 0) {
        ...
        start = block % 4;
        if (start) {
            this_count = ((this_count > 4 - start) ? (4 - start) : (this_count));
    ...
    scsi_do_cmd(SCpnt, (void *)cmd, buffer, realcount * scsi_CDs[dev].sector_size, rw_intr,
...
static void get_sectorsize(int i)
{
    ...
    do {
        ...
        scsi_do_cmd(SCpnt, (void *)cmd, (void *)buffer, 512, sr_init_done, SR_TIMEOUT, MAX_RETRIES);
    ...
    if (the_result) {
        ...
        scsi_CDs[i].capacity = (buffer[0] << 24) | (buffer[1] << 16) | (buffer[2] << 8) | buffer[3];
        scsi_CDs[i].sector_size =
            (buffer[4] << 24) | (buffer[5] << 16) | (buffer[6] << 8) | buffer[7];
        if (scsi_CDs[i].sector_size == 0)
            scsi_CDs[i].sector_size = 2048;
        if (scsi_CDs[i].sector_size != 2048 && scsi_CDs[i].sector_size != 512) {
