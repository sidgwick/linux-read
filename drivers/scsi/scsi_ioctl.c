...
static int ioctl_internal_command(Scsi_Device *dev, char *cmd)
{
    ...
    scsi_do_cmd(SCpnt, cmd, NULL, 0, scsi_ioctl_done, MAX_TIMEOUT, MAX_RETRIES);
    ...
    if (driver_byte(SCpnt->result) != 0)
        ...
        default: /* Fall through for non-removable media */
            printk("SCSI CD error: host %d id %d lun %d return code = %x\n", dev->host->host_no,
                   dev->id, dev->lun, SCpnt->result);
            printk("\tSense class %x, sense error %x, extended sense %x\n",
                   sense_class(SCpnt->sense_buffer[0]), sense_error(SCpnt->sense_buffer[0]),
...
static int ioctl_command(Scsi_Device *dev, void *buffer)
{
    ...
    scsi_do_cmd(SCpnt, cmd, buf, needed, scsi_ioctl_done, MAX_TIMEOUT, MAX_RETRIES);
...
int scsi_ioctl(Scsi_Device *dev, int cmd, void *arg)
{
    ...
    case SCSI_IOCTL_GET_IDLUN:
        ...
        put_fs_long(dev->id + (dev->lun << 8) + (dev->host->host_no << 16), (unsigned long *)arg);
        return 0;
