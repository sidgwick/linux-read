...
int aha1740_makecode(unchar *sense, unchar *status)
{
    ...
    if (!status_word.don) /* Anything abnormal was detected */
    {
        ...
        } else { /* Michael suggests, and Brad concurs: */
            if (status_word.qf) {
...
void aha1740_intr_handle(int foo)
{
    ...
    while (inb(G2STAT) & G2STAT_INTPEND) {
        ...
        case G2INTST_CCBGOOD:
            ecbptr = (struct ecb *)(((ulong)inb(MBOXIN0)) + ((ulong)inb(MBOXIN1) << 8) +
                                    ((ulong)inb(MBOXIN2) << 16) + ((ulong)inb(MBOXIN3) << 24));
         ...
         we will still have it in the cdb when we come back */
            if ((adapstat & G2INTST_MASK) == G2INTST_CCBERROR) {
                memcpy(SCtmp->sense_buffer, ecbptr->sense, sizeof(SCtmp->sense_buffer));
        ...
        case G2INTST_ASNEVENT:
            printk("aha1740 asynchronous event: %02x %02x %02x %02x %02x\n", adapstat, inb(MBOXIN0),
                   inb(MBOXIN1), inb(MBOXIN2), inb(MBOXIN3)); /* Say What? */
            outb(G2CNTRL_HRDY, G2CNTRL); /* Host Ready -> Mailbox in complete */
            break;
...
int aha1740_queuecommand(Scsi_Cmnd *SCpnt, void (*done)(Scsi_Cmnd *))
{
    ...
    printk("\n");
#endif
    if (done) { /*  You may question the code below, which contains potentially
      ...
      The loop actually only cycled < 3 times (we instrumented it). */
        ...
        while (!(inb(G2STAT) & G2STAT_MBXOUT))
            ;                                        /* Oh Well. */
...
int aha1740_detect(int hostnum)
{
    ...
    for (slot = MINEISA; slot <= MAXEISA; slot++) {
       ...
       go through the motions. */
        ...
        if (check_region(base, 0x5c))
            continue; /* See if in use */
    ...
    if ((inb(G2STAT) & (G2STAT_MBXOUT | G2STAT_BUSY)) !=
        G2STAT_MBXOUT) { /* If the card isn't ready, hard reset it */
