...
static struct Scsi_Host *aha_host[7] = {
    NULL,
...
static int aha1542_out(unsigned int base, unchar *cmdp, int len)
{
    ...
    return 0;
fail:
    printk("aha1542_out failed(%d): ", len + 1);
    aha1542_stat();
    return 1;
}
...
static int aha1542_in(unsigned int base, unchar *cmdp, int len)
{
    ...
    return 0;
fail:
    printk("aha1542_in failed(%d): ", len + 1);
    aha1542_stat();
    return 1;
}
...
#ifdef DEBUG
        if (errstatus)
            printk("(aha1542 error:%x %x %x) ", errstatus, ccb[mbo].hastat, ccb[mbo].tarstat);
#endif
   ...
   is how the host adapter will scan for them */
    ...
    if (SCpnt->use_sg) {
        ...
        for (i = 0; i < SCpnt->use_sg; i++) {
            if (sgpnt[i].length == 0 || SCpnt->use_sg > 16 || (((int)sgpnt[i].address) & 1) ||
                (sgpnt[i].length & 1)) {
                unsigned char *ptr;
                printk("Bad segment list supplied to aha1542.c (%d, %d)\n", SCpnt->use_sg, i);
                for (i = 0; i < SCpnt->use_sg; i++) {
                    printk("%d: %x %x %d\n", i, (unsigned int)sgpnt[i].address,
                           (unsigned int)sgpnt[i].alt_address, sgpnt[i].length);
                };
        ...
        if (((unsigned int)buff & 0xff000000))
            goto baddma;
...
*/
    ...
    if (inquiry_result[0] == 0x44 || inquiry_result[0] == 0x45) { /* Detect 1542C  */
...
int aha1542_detect(int hostnum)
{
    ...
    for (indx = 0; indx < sizeof(bases) / sizeof(bases[0]); indx++)
        if (!check_region(bases[indx], 4)) {
            shpnt = scsi_register(hostnum, sizeof(struct aha1542_hostdata));
            ...
            if (aha1542_getconfig(base_io, &irq_level, &dma_chan) == -1)
                goto unregister;
