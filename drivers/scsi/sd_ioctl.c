...
int sd_ioctl(struct inode *inode, struct file *file, unsigned int cmd, unsigned long arg)
{
    ...
    case HDIO_REQ: /* Return BIOS disk parameters */
        ...
        if (host->hostt->bios_param != NULL)
            host->hostt->bios_param(rscsi_disks[MINOR(dev) >> 4].capacity, dev, &diskinfo[0]);
        put_fs_byte(diskinfo[0], (char *)&loc->heads);
        put_fs_byte(diskinfo[1], (char *)&loc->sectors);
        put_fs_word(diskinfo[2], (short *)&loc->cylinders);
        put_fs_long(sd[MINOR(inode->i_rdev)].start_sect, (long *)&loc->start);
        return 0;
    case BLKGETSIZE: /* Return device size */
        ...
        if (error)
            return error;
        put_fs_long(sd[MINOR(inode->i_rdev)].nr_sects, (long *)arg);
        return 0;
    case BLKFLSBUF:
        if (!suser())
            return -EACCES;
