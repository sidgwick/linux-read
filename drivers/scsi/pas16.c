...
int scsi_irq_translate[] = {0, 0, 1, 2, 3, 4, 5, 6, 0, 0, 7, 8, 9, 0, 10, 11};
 ...
 */
int default_irqs[] = {PAS16_DEFAULT_BOARD_1_IRQ, PAS16_DEFAULT_BOARD_2_IRQ,
                      PAS16_DEFAULT_BOARD_3_IRQ, PAS16_DEFAULT_BOARD_4_IRQ};
...
int pas16_detect(int hostno)
{
    ...
    for (count = 0; current_override < NO_OVERRIDES; ++current_override) {
        ...
        printk("scsi%d : at 0x%04x", instance->host_no, (int)instance->io_port);
        if (instance->irq == IRQ_NONE)
...
static inline int NCR5380_pread(struct Scsi_Host *instance, unsigned char *dst, int len)
{
    ...
    register unsigned short reg = (unsigned short)(instance->io_port + P_DATA_REG_OFFSET);
    register i = len;
