...
static int do_ioctl(int target, unsigned char *sr_cmd, void *buffer, unsigned buflength)
{
    ...
    scsi_do_cmd(SCpnt, (void *)sr_cmd, buffer, buflength, sr_ioctl_done, IOCTL_TIMEOUT,
                IOCTL_RETRIES);
    ...
    if (driver_byte(result) != 0)
        ...
        default:
            printk("SCSI CD error: host %d id %d lun %d return code = %03x\n",
                   scsi_CDs[target].device->host->host_no, scsi_CDs[target].device->id,
                   scsi_CDs[target].device->lun, result);
