...
static struct override {
    ...
    [] = T128_OVERRIDE;
#else
    [4] = {{NULL, IRQ_AUTO}, {NULL, IRQ_AUTO}, {NULL, IRQ_AUTO}, {NULL, IRQ_AUTO}};
#endif
...
static struct base {
    unsigned char *address;
    int noauto;
} bases[] = {{(unsigned char *)0xcc000, 0},
             {(unsigned char *)0xc8000, 0},
             {(unsigned char *)0xdc000, 0},
             {(unsigned char *)0xd8000, 0}};
...
int t128_detect(int hostno)
{
    ...
    for (count = 0; current_override < NO_OVERRIDES; ++current_override) {
            ...
            for (; !base && (current_base < NO_BASES); ++current_base) {
                ...
                for (sig = 0; sig < NO_SIGNATURES; ++sig)
                    if (!bases[current_base].noauto &&
                        !memcmp(bases[current_base].address + signatures[sig].offset,
        ...
        printk("scsi%d : at 0x%08x", instance->host_no, (int)instance->base);
        if (instance->irq == IRQ_NONE)
            printk(" interrupts disabled");
        else
            printk(" irq %d", instance->irq);
        printk(" options CAN_QUEUE=%d  CMD_PER_LUN=%d release=%d", CAN_QUEUE, CMD_PER_LUN,
               T128_PUBLIC_RELEASE);
...
{
    register unsigned char *reg = (unsigned char *)(instance->base + T_DATA_REG_OFFSET), *d = dst;
    register i = len;
...
{
    register unsigned char *reg = (unsigned char *)(instance->base + T_DATA_REG_OFFSET), *s = src;
    register i = len;
