...
static void *addresses[] = {(void *)0xc8000, (void *)0xca000, (void *)0xce000, (void *)0xde000};
...
static void print_banner(void)
{
    ...
    printk("Future Domain: BIOS version %d.%d, %s\n", bios_major, bios_minor,
           chip == tmc1800 ? "TMC-1800" : (chip == tmc18c50 ? "TMC-18C50" : "Unknown"));
...
static int fdomain_is_valid_port(int port)
{
...
#if DEBUG_DETECT
    printk(" (%x%x),", inb(port + MSB_ID_Code), inb(port + LSB_ID_Code));
#endif
    ...
    */
    ...
    if (inb(port + LSB_ID_Code) != 0xe9) { /* test for 0x6127 id */
        if (inb(port + LSB_ID_Code) != 0x27)
            return 0;
        if (inb(port + MSB_ID_Code) != 0x61)
            return 0;
        chip = tmc1800;
    } else { /* test for 0xe960 id */
        if (inb(port + MSB_ID_Code) != 0x60)
            return 0;
        chip = tmc18c50;
    }
...
int fdomain_16x0_detect(int hostnum)
{
    ...
    for (i = 0; !bios_base && i < ADDRESS_COUNT; i++) {
        ...
        for (j = 0; !bios_base && j < SIGNATURE_COUNT; j++) {
            if (!memcmp(((char *)addresses[i] + signatures[j].sig_offset), signatures[j].signature,
    ...
    if (bios_major == 2) {
     ...
     DOS (this geometry has nothing to do with physical geometry).
       */
        ...
        port_base = *((char *)bios_base + 0x1fcc) + (*((char *)bios_base + 0x1fcd) << 8);
        ...
        if (!retcode) {
            ...
            if (!retcode) {
                ...
                if (!retcode) {
                    ...
                    printk("%lu MB (%lu byte blocks)", ((capacity + 5L) / 10L), size);
                } else {
                    memcpy(SCinit.cmnd, do_request_sense, sizeof(do_request_sense));
                    retcode = fdomain_16x0_command(&SCinit);
                }
...
void my_done(int error)
{
    if (in_command) {
        ...
        if (current_SC->scsi_done)
            ...
            panic("Future Domain: current_SC->scsi_done() == NULL");
    } else {
...
#endif
        my_done((current_SC->SCp.Status & 0xff) | ((current_SC->SCp.Message & 0xff) << 8) |
                (DID_OK << 16));
#if EVERY_ACCESS
        printk("RETURNING.\n");
#endif
...
int fdomain_16x0_queue(Scsi_Cmnd *SCpnt, void (*done)(Scsi_Cmnd *))
{
    ...
    printk("queue: target = %d cmnd = 0x%02x pieces = %d size = %u\n", SCpnt->target,
           *(unsigned char *)SCpnt->cmnd, SCpnt->use_sg, SCpnt->request_bufflen);
#endif
...
int fdomain_16x0_biosparam(int size, int dev, int *info_array)
{
    ...
    */
    ...
    if (bios_major == 2) {
        ...
        if (!info_array[0] || !info_array[1] || !info_array[2] ||
            ...
            info_array[0] = info_array[1] = info_array[2] = 0;
        }
    }
