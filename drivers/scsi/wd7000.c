...
#ifdef DEBUG
    if (scsierr || hosterr)
        printk("\nSCSI command error: SCSI %02x host %04x return %d", scsierr, in_error, hosterr);
#endif
...
#ifdef DEBUG
    printk(" ICMB %d posted for SCB/ICB %06x, status %02x, vue %02x", icmb, scb, icmb_status,
           scb->vue);
#endif
...
{
    int i;
    unchar init_block[] = {INITIALIZATION, 7, BUS_ON, BUS_OFF, 0, 0, 0, 0, OGMB_CNT, ICMB_CNT};
...
void wd7000_revision(void)
{
    ...
    */
    if (rev_1 >= 7)
        scsi_hosts[wd7000_host].sg_tablesize = WD7000_SG;
}
