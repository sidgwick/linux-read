...
static int mcd_ioctl(struct inode *ip, struct file *fp, unsigned int cmd, unsigned long arg)
{
    int i, st;
    ...
    case CDROMPLAYTRKIND: /* Play a track.  This currently ignores index. */
        ...
        if (ti.cdti_trk0 < DiskInfo.first || ti.cdti_trk0 > DiskInfo.last ||
        ...
        printk("play: %02x:%02x.%02x to %02x:%02x.%02x\n", mcd_Play.start.min, mcd_Play.start.sec,
               mcd_Play.start.frame, mcd_Play.end.min, mcd_Play.end.sec, mcd_Play.end.frame);
#endif
    ...
    case CDROMREADTOCENTRY: /* Read an entry in the table of contents */
        ...
        else if (entry.cdte_track > DiskInfo.last || entry.cdte_track < DiskInfo.first)
            return -EINVAL;
...
static struct sigaction mcd_sigaction = {mcd_interrupt, 0, SA_INTERRUPT, NULL};
...
static int GetDiskInfo()
{
    ...
    printk("Disk Info: first %d last %d length %02x:%02x.%02x first %02x:%02x.%02x\n",
           DiskInfo.first, DiskInfo.last, DiskInfo.diskLength.min, DiskInfo.diskLength.sec,
           DiskInfo.diskLength.frame, DiskInfo.firstTrack.min, DiskInfo.firstTrack.sec,
...
static int GetToc()
{
    ...
    for (i = 1; i <= DiskInfo.last; i++)
        printk("i = %2d ctl-adr = %02X track %2d px %02X %02X:%02X.%02X    %02X:%02X.%02X\n", i,
               Toc[i].ctrl_addr, Toc[i].track, Toc[i].pointIndex, Toc[i].trackTime.min,
               Toc[i].trackTime.sec, Toc[i].trackTime.frame, Toc[i].diskTime.min,
