...
static unsigned short cdu31a_addresses[] = {0x340,  /* Standard configuration Sony Interface */
                                            0x1f88, /* Fusion CD-16 */
 ...
 */
int check_cdu31a_media_change(int full_dev, int flag)
{
    int retval, target;
 ...
 * disk is inserted.
 */
static void set_drive_params(void)
{
    ...
    params[1] = 0x03;
    do_sony_cd_cmd(SONY_SET_DRIVE_PARAM_CMD, params, 2, res_reg, &res_size);
 ...
 */
static void read_data_block(unsigned char *data, unsigned char *result_buffer,
 ...
 */
static unsigned int get_data(unsigned char *orig_data,
                             unsigned char *params, /* 6 bytes with the MSF start address
                                           and number of sectors to read. */
                             unsigned int orig_data_size, unsigned char *result_buffer,
                             unsigned int *result_size)
{
...
retry_cd_operation:
    ...
    if (((result_buffer[0] & 0x20) == 0x20) && (num_retries < MAX_CDU31A_RETRIES)) {
        num_retries++;
        current->state = TASK_INTERRUPTIBLE;
 ...
 */
static void do_cdu31a_request(void)
{
    int block;
    unsigned int dev;
    int nsect;
...
static int read_subcode(void)
{
    ...
    do_sony_cd_cmd(SONY_REQ_SUBCODE_ADDRESS_CMD, NULL, 0, (unsigned char *)last_sony_subcode,
                   &res_size);
 ...
 */
static int sony_get_subchnl_info(long arg)
{
 ...
 */
static int scd_ioctl(struct inode *inode, struct file *file, unsigned int cmd, unsigned long arg)
{
    unsigned int dev;
    unsigned char res_reg[2];
    unsigned int res_size;
    ...
    case CDROMPLAYTRKIND: /* Play a track.  This currently ignores index. */
    {
        ...
        if ((ti.cdti_trk0 < sony_toc->first_track_num) ||
            (ti.cdti_trk0 > sony_toc->last_track_num) || (ti.cdti_trk1 < ti.cdti_trk0)) {
            return -EINVAL;
        }
        ...
        if (ti.cdti_trk1 >= bcd_to_int(sony_toc->last_track_num)) {
            log_to_msf(msf_to_log(sony_toc->lead_out_start_msf) - 1, &(params[4]));
        } else {
            track_idx = find_track(int_to_bcd(ti.cdti_trk1 + 1));
            if (track_idx < 0) {
                return -EINVAL;
            }
            log_to_msf(msf_to_log(sony_toc->tracks[track_idx].track_start_msf) - 1, &(params[4]));
        }
        params[0] = 0x03;
...
static int scd_open(struct inode *inode, struct file *filp)
{
    ...
    respinup_on_open:
        ...
        if ((res_size < 2) || ((res_reg[0] != 0) && (res_reg[1] != 0))) {
            ...
            if ((res_reg[1] == SONY_NOT_SPIN_ERR) && (num_spin_ups < MAX_CDU31A_RETRIES)) {
                num_spin_ups++;
 ...
 */
static void scd_release(struct inode *inode, struct file *filp)
{
    unsigned char res_reg[2];
    unsigned int res_size;
...
void get_drive_configuration(unsigned short base_io, unsigned char res_reg[],
                             unsigned int *res_size)
{
    int retry_count;
 ...
 */
unsigned long cdu31a_init(unsigned long mem_start, unsigned long mem_end)
{
    ...
    */
    ...
    i = 0;
    drive_found = 0;
    while ((cdu31a_addresses[i] != 0) && (!drive_found)) {
        if (check_region(cdu31a_addresses[i], 4)) {
            i++;
            continue;
        }
        get_drive_configuration(cdu31a_addresses[i], drive_config.exec_status, &res_size);
        if ((res_size > 2) && ((drive_config.exec_status[0] & 0x20) == 0x00)) {
            ...
            printk("Sony I/F CDROM : %8.8s %16.16s %8.8s with %s load mechanism\n",
                   drive_config.vendor_id, drive_config.product_id, drive_config.product_rev_level,
