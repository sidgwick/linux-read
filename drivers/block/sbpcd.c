...
#else
static int sbpcd_debug = (1 << DBG_INF) | (1 << DBG_TOC) | (1 << DBG_IOC) | (1 << DBG_IOX);
#endif
...
static void flush_status(void)
{
    ...
    if (current == task[0])
        for (i = maxtim02; i != 0; i--)
            inb(CDi_status);
    else {
        sbp_sleep(150);
        for (i = maxtim_data; i != 0; i--)
            inb(CDi_status);
    }
#else
    timed_out = 0;
...
static int EvaluateStatus(int st)
{
    if (!new_drive) {
        DS[d].status_byte = 0;
        if (st & p_caddin_old)
            DS[d].status_byte |= p_door_closed | p_caddy_in;
        if (st & p_spinning)
            DS[d].status_byte |= p_spinning;
        if (st & p_check)
            DS[d].status_byte |= p_check;
        if (st & p_busy_old)
            DS[d].status_byte |= p_busy_new;
        if (st & p_disk_ok)
            DS[d].status_byte |= p_disk_ok;
    } else {
        DS[d].status_byte = st;
...
static int ResponseStatus(void)
{
    ...
    if (current == task[0]) {
        if (flags_cmd_out & f_respo3)
            j = maxtim_8;
        else if (flags_cmd_out & f_respo2)
            j = maxtim16;
        else
            j = maxtim04;
        for (; j != 0; j--) {
...
int cmd_out(void)
{
    ...
    if (DS[d].in_SpinUp != 0)
        DPRINTF((DBG_SPI, "SBPCD: to CDi_stat_loop.\n"));
    if (flags_cmd_out & f_lopsta) {
        i = CDi_stat_loop();
        if ((i < 0) || !(i & s_attention))
            return (-9);
    }
    if (!(flags_cmd_out & f_getsta))
        goto LOC_229;

LOC_228:
    if (DS[d].in_SpinUp != 0)
        DPRINTF((DBG_SPI, "SBPCD: to xx_ReadStatus.\n"));
    xx_ReadStatus();

LOC_229:
    if (flags_cmd_out & f_ResponseStatus) {
        if (DS[d].in_SpinUp != 0)
            DPRINTF((DBG_SPI, "SBPCD: to ResponseStatus.\n"));
        i = ResponseStatus();
        ...
        if (flags_cmd_out & (f_bit1 | f_wait_if_busy)) {
            if (!st_check) {
                if (flags_cmd_out & f_bit1)
                    if (i & p_success_old)
                        goto LOC_232;
                if (!(flags_cmd_out & f_wait_if_busy))
                    goto LOC_228;
                if (!st_busy)
                    goto LOC_228;
            }
...
LOC_232:
    if (!(flags_cmd_out & f_obey_p_check))
        return (0);
    if (!st_check)
        return (0);
    if (DS[d].in_SpinUp != 0)
        DPRINTF((DBG_SPI, "SBPCD: to xx_ReadError.\n"));
    i = xx_ReadError();
    if (DS[d].in_SpinUp != 0)
        DPRINTF((DBG_SPI, "SBPCD: to cmd_out OK.\n"));
    return (i);
}
/*==========================================================================*/
static int xx_Seek(u_int pos, char f_blk_msf)
{
    ...
    if (!new_drive) {
        ...
        drvcmd[4] = pos & 0x00FF;
        flags_cmd_out =
            f_putcmd | f_respo2 | f_lopsta | f_getsta | f_ResponseStatus | f_obey_p_check | f_bit1;
    } else {
        if (f_blk_msf == 0)
            pos = blk2msf(pos);
...
static int xx_SetVolume(void)
{
    ...
    if (new_drive) {
        ...
        if (DS[d].drv_type >= drv_300) {
            ...
            value0 = volume1 & 0xFC;
            if ((volume0 != 0) && (volume0 < 4))
                control0 |= 0x04;
            if ((volume1 != 0) && (volume1 < 4))
                value0 |= 0x04;
            if (channel0 != 0)
                control0 |= 0x01;
            if (channel1 == 1)
            ...
            if ((volume0 != 0) && (volume1 != 0)) {
                if (volume0 == 0xFF)
                    volume1 = 0xFF;
                else if (volume1 == 0xFF)
                    volume0 = 0xFF;
            } else if (DS[d].drv_type < drv_201)
                volume0 = volume1 = value0;
...
static int SetSpeed(void)
{
    ...
    if (!(DS[d].drv_options & (speed_auto | speed_300 | speed_150)))
        return (0);
    speed = 0x80;
    if (!(DS[d].drv_options & speed_auto)) {
        speed |= 0x40;
        if (!(DS[d].drv_options & speed_300))
            speed = 0;
    }
...
static int xx_ReadSubQ(void)
{
    ...
    if (!new_drive)
        i = 5;
    DS[d].SubQ_run_tot =
        make32(make16(0, infobuf[i]), make16(infobuf[i + 1], infobuf[i + 2])); /* msf-bin */
    i = 7;
    if (!new_drive)
        i = 9;
    DS[d].SubQ_run_trk =
...
static int xx_ReadCapacity(void)
{
    ...
    if (new_drive)
        DS[d].CDsize_blk = msf2blk(DS[d].CDsize_blk);
...
static int xx_ReadTocDescr(void)
{
    ...
    DS[d].diskstate_flags |= toc_bit;
    DPRINTF((DBG_TOC, "SBPCD: TocDesc: %02X %02X %02X %08X\n", DS[d].xa_byte, DS[d].n_first_track,
             DS[d].n_last_track, DS[d].size_msf));
    return (0);
}
/*==========================================================================*/
static int xx_ReadTocEntry(int num)
{
    ...
    DPRINTF((DBG_TOC, "SBPCD: TocEntry: %02X %02X %02X %02X %08X\n", DS[d].TocEnt_nixbyte,
             DS[d].TocEnt_ctl_adr, DS[d].TocEnt_number, DS[d].TocEnt_format, DS[d].TocEnt_address));
    return (0);
}
/*==========================================================================*/
    ...
    if (new_drive) {
        ...
        if ((infobuf[0] & 0x80) != 0) {
            DPRINTF((DBG_MUL, "SBPCD: MultiSession CD detected: %02X %02X %02X %02X %02X %02X\n",
                     infobuf[0], infobuf[1], infobuf[2], infobuf[3], infobuf[4], infobuf[5]));
            DS[d].f_multisession = 1;
            DS[d].lba_multi =
                msf2blk(make32(make16(0, infobuf[1]), make16(infobuf[2], infobuf[3])));
        }
    }
    DS[d].diskstate_flags |= multisession_bit;
    return (0);
...
static int check_version(void)
{
    ...
    i = cmd_out();
    if (i < 0)
        DPRINTF((DBG_INI, "SBPCD: cmd_83 returns %d\n", i));

    DPRINTF((DBG_INI, "SBPCD: infobuf = \""));
    for (i = 0; i < 12; i++)
        DPRINTF((DBG_INI, "%c", infobuf[i]));
    DPRINTF((DBG_INI, "\"\n"));

    for (i = 0; i < 4; i++)
        if (infobuf[i] != drive_family[i])
            break;
    if (i == 4) {
    ...
    for (j = 0; j < 4; j++)
        DS[d].firmware_version[j] = infobuf[i + j];
    j = (DS[d].firmware_version[0] & 0x0F) * 100 + (DS[d].firmware_version[2] & 0x0F) * 10 +
        (DS[d].firmware_version[3] & 0x0F);
    if (new_drive) {
        if (j < 100)
            DS[d].drv_type = drv_099;
        else
            DS[d].drv_type = drv_100;
    } else if (j < 200)
        DS[d].drv_type = drv_199;
    else if (j < 201)
        DS[d].drv_type = drv_200;
    else if (j < 210)
        DS[d].drv_type = drv_201;
    else if (j < 211)
        DS[d].drv_type = drv_210;
    else if (j < 300)
        DS[d].drv_type = drv_211;
    else
        DS[d].drv_type = drv_300;
    return (0);
}
/*==========================================================================*/
...
static int check_drives(void)
{
    ...
    for (j = 0; j < NR_SBPCD; j++) {
        ...
        if (i >= 0) {
            ...
            if (!new_drive)
                DS[d].drv_options &= ~(speed_auto | speed_300 | speed_150);
            printk("%sDrive %d: %s%.4s (%.4s)\n", printk_header, DS[d].drv_minor, drive_family,
                   DS[d].drive_model, DS[d].firmware_version);
            printk_header = "       - ";
        } else
            DS[d].drv_minor = -1;
    }
...
static int check_allowed1(u_char func1, u_char func2)
{
    ...
    if (func1 == read_long)
        return (-1);
    if (func1 == read_long_prefetch)
        return (-1);
    if (func1 == seek)
        return (-1);
    if (func1 == audio_play)
        return (-1);
    if (func1 == audio_pause)
        return (-1);
    if (func1 == audio_resume)
        return (-1);
    if (func1 != ioctl_i)
        return (0);
    if (func2 == tell_SubQ_run_tot)
        return (-1);
    if (func2 == tell_cdsize)
        return (-1);
    if (func2 == tell_TocDescrip)
        return (-1);
    if (func2 == tell_TocEntry)
        return (-1);
    if (func2 == tell_subQ_info)
        return (-1);
    if (new_drive)
        if (func2 == tell_SubChanInfo)
            return (-1);
    if (func2 == tell_UPC)
        return (-1);
#else
 ...
 */
static int sbpcd_ioctl(struct inode *inode, struct file *file, u_int cmd, u_long arg)
{
    int i, st;
    ...
    case CDROMPLAYMSF:
        ...
        DS[d].pos_audio_start = (msf.cdmsf_min0 << 16) | (msf.cdmsf_sec0 << 8) | msf.cdmsf_frame0;
        DS[d].pos_audio_end = (msf.cdmsf_min1 << 16) | (msf.cdmsf_sec1 << 8) | msf.cdmsf_frame1;
        DPRINTF((DBG_IOX, "SBPCD: ioctl: CDROMPLAYMSF %08X %08X\n", DS[d].pos_audio_start,
                 DS[d].pos_audio_end));
    ...
    case CDROMPLAYTRKIND: /* Play a track.  This currently ignores index. */
        ...
        DPRINTF((DBG_IOX, "SBPCD: ioctl: trk0: %d, ind0: %d, trk1:%d, ind1:%d\n", ti.cdti_trk0,
                 ti.cdti_ind0, ti.cdti_trk1, ti.cdti_ind1));
        if (ti.cdti_trk0 < DS[d].n_first_track)
            return (-EINVAL);
        if (ti.cdti_trk0 > DS[d].n_last_track)
            return (-EINVAL);
        if (ti.cdti_trk1 < ti.cdti_trk0)
            ti.cdti_trk1 = ti.cdti_trk0;
        if (ti.cdti_trk1 > DS[d].n_last_track)
    ...
    case CDROMREADTOCENTRY: /* Read an entry in the table of contents */
        ...
        if (i == CDROM_LEADOUT)
            i = DS[d].n_last_track + 1;
        else if (i < DS[d].n_first_track || i > DS[d].n_last_track)
            return (-EINVAL);
    ...
    case CDROMSUBCHNL: /* Get subchannel info */
      ...
      if (DS[d].SubQ_audio==0x80) DS[d].SubQ_audio=CDROM_AUDIO_NO_STATUS;
        ...
        DPRINTF((DBG_IOS, "SBPCD: CDROMSUBCHNL: %1X %02X %08X %08X %02X %02X %06X %06X\n",
                 SC.cdsc_format, SC.cdsc_audiostatus, SC.cdsc_adr, SC.cdsc_ctrl, SC.cdsc_trk,
                 SC.cdsc_ind, SC.cdsc_absaddr, SC.cdsc_reladdr));
        return (0);
...
static void sbp_transfer(void)
{
    ...
    while ((CURRENT->nr_sectors > 0) && (CURRENT->sector / 4 >= DS[d].sbp_first_frame) &&
...
static int sbp_status(void)
{
    ...
    if (!st_spinning)
        DPRINTF((DBG_SPI, "SBPCD: motor got off - ignoring.\n"));
...
static void sbp_read_cmd(void)
{
    ...
    if (new_drive) {
        ...
        if ((DS[d].f_multisession) && (multisession_valid)) {
            DPRINTF((DBG_MUL, "SBPCD: MultiSession: use %08X for %08X (msf)\n",
                     blk2msf(DS[d].lba_multi + block), blk2msf(block)));
            block = DS[d].lba_multi + block;
        }
#else
        if ((block == 166) && (DS[d].f_multisession) && (multisession_valid)) {
            DPRINTF((DBG_MUL, "SBPCD: MultiSession: use %08X for %08X (msf)\n",
                     blk2msf(DS[d].lba_multi + 16), blk2msf(block)));
    ...
    if (block + SBP_BUFFER_FRAMES <= DS[d].CDsize_frm)
        ...
        if (DS[d].sbp_read_frames < 1) {
            DPRINTF(
                (DBG_INF, "SBPCD: requested frame %d, CD size %d ???\n", block, DS[d].CDsize_frm));
...
static int sbp_data(void)
{
    ...
    for (frame = DS[d].sbp_current; frame < DS[d].sbp_read_frames && !error_flag; frame++) {
        ...
        if (j & s_not_data_ready) {
            if ((DS[d].ored_ctl_adr & 0x40) == 0)
                ...
                printk("SBPCD: sbp_data: DATA_READY timeout.\n");
            error_flag++;
            break;
        }
    ...
    if (!new_drive) {
        ...
        for (timeout = jiffies + 100; timeout > jiffies; timeout--) {
            ...
            if (i != 0 || timeout <= jiffies)
                break;
            sbp_sleep(0);
            i = 1;
        }
        if (i == 0) {
            DPRINTF((DBG_INF, "SBPCD: STATUS TIMEOUT AFTER READ"));
        }
        if (!(j & s_attention)) {
    ...
    do {
        ...
        if (i < 0) {
            DPRINTF((DBG_INF, "SBPCD: xx_ReadStatus error after read: %02X\n", DS[d].status_byte));
            return (0);
        }
 ...
 */
#if SBPCD_USE_IRQ
static struct sigaction sbpcd_sigaction = {sbpcd_interrupt, 0, SA_INTERRUPT, NULL};
#endif SBPCD_USE_IRQ
/*==========================================================================*/
/*
...
u_long sbpcd_init(u_long mem_start, u_long mem_end)
{
    ...
    for (port_index = 0; port_index < NUM_AUTOPROBE; port_index += 2) {
        ...
        if (autoprobe[port_index + 1] == 0)
            type = str_lm;
        else
            type = str_sb;
        sbpcd_setup(type, addr);
        DPRINTF(
            (DBG_INF, "SBPCD: Trying to detect a %s CD-ROM drive at 0x%X.\n", type, CDo_command));
...
int check_sbpcd_media_change(int full_dev, int unused_minor)
{
    ...
    if (DS[d].CD_changed == 0xFF)
        DPRINTF((DBG_CHK, "SBPCD: media_check: \"changed\" assumed.\n"));
    if (!st_spinning)
        DPRINTF((DBG_CHK, "SBPCD: media_check: motor off.\n"));
    if (!st_door_closed) {
    ...
    if (!st_diskok)
        DPRINTF((DBG_CHK, "SBPCD: media_check: !st_diskok.\n"));
    ...
    if (!st_diskok)
        return (1); /* disk not o.k. */
    if (!st_caddy_in)
        return (1); /* disk removed */
    if (!st_door_closed)
        return (1); /* door open */
    return (0);
}
