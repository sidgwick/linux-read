...
static int floppy_sizes[] = {
    MAX_DISK_SIZE, MAX_DISK_SIZE, MAX_DISK_SIZE, MAX_DISK_SIZE, 360,  360,  360,  360,
    1200,          1200,          1200,          1200,          360,  360,  360,  360,
 ...
 * format request descriptor.
 */
#define CURRENT_DEVICE (format_status == FORMAT_BUSY ? format_req.device : (CURRENT->dev))

/* Current error count. */
#define CURRENT_ERRORS (format_status == FORMAT_BUSY ? format_errors : (CURRENT->errors))
...
static struct timer_list motor_on_timer[4] = {{NULL, NULL, 0, 0, motor_on_callback},
...
static struct timer_list motor_off_timer[4] = {{NULL, NULL, 0, 0, motor_off_callback},
...
static void tell_sector(int nr)
{
    if (nr != 7) {
        ...
        printk(": track %d, head %d, sector %d", reply_buffer[3], reply_buffer[4], reply_buffer[5]);
} /* tell_sector */
...
static void rw_interrupt(void)
{
    ...
    if (read_track) {
        ...
        buffer_area = floppy_track_buffer + ((sector - 1 + head * floppy->sect) << 9);
        copy_buffer(buffer_area, CURRENT->buffer);
    } else if (command == FD_READ && (unsigned long)(CURRENT->buffer) >= LAST_DMA_ADDR)
...
{
    read_track =
        (command == FD_READ) && (CURRENT_ERRORS < 4) && (floppy->sect <= MAX_BUFFER_SECTORS);
...
static void setup_format_params(void)
{
    ...
    /* count backwards */
    total_shift = floppy->sect - ((track_shift * track + head_shift * head) % floppy->sect);
...
repeat:
    ...
    if (format_status != FORMAT_BUSY) {
        ...
        if (((unsigned)format_req.track) >= floppy->track || (format_req.head & 0xfffe) ||
            probing) {
            request_done(0);
            goto repeat;
        }
    ...
    if ((seek_track == buffer_track) && (current_drive == buffer_drive)) {
        buffer_area = floppy_track_buffer + ((sector + head * floppy->sect) << 9);
        if (command == FD_READ) {
            copy_buffer(buffer_area, CURRENT->buffer);
            request_done(1);
            goto repeat;
 ...
 */
static struct sigaction floppy_sigaction = {floppy_interrupt, 0, SA_INTERRUPT, NULL};
