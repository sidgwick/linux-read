...
static XD_SIGNATURE xd_sigs[] = {
    {0x0000, "Override geometry handler", NULL, xd_override_init_drive,
     "n unknown"}, /* Pat Mackinlay, smackinla@cc.curtin.edu.au (pat@gu.uwa.edu.au) */
    {0x000B, "CXD23A Not an IBM ROM (C)Copyright Data Technology Corp 12/03/88",
     xd_dtc_init_controller, xd_dtc_init_drive,
     " DTC 5150X"}, /* Pat Mackinlay, smackinla@cc.curtin.edu.au (pat@gu.uwa.edu.au) */
    {0x0008, "07/15/86 (C) Copyright 1986 Western Digital Corp", xd_wd_init_controller,
     xd_wd_init_drive,
     " Western Digital 1002AWX1"}, /* Ian Justman, citrus!ianj@csusac.ecs.csus.edu */
    {0x0008, "06/24/88 (C) Copyright 1988 Western Digital Corp", xd_wd_init_controller,
     xd_wd_init_drive, " Western Digital 1004A27X"}, /* Dave Thaler, thalerd@engin.umich.edu */
    {0x0008, "06/24/88(C) Copyright 1988 Western Digital Corp.", xd_wd_init_controller,
     xd_wd_init_drive,
     " Western Digital WDXT-GEN2"}, /* Dan Newcombe, newcombe@aa.csc.peachnet.edu */
    {0x0015, "SEAGATE ST11 BIOS REVISION", xd_seagate_init_controller, xd_seagate_init_drive,
     " Seagate ST11M/R"}, /* Salvador Abreu, spa@fct.unl.pt */
    {0x0010, "ST11R BIOS", xd_seagate_init_controller, xd_seagate_init_drive,
     " Seagate ST11M/R"}, /* Risto Kankkunen, risto.kankkunen@cs.helsinki.fi */
    {0x1000, "(c)Copyright 1987 SMS", xd_omti_init_controller, xd_omti_init_drive,
     "n OMTI 5520"}, /* Dirk Melchers, dirk@merlin.nbg.sub.org */
};
static u_char *xd_bases[] = {(u_char *)0xC8000, (u_char *)0xCA000, (u_char *)0xCC000,
...
static struct gendisk xd_gendisk = {
    MAJOR_NR, "xd", 6, 1 << 6, XD_MAXDRIVES, xd_geninit, xd, xd_sizes, 0, (void *)xd_info, NULL};
static struct file_operations xd_fops = {NULL, block_read, block_write, NULL,       NULL, xd_ioctl,
                                         NULL, xd_open,    xd_release,  block_fsync};
    ...
    if (xd_detect(&controller, &address)) {
        ...
        for (i = 0; i < xd_drives; i++)
            printk("xd_init: drive %d geometry - heads = %d, cylinders = %d, sectors = %d\n", i,
                   xd_info[i].heads, xd_info[i].cylinders, xd_info[i].sectors);
...
static u_char xd_detect(u_char *controller, u_char **address)
{
    ...
    for (i = 0; i < (sizeof(xd_bases) / sizeof(xd_bases[0])) && !found; i++)
        for (j = 1; j < (sizeof(xd_sigs) / sizeof(xd_sigs[0])) && !found; j++)
            if (!memcmp(xd_bases[i] + xd_sigs[j].offset, xd_sigs[j].string,
...
static void do_xd_request(void)
{
    ...
    while (code = 0, CURRENT) {
        ...
        if (CURRENT_DEV < xd_drives &&
            CURRENT->sector + CURRENT->nr_sectors <= xd[MINOR(CURRENT->dev)].nr_sects) {
...
static int xd_reread_partitions(int dev)
{
    ...
    xd_gendisk.part[start].nr_sects =
...
static int xd_readwrite(u_char operation, u_char drive, char *buffer, u_int block, u_int count)
{
    ...
    while (count) {
        ...
        mode = xd_setup_dma(operation == READ ? DMA_MODE_READ : DMA_MODE_WRITE, (u_char *)buffer,
                            temp * 0x200);
        xd_build(cmdblk, operation == READ ? CMD_READ : CMD_WRITE, drive, head, cylinder, sector,
                 temp & 0xFF, control);
        ...
        case 2:
            ...
            case 0:
                printk("xd_readwrite: drive error, code = 0x%X", sense[0] & 0x0F);
                break;
            case 1:
                printk("xd_readwrite: controller error, code = 0x%X", sense[0] & 0x0F);
                break;
            case 2:
                printk("xd_readwrite: command error, code = 0x%X", sense[0] & 0x0F);
                break;
            case 3:
                printk("xd_readwrite: miscellaneous error, code = 0x%X", sense[0] & 0x0F);
                break;
            }
            if (sense[0] & 0x80)
                printk(" - drive = %d, head = %d, cylinder = %d, sector = %d\n", sense[1] & 0xE0,
                       sense[1] & 0x1F, ((sense[2] & 0xC0) << 2) | sense[3], sense[2] & 0x3F);
            else
...
static u_char *xd_build(u_char *cmdblk, u_char command, u_char drive, u_char head, u_short cylinder,
                        u_char sector, u_char count, u_char control)
{
    cmdblk[0] = command;
...
static u_int xd_command(u_char *command, u_char mode, u_char *indata, u_char *outdata,
                        u_char *sense, u_long timeout)
{
...
static void xd_dtc_init_controller(u_char *address)
{
    ...
    case 0xCA000:
        ...
        break;
    default:
        printk("xd_dtc_init_controller: unsupported BIOS address %p\n", address);
        xd_iobase = 0x320;
        break;
    }
...
static void xd_dtc_init_drive(u_char drive)
{
    ...
    if (!xd_command(cmdblk, PIO_MODE, buf, 0, 0, XD_TIMEOUT * 2)) {
        ...
        xd_setparam(CMD_DTCSETPARAM, drive, xd_info[drive].heads, xd_info[drive].cylinders,
                    ((u_short *)(buf + 1))[0x05], ((u_short *)(buf + 1))[0x06], buf[0x0F]);
...
static void xd_wd_init_controller(u_char *address)
{
    ...
    case 0xD8000:
        ...
        break;
    default:
        printk("xd_wd_init_controller: unsupported BIOS address %p\n", address);
        xd_iobase = 0x320;
        break;
    }
...
static void xd_seagate_init_controller(u_char *address)
{
    ...
    case 0xE0000:
        ...
        break;
    default:
        printk("xd_seagate_init_controller: unsupported BIOS address %p\n", address);
        xd_iobase = 0x320;
        break;
    }
...
static void xd_omti_init_controller(u_char *address)
{
    ...
    case 0xE0000:
        ...
        break;
    default:
        printk("xd_omti_init_controller: unsupported BIOS address %p\n", address);
        xd_iobase = 0x320;
        break;
    }
...
static void xd_override_init_drive(u_char drive)
{
    ...
    for (i = 0; i < 3; i++) {
        while (min[i] != max[i] - 1) {
            ...
            xd_build(cmdblk, CMD_SEEK, drive, (u_char)test[0], (u_short)test[1], (u_char)test[2], 0,
                     0);
            if (!xd_command(cmdblk, PIO_MODE, 0, 0, 0, XD_TIMEOUT * 2))
                min[i] = test[i];
            else
...
static void xd_setparam(u_char command, u_char drive, u_char heads, u_short cylinders,
                        u_short rwrite, u_short wprecomp, u_char ecc)
{
    u_char cmdblk[14];
