...
int reg_mul(FPU_REG const *a, FPU_REG const *b, FPU_REG *dest, unsigned int control_w)
{
...
#ifdef DENORM_OPERAND
                if ((b->tag == TW_Valid) && (b->exp <= EXP_UNDER) && denormal_operand())
                    return 1;
#endif DENORM_OPERAND
                reg_move(a, dest);
                dest->sign = sign;
...
#ifdef DENORM_OPERAND
                if ((a->tag == TW_Valid) && (a->exp <= EXP_UNDER) && denormal_operand())
                    return 1;
#endif DENORM_OPERAND
                reg_move(b, dest);
                dest->sign = sign;
