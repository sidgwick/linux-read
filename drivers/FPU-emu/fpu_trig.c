...
static void rem_kernel(unsigned long long st0, unsigned long long *y, unsigned long long st1,
...
static int trig_arg(FPU_REG *X, int even)
{
    ...
    q = significand(&tmp);
    if (q) {
        rem_kernel(significand(X), &significand(&tmp), significand(&CONST_PI2), q,
                   X->exp - CONST_PI2.exp);
    ...
    if (even == FPTAN) {
        if (((X->exp >= EXP_BIAS) || ((X->exp == EXP_BIAS - 1) && (X->sigh >= 0xc90fdaa2))) ^
            (q & 1))
            even = FCOS;
        else
            even = 0;
         ...
         128 bits precision. */
            ...
            if ((X->exp == CONST_PI2.exp) &&
                ((X->sigh > CONST_PI2.sigh) ||
                 ((X->sigh == CONST_PI2.sigh) && (X->sigl > CONST_PI2.sigl)))) {
                       ...
                       unsigned long long q, int n)
{
    ...
    asm volatile("movl %2,%%eax; mull %4; subl %%eax,%0; sbbl %%edx,%1;
                     ...
                     3,
                 % % eax;
                 mull % 4; subl % % eax, % 1; movl % 2, % % eax; mull % 5; subl % % eax, % 1; "
                 : "=m"(x), "=m"(((unsigned *)&x)[1])
                 : "m"(st1), "m"(((unsigned *)&st1)[1]), "m"(q), "m"(((unsigned *)&q)[1])
                 : "%ax", "%dx");
...
#ifdef DENORM_OPERAND
        if (((FPU_st0_ptr->exp <= EXP_UNDER) || (st1_ptr->exp <= EXP_UNDER)) &&
            (denormal_operand()))
            return;
#endif DENORM_OPERAND
        ...
        if (expdif < 64) {
            ...
            if (expdif > -2) {
                ...
                if (tmp.exp >= EXP_BIAS) {
                    ...
                    rem_kernel(significand(FPU_st0_ptr), &significand(&tmp), significand(st1_ptr),
                               q, expdif);
         ...
         be done at high precision. */
            ...
            rem_kernel(significand(FPU_st0_ptr), &significand(&tmp), significand(st1_ptr),
                       significand(&tmp), tmp.exp - EXP_BIAS);
...
static void fyl2x(void)
{
    ...
    /* st(1) must be infinity here */
        ...
        if (FPU_st0_ptr->exp >= EXP_BIAS) {
            if ((FPU_st0_ptr->exp == EXP_BIAS) && (FPU_st0_ptr->sigh == 0x80000000) &&
                (FPU_st0_ptr->sigl == 0)) {
                /* st(0) holds 1.0 */
                /* infinity*log(1) */
                if (!arith_invalid(st1_ptr))
...
static void fpatan(void)
{
    ...
    if (!((FPU_st0_tag ^ TW_Valid) | (st1_tag ^ TW_Valid))) {
        ...
        if ((compare(st1_ptr) & ~COMP_Denormal) == COMP_A_lt_B) {
            ...
            inverted = 0;
            if ((st0_sign == 0) && (st1_ptr->exp - FPU_st0_ptr->exp < -64)) {
                control_word = saved_control;
                partial_status = saved_status;
                reg_div(st1_ptr, FPU_st0_ptr, st1_ptr, control_word | PR_64_BITS);
        ...
        if (FPU_st0_ptr->sign == SIGN_POS) { /* An 80486 preserves the sign */
            pop();
            return;
        } else
            reg_move(&CONST_PI, st1_ptr);
...
static void fyl2xp1(void)
{
    ...
    if (!((FPU_st0_tag ^ TW_Valid) | (st1_tag ^ TW_Valid))) {
...
#ifdef DENORM_OPERAND
        if (((FPU_st0_ptr->exp <= EXP_UNDER) || (st1_ptr->exp <= EXP_UNDER)) && denormal_operand())
            return;
#endif DENORM_OPERAND
        ...
        if (st1_tag <= TW_Zero) {
#ifdef DENORM_OPERAND
            if ((st1_tag == TW_Valid) && (st1_ptr->exp <= EXP_UNDER) && (denormal_operand()))
                return;
#endif DENORM_OPERAND
        ...
        if (st1_tag == TW_Infinity) {
            if (FPU_st0_ptr->sign == SIGN_NEG) {
                if ((FPU_st0_ptr->exp >= EXP_BIAS) &&
                    !((FPU_st0_ptr->sigh == 0x80000000) && (FPU_st0_ptr->sigl == 0))) {
                    /* st(0) holds < -1.0 */
#ifdef PECULIAR_486 /* Stupid 80486 doesn't worry about log(negative). */
                    st1_ptr->sign ^= SIGN_POS ^ SIGN_NEG;
#else
...
static void fscale(void)
{
    ...
    if (!((FPU_st0_tag ^ TW_Valid) | (st1_tag ^ TW_Valid))) {
        ...
        if (((st1_tag == TW_Infinity) && (st1_ptr->sign == SIGN_POS)) || (st1_tag == TW_Zero))
            return;
        else if (st1_tag == TW_Infinity) {
...
static FUNC const trig_table_a[] = {f2xm1, fyl2x, fptan, fpatan, fxtract, fprem1, fdecstp, fincstp};
...
static FUNC const trig_table_b[] = {fprem, fyl2xp1, fsqrt_, fsincos, frndint_, fscale, fsin, fcos};
