...
static unsigned short const lterms[HIPOWER][4] = {
    {0x846a, 0x42d1, 0xb544, 0x921f}, {0xe110, 0x75aa, 0xbc67, 0x1466},
    {0x503d, 0xa43f, 0x83c1, 0x000a}, {0x8f9d, 0x7a19, 0x00f4, 0x0000},
    {0xda03, 0x06aa, 0x0000, 0x0000},
};

static unsigned short const negterms[HIPOWER][4] = {
    {0x95ed, 0x2df2, 0xe731, 0xa55d}, {0xd159, 0xe62b, 0xd2cc, 0x0132},
    {0x6342, 0xe9fb, 0x3c60, 0x0000}, {0x6256, 0xdf5a, 0x0002, 0x0000},
...
void poly_sine(FPU_REG const *arg, FPU_REG *result)
{
    ...
    mul64(&significand(&fixed_arg), &significand(&fixed_arg), &significand(&arg_sqrd));
    mul64(&significand(&arg_sqrd), &significand(&arg_sqrd), &significand(&arg_to_4));
    ...
    *(short *)&(negaccum.sign) = 0;
    ...
    mul64(&significand(&arg_sqrd), &significand(&negaccum), &significand(&negaccum));
    ...
    if (result->exp >= EXP_BIAS) {
        ...
        if ((result->exp > EXP_BIAS)        /* Larger or equal 2.0 */
            ...
            printk("\nEXP=%d, MS=%08x, LS=%08x\n", result->exp, result->sigh, result->sigl);
            RE_ENTRANT_CHECK_ON;
#endif DEBUGGING
