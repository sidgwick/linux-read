...
static unsigned short const lterms[HIPOWER][4] = {
    ...
    {0xe177, 0xb82f, 0x7652, 0x7154}, {0xee0f, 0xe80f, 0x2770, 0x7b1c},
    {0x0fc0, 0xbe87, 0xb143, 0x49dd}, {0x78b9, 0xdadd, 0xec54, 0x34c2},
    {0x003a, 0x5de9, 0x628b, 0x2909}, {0x5588, 0xed16, 0x4abf, 0x2193},
    {0xb461, 0x85f7, 0x347a, 0x1c6a}, {0x0975, 0x87b3, 0xd5bf, 0x1876},
...
void poly_l2(FPU_REG const *arg, FPU_REG *result)
{
    ...
    mul64(&significand(&accum), &significand(&Xx), &significand(&accum));
...
int poly_l2p1(FPU_REG const *arg, FPU_REG *result)
{
    ...
    if ((arg_pl1.sign) | (arg_pl1.tag)) { /* We need a valid positive number! */
        return 1;
    }
    ...
    if (local_arg.exp >= EXP_BIAS - 3) {
        if ((local_arg.exp > EXP_BIAS - 3) || (local_arg.sigh > (unsigned)0xafb0ccc0)) {
