...
#define SPEAKER_CONTROL_GHOST                                                                      \
    0x738B                         /* R W    PC speaker     Control ghost register                */
#define SPEAKER_TIMER_CONTROL 0x43 /*   W  PC speaker     Timer control register                */
#define SPEAKER_TIMER_CONTROL_GHOST                                                                \
    0x778B                      /* R W  PC speaker     Timer control register ghost            */
#define SPEAKER_TIMER_DATA 0x42 /*   W  PC speaker     Timer data register                */
#define SPEAKER_TIMER_DATA_GHOST                                                                   \
    0x138A /* R W  PC speaker    Timer data register ghost            */

#define WARM_BOOT 0x41 /*   W  Control        Used to detect system warm boot              */
#define WARM_BOOT_GHOST                                                                            \
    0x7789                   /* ? W  Control        Use to get the card to fake warm boot        */
#define MASTER_DECODE 0x9A01 /*   W  Control        Address >> 2 of card base address        */
#define PRESCALE_DIVIDER                                                                           \
    0xBF8A                /* R W    PCM        Ration between Codec clock and master clock    */
...
#define S_C_1_PCM_CLOCK_SELECT                                                                     \
    0x02 /* R W  PCM        1=14.31818Mhz/12, 0=28.224Mhz master clock    */
#define S_C_1_FM_EMULATE_CLOCK                                                                     \
    0x04 /* R W  FM        1=use 28.224Mhz/2, 0=use 14.31818Mhz clock    */
#define S_C_1_PCS_STEREO                                                                           \
    0x10 /* R W  PC speaker    1=enable PC speaker stereo effect, 0=disable    */
#define S_C_1_PCS_REALSOUND                                                                        \
    0x20 /* R W  PC speaker    1=enable RealSound enhancement, 0=disable    */
...
#define S_C_3_PCM_CLOCK_SELECT                                                                     \
    0x02 /* R W    PCM        1=use 1.008Mhz clock for PCM, 0=don't        */
#define SYSTEM_CONFIGURATION_4 0x838B /* R W  Control        CD-ROM interface controls            */

#define IO_CONFIGURATION_1 0xF388 /* R W    Control                                */
#define I_C_1_BOOT_RESET_ENABLE                                                                    \
    0x80 /* R W  Control        1=reset board on warm boot, 0=don't        */
...
#define EMULATION_CONFIGURATION                                                                    \
    0xFB8A                  /* R W            ***** Only valid on newer PAS2 cards (?) *****    */
...
#define O_M_2_PCS_ENABLED                                                                          \
    0x02                      /* R    PC speaker    PC speaker emulation 1=enabled, 0=disabled    */
...
#define I_M_PCM_BUFFER_IRQ_ENABLE                                                                  \
    0x08                         /* R W    PCM        Enable Sample Buffer interrupt            */
...
#define I_S_PCM_SAMPLE_RATE_IRQ                                                                    \
    0x04 /* R W    PCM        Sample Rate Interrupt Pending            */
#define I_S_PCM_SAMPLE_BUFFER_IRQ                                                                  \
    0x08                     /* R W    PCM        Sample Buffer Interrupt Pending            */
#define I_S_MIDI_IRQ 0x10    /* R W    MIDI        MIDI Interrupt Pending                */
#define I_S_PCM_CHANNEL 0x20 /* R W    PCM        1=right, 0=left                    */
#define I_S_RESET_ACTIVE                                                                           \
    0x40 /* R W    Control        Reset is active (Timed pulse not finished)    */
...
#define F_F_PCM_RATE_COUNTER                                                                       \
    0x40 /* R W    PCM        1=enable, 0=disable sample rate counter        */
#define F_F_PCM_BUFFER_COUNTER                                                                     \
    0x80 /* R W     PCM        1=enable, 0=disable sample buffer counter    */
...
#else
extern char
    I_C_2_PCM_DMA_translate[]; /* R W  PCM        PCM DMA channel value translations        */
extern char
    I_C_3_PCM_IRQ_translate[]; /* R W    PCM        PCM IRQ level value translation            */
extern char E_C_MPU401_IRQ_translate
    []; /* R W    MIDI        MPU401 emulation IRQ value translation        */
...
#define PARALLEL_MIXER                                                                             \
    0x078B /*   W    Mixer        Documented for MVD101 as FM Mono Right decode?? */
...
#define P_M_MV508_MIXER                                                                            \
    0x10 /*   W    Mixer        MVD508    Select a mixer (rather than a volume)     */
...
#define P_M_MV508_MASTER_A                                                                         \
    0x01 /*   W    Mixer        MVD508    Master volume control A (output)    */
#define P_M_MV508_MASTER_B                                                                         \
    0x02 /*   W    Mixer        MVD508    Master volume control B (DSP input)    */
...
#define P_M_MV508_LOUDNESS                                                                         \
    0x04 /*   W    Mixer        MVD508    Mode control - Loudness filter         */
#define P_M_MV508_ENHANCE_BITS 0x03
#define P_M_MV508_ENHANCE_NONE                                                                     \
    0x00 /*   W    Mixer        MVD508    Mode control - No stereo enhancement    */
#define P_M_MV508_ENHANCE_40                                                                       \
    0x01 /*   W    Mixer        MVD508    Mode control - 40% stereo enhancement    */
#define P_M_MV508_ENHANCE_60                                                                       \
    0x02 /*   W    Mixer        MVD508    Mode control - 60% stereo enhancement    */
#define P_M_MV508_ENHANCE_80                                                                       \
    0x03 /*   W    Mixer        MVD508    Mode control - 80% stereo enhancement    */

#define P_M_MV508_FM 0x00 /*   W    Mixer        MVD508    Channel 0 - FM                */
#define P_M_MV508_IMIXER                                                                           \
    0x01 /*   W    Mixer        MVD508    Channel 1 - Input mixer (rec monitor)    */
...
#define S_M_INTEGRATOR_ENABLE                                                                      \
    0x40 /* R W    Speaker        Enable PC speaker integrator (FORCE RealSound)    */
...
#define SAMPLE_COUNTER_CONTROL                                                                     \
    0x138B /* R W    PCM        Sample counter control register            */
#define S_C_C_SQUARE_WAVE                                                                          \
    0x04                /* R W    PCM        Square wave generator (use for sample rate)    */
#define S_C_C_RATE 0x06 /* R W    PCM        Rate generator (use for sample buffer count)    */
#define S_C_C_LSB_THEN_MSB                                                                         \
    0x30 /* R W    PCM        Change all 16 bits, LSB first, then MSB        */
...
#define SAMPLE_RATE_TIMER                                                                          \
    0x1388 /*   W    PCM        Sample rate timer register (PCM wait interval)    */
#define SAMPLE_BUFFER_COUNTER                                                                      \
    0x1389 /* R W    PCM        Sample buffer counter (DMA buffer size)        */
...
#define M_C_ENA_OUTPUT_HALF_IRQ                                                                    \
    0x10 /* R W  MIDI        Enable output FIFO half full interrupts        */
...
#define M_S_OUTPUT_HALF_EMPTY                                                                      \
    0x10 /* R W  MIDI        Midi output FIFO half empty interrupt occurred    */
