...
{
    if (dmanr <= 3)
        deb_outb(dmanr, DMA1_MASK_REG) else deb_outb(dmanr & 3, DMA2_MASK_REG);
}

static __inline__ void disable_dma(unsigned int dmanr)
{
    if (dmanr <= 3)
        deb_outb(dmanr | 4, DMA1_MASK_REG) else deb_outb((dmanr & 3) | 4, DMA2_MASK_REG);
}
...
{
    if (dmanr <= 3)
        deb_outb(0, DMA1_CLEAR_FF_REG) else deb_outb(0, DMA2_CLEAR_FF_REG);
}
...
{
    if (dmanr <= 3)
        deb_outb(mode | dmanr, DMA1_MODE_REG) else deb_outb(mode | (dmanr & 3), DMA2_MODE_REG);
}
