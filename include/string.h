 ...
 */
  ...
  * 一些内联汇编的知识
  *
  *  ​​r​​  通用寄存器         变量可存放于任意通用寄存器，如"r"(a)表示变量a存入寄存器
  *  ​​m​​  内存操作数         变量直接存储在内存中，避免寄存器加载开销，如"m"(*ptr)
  *  ​​i​​  立即数            直接使用常量值（如"i"(5)），无需额外加载指令
  *  ​​g​​  通用/内存/立即数    编译器自动选择最优存储方式（寄存器/内存/常量）
  */
...
extern inline char * strncpy(char * dest,const char *src,int count)
{
     ...
     *     jne 1b
     *     rep stosb # 这里是复制中遇到 NULL 的情况, 剩下的空间全部使用 NULL 字符填充
     * 2:
     * ------------------------ */
 ...
 * strspn 是 string span 的缩写, span 强调对连续字符序列长度的计算, 而非定位单个字符 */
extern inline int strspn(const char * cs, const char * ct)
{
     ...
     *     cld
     *
     *     # 入参列表已经设置过 %al = 0, 这里利用 repne 不断寻找, 直到找到 ~ct 的结束符
     *     # 找到后, %edi 指向这个结束符
     * 	   movl ~ct, %edi
     * 	   repne scasb      # 本步骤比较 %al 和 (%edi)
     *
     ...
     *     je 1b            # 如果是因为相等退出上一行循环, 就尝试 cs 往后移一位, 继续比较
     *     # 因此如果是因为不相等推出了, 那就实现了本函数的目的
...
extern inline int strcspn(const char * cs, const char * ct)
{
     ...
     *     jne 1b           # 如果因为不相等退出上一行 rep 循环, 跳转继续找是否还有更多不相等
     *     # JNE 说明还没有找到全部的不属于 ct 前缀字符集合, 因此要继续
     *     #     否则, 说明当前的 %al 出现在了 ct 里面, 与 strcspn 目的不符, 因此就到了终止的时候
     * 2:
     *     decl %0
     * ------------------------ */
register char * __res __asm__("si");
...
/**
 * 在 cs 中搜索​​首个出现​​在 ct 中的字符, 并返回该字符在 cs 中的指针
 * strpbrk = String Pointer Break
 * 顺便说如果 ct 中有 cs 中的字符, 这个函数实际上就是 res = cs + strspn(cs, ct) */
extern inline char * strpbrk(const char * cs,const char * ct)
{
     ...
     *     decl %esi        # RES-=1, 如果 C 出现在 ct 里面, 把 %esi 往前指, 这样 %esi 指向的
     *                      #         就是 cs 中前面的字符从来都不在 ct 里面出现的那个位置
...
extern inline char * strstr(const char * cs,const char * ct)
{
     ...
     *                      # 以第一轮循环为例, 原来的 EAX 是指向 cs[0] 的指针, ESI 是 cs[x] != ct[x] 的位置
     *                      # 因此这里交换之后 ESI 指向本次循环的 cs 初始位置, EAX 指向那个不同位置
     *     incl %esi        # 现在直接把 ESI 往后移动一位, 打算重新比较
     *     cmpb $0, -1(%eax) # 比较一下 EAX 指向的前个位置是不是 NULL, 是的话就是没有在 cs 发现 ct 子串, 程序结束
     *     jne 1b
     *     xorl %eax, %eax
     * 2:
     * ------------------------ */
 ...
 *
 * 将 s 看作是包含零个或多个 token 的序列, 并由分割符字符串 ct 中的一个或多个字符分开
 * 第一次调用 strtok 时, 将返回指向 s 中第 1 个 token 首字符的指针, 并在返回 token 时
 * 将 null 字符写到分割符处, 后续使用 null 作为 s 的调用, 将用这种方法继续扫描 s,
 * 直到没有 token 为止. 在不同的调用过程中, 分割符串 ct 可以不同
 *
...
extern inline char * strtok(char * s,const char * ct)
{
     ...
     *     movl %ebx, %esi     # s == NULL && __strtok != NULL, 这说明 strtok 不是第一次调用
     *                         # 这种情况就把之前保存的 __strtok 赋值给 %esi
     *
     ...
     *     lodsb               # %(esi) -> %al
     *     testb %al, %al
     *     je 7f               # [(%al & %al) == 0], 已经扫描到末尾, 则跳转到标号 7 结束, NULL Token
     *     movl CT, %edi
     *     movl %edx, %ecx
     *     repne scasb         # 注意这里 %al 还在字符串的开头位置
     *     je 2b               # 如果 s[x] 等于 ct 里面的某个字符, 这些字符处于开头位置
     *                         # 拿到的实际上是一个空 token, 丢弃, 重新找非空的 token
     *
     *     # 到这里我们的 s 最少能提供一个字符的非空 token
     *     decl %esi           # 指向这个 token 的字符, 也就是 s[x-1]
     *     cmpb $0, (%esi)     # 判断 s[x-1] 是否等于 NULL, 如果等于 NULL 跳转到标号 7 结束, NULL Token
     *     je 7f
     *     movl %esi, %ebx     # 保存 s[x-1] 位置到 __res
     *
     ...
     *     decl %esi       # 尤其要注意, 上面说的 s[y] 是通过 lodsb 加载到 %al 里面的
     *                     # lodsb 之后, %esi 会指向 y+1, 于是这里需要 %esi-1, 才能让 %esi 指向分隔符的位置
     *     cmpb $0, (%esi)
     *     je 5f           # 分隔符 == NULL 说明已经扫描结束, 跳转到标号 5, 计算最终结果
     *     movb $0, (%esi) # 把分隔符替换为 NULL - strtok 就可以愉快的返回那个 NULL 结尾的 token 了
     ...
     * 6:
     *     cmpb $0, (%ebx) # %ebx 指向的是 Token 开始位置, 因此这里在检查 Token 是不是 NULL Token
     *     jne 7f          # Token != NULL ?
     *     xorl %ebx, %ebx # res = NULL
     *
