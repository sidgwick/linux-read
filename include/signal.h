 ...
 * sa_handler 是对应某信号指定要采取的行动, 可以用上面的 SIG_DFL/SIG_IGN 来忽略该信号,
 *            也可以是指向处理该信号函数的一个指针
...
/**
 * 在进程的任务结构中, 除有一个以比特位表示当前进程待处理的 32bits 信号字段 signal 以外,
 * 还有一个同样以 32bits 比特位表示的用于屏蔽进程当前阻塞信号集(屏蔽信号集)的字段 blocked,
 * 每个比特代表一个对应的阻塞信号. 修改进程的屏蔽信号集可以阻塞或解除阻塞所指定的信号.
 *
 * 以下五个函数就是用于操作进程 **屏蔽信号集**, 虽然简单实现起来很简单, 但本版本内核中还未实现
 *
 * 函数 sigaddset 和 sigdelset 用于对信号集中的信号进行增/删/修改.
 * sigaddset 用于向 mask 指向的信号集中增加指定的信号 signo, sigdelset 则反之
 *
 * 函数 sigemptyset 和 sigfillset 用于初始化进程屏蔽信号集. 每个程序在使用信号集前,
 * 都需要使用这两个函数之一对屏蔽信号集进行初始化
 *
 ...
 * 返回进程当前屏蔽信号集. 若 set 指针不是 NULL, 则根据 how 指示修改进程屏蔽信号集 */
int sigprocmask(int how, sigset_t *set, sigset_t *oldset);

/**
 * 用 sigmask 临时替换进程的信号屏蔽码, 然后暂停该进程直到收到一个信号. 若捕捉到某一
 * 信号并从该信号处理程序中返回, 则该函数也返回, 并且信号屏蔽码会恢复到调用调用前的值 */
