...
static __inline__ unsigned long int __ntohl(unsigned long int x)
{
    __asm__("xchgb %b0,%h0\n\t" /* swap lower bytes    */
            "rorl $16,%0\n\t"   /* swap words        */
...
static __inline__ unsigned long int __constant_ntohl(unsigned long int x)
{
    return (((x & 0x000000ff) << 24) | ((x & 0x0000ff00) << 8) | ((x & 0x00ff0000) >> 8) |
            ((x & 0xff000000) >> 24));
}

static __inline__ unsigned short int __ntohs(unsigned short int x)
{
...
static __inline__ unsigned short int __constant_ntohs(unsigned short int x)
{
...
#ifdef __OPTIMIZE__
#define ntohl(x) (__builtin_constant_p((x)) ? __constant_ntohl((x)) : __ntohl((x)))
#define ntohs(x) (__builtin_constant_p((x)) ? __constant_ntohs((x)) : __ntohs((x)))
#define htonl(x) (__builtin_constant_p((x)) ? __constant_htonl((x)) : __htonl((x)))
