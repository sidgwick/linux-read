...
// 定义1页内存页面字节数。注意高速缓冲块长度是1024字节
#define PAGE_SIZE 4096

#include <linux/kernel.h>
#include <signal.h>

// 内存页面交换设备号。定义在mm/memory.c文件中
extern int SWAP_DEV;

// 从交换设备读入和写出被交换内存页面. ll_rw_page 定义在 blk_drv/ll_rw_block.c
// 文件中 参数 nr 是主内存区中页面号; buffer 是读/写缓冲区
...
// 这里使用重新加载页目录基址寄存器 cr3 的方法来进行刷新, 下面 eax = 0,
// 是页目录的基址 还有个更精细的 invlpg 指令, 可以失效特定的 TLB 记录, Linux
// 没有使用
#define invalidate() __asm__("movl %%eax,%%cr3" ::"a"(0))

/* these are not to be changed without changing head.s etc
 * 下面定义若需要改动, 则需要与 head.s 等文件中的相关信息一起改变
 * Linux 0.12 内核默认支持的最大内存容量是 16MB,
 * 可以修改这些定义以适合更多的内存 */
...
#define MAP_NR(addr)                                                           \
    (((addr) - LOW_MEM) >> 12) // 给定 addr, 计算 addr 所属的页面编号
#define USED 100               // 页面被占用标志，参见449行

// 内存映射字节图(1 字节代表 1 页内存),
// 每个页面对应的字节用于标志页面当前被引用(占用)次数 它最大可以映射 15Mb
// 的内存空间. 在初始化函数 mem_init 中, 对于不
