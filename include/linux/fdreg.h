...
#define FD_DOR                                                                 \
    0x3f2 /* Digital Output Register, 数字输出寄存器(也称为数字控制寄存器) */
#define FD_DIR 0x3f7 /* Digital Input Register (read), 数字输入寄存器 */
#define FD_DCR                                                                 \
    0x3f7 /* Diskette Control Register (write), 数据传输率控制寄存器 */

/* Bits of main status register, 主状态寄存器各比特位的含义 */
#define STATUS_BUSYMASK                                                        \
    0x0F /* drive busy mask, 驱动器忙位(每位对应一个驱动器)*/
#define STATUS_BUSY 0x10 /* FDC busy, 软盘控制器忙 */
#define STATUS_DMA                                                             \
    0x20 /* 0- DMA mode, 0 - 为DMA数据传输模式，1 - 为非DMA模式 */
...
#define FD_READ                                                                \
    0xE6               /* read with MT, MFM, SKip deleted,                     \
...
#define FD_SPECIFY                                                             \
    0x03 /* specify HUT etc, 设定驱动器参数（步进速率、磁头卸载时间等） */

/* DMA commands */
#define DMA_READ 0x46  // DMA读盘，DMA方式字（送DMA端口12，11）
#define DMA_WRITE 0x4A // DMA写盘，DMA方式字
