 ...
 * 下面是有关软盘控制器一些端口和符号的定义, 这里涉及到了 <System & Control> 这本书 */
...
#define STATUS_DMA	0x20		/* 0- DMA mode, 0 - 为DMA数据传输模式，1 - 为非DMA模式 */
...
#define ST0_INTR	0xC0		/* Interrupt code mask, 中断代码位（中断原因），00 - 命令正常结束, 01 - 命令异常结束；10 - 命令无效；11 - FDD就绪状态改变 */
...
#define FD_READ		0xE6		/* read with MT, MFM, SKip deleted, 读数据（MT多磁道操作，MFM格式，跳过删除数据） */
