 ...
 * 下面是有关软盘控制器一些端口和符号的定义, 这里涉及到了 <System & Control>
 * 这本书 */
#define FD_STATUS 0x3f4 /* 主状态寄存器端口 */
#define FD_DATA 0x3f5   /* 数据端口 */
#define FD_DOR 0x3f2    /* Digital Output Register, 数字输出寄存器 */
#define FD_DIR 0x3f7    /* Digital Input Register, 数字输入寄存器 */
#define FD_DCR 0x3f7    /* Diskette Control Register, 数据传输率控制寄存器 */

/* Bits of main status register, 主状态寄存器各比特位的含义 */
#define STATUS_BUSYMASK 0x0F /* drive busy mask, 每位对应一个驱动器 */
#define STATUS_BUSY 0x10     /* FDC busy, 软盘控制器忙 */
#define STATUS_DMA 0x20      /* 0: DMA mode, 1: 非DMA模式 */
...
#define ST0_DS 0x03  /* drive select mask, 驱动器选择号(发生中断时驱动器) */
...
/* Interrupt code mask, 中断代码位.
 * 00-命令正常结束, 01-命令异常结束, 10-命令无效, 11-FDD就绪状态改变 */
#define ST0_INTR 0xC0

/* Bits of FD_ST1, 状态字节1（ST1）各比特位的含义 */
#define ST1_MAM 0x01 /* Missing Address Mark, 未找到地址标志(ID AM) */
...
#define ST1_CRC 0x20 /* CRC error in data or addr, CRC检验出错 */
#define ST1_EOC 0x80 /* End Of Cylinder, 访问超过一个磁道上的最大扇区号 */
...
#define ST2_WC 0x10  /* Wrong Cylinder, 磁道（柱面）号不符 */
#define ST2_CRC 0x20 /* CRC error in data field, 数据场CRC校验错 */
...
#define FD_READ 0xE6        /* 读数据(MT多磁道操作, MFM格式, 跳过删除数据) */
...
/* DMA commands */
#define DMA_READ 0x46  // DMA 读盘方式字(送 DMA 端口 12, 11)
#define DMA_WRITE 0x4A // DMA 写盘方式字
