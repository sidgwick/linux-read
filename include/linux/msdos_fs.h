...
#define IS_FREE(n)                                                                                 \
    (!*(n) || *(unsigned char *)(n) == DELETED_FLAG || *(unsigned char *)(n) == FD_FILL_BYTE)
...
#define MSDOS_CAN_BMAP(mib) (!(((mib)->cluster_size & 1) || ((mib)->data_start & 1)))
...
extern int msdos_lookup(struct inode *dir, const char *name, int len, struct inode **result);
...
extern struct super_block *msdos_read_super(struct super_block *s, void *data, int);
