...
// CPU产生异常中断 `int 7` 时在栈中分布的数据构成的结构,
// 与系统调用时内核栈中数据分布类似
struct info {
    ...
    long ___esi;
    long ___ebp;
    long ___sys_call_ret; // 中断7 返回时将去执行系统调用的返回处理代码
    ...
    long ___es;
    long ___ds;
    long ___eip; // ___eip ~ ___ss 行 由CPU 自动入栈
    long ___cs;
    long ___eflags;
    long ___esp;
    long ___ss;
 ...
 * 因此下面的宏定义, 其主要目的就是利用 __math_abort,
 * 让它即可用作普通有返回函数, 又可以在使用宏定义 math_abort 时用作不返回的函数
 */
void __math_abort(struct info *, unsigned int);

#define math_abort(x, y) (((volatile void (*)(struct info *, unsigned int))__math_abort)((x), (y)))
...
typedef struct {
    long a,
        b;          // 共 64 比特尾数. 其中 a 为低 32 位, b 为高 32 位(包括 1 位固定位)
    short exponent; // 指数值
} temp_real;
...
struct swd {
    ...
    int ir : 1; // ir, b: 若上面6位任何未屏蔽异常发生，则置位
    int c0 : 1; // c0--c3: 条件码比特位
    int c1 : 1;
    int c2 : 1;
    int top : 3; // 指示80387中当前位于栈顶的80位寄存器
    int c3 : 1;
    int b : 1;
};

// 80387内部寄存器控制方式常量
#define I387 (current->tss.i387)        // 进程的80387状态信息。参见sched.h文件
#define SWD (*(struct swd *)&I387.swd)  // 80387中状态控制字
#define ROUNDING ((I387.cwd >> 10) & 3) // 取控制字中舍入控制方式
#define PRECISION ((I387.cwd >> 8) & 3) // 取控制字中精度控制方式

// 定义精度有效位常量
#define BITS24 0 // 精度有效数：24位。（参见图11-6)
#define BITS53 2 // 精度有效数：53位
#define BITS64 3 // 精度有效数：64位

// 定义舍入方式常量
#define ROUND_NEAREST 0 // 舍入方式：舍入到最近或偶数
#define ROUND_DOWN 1    // 舍入方式：趋向负无限
#define ROUND_UP 2      // 舍入方式：趋向正无限
#define ROUND_0 3       // 舍入方式：趋向截0

// 常数定义
#define CONSTZ (temp_real_unaligned){0x0000, 0x0000, 0x0000, 0x0000, 0x0000}   // 0
#define CONST1 (temp_real_unaligned){0x0000, 0x0000, 0x0000, 0x8000, 0x3FFF}   // 1.0
