...
#define INODES_PER_BLOCK                                                       \
    ((BLOCK_SIZE) / (sizeof(struct d_inode))) // 每个逻辑块可存放的i节点数
#define DIR_ENTRIES_PER_BLOCK                                                  \
    ((BLOCK_SIZE) / (sizeof(struct dir_entry))) // 每个逻辑块可存放的目录项数
...
#define PIPE_SIZE(inode)                                                       \
    ((PIPE_HEAD(inode) - PIPE_TAIL(inode)) & (PAGE_SIZE - 1))
...
struct d_inode {
    ...
    unsigned short i_zone[9]; // 直接(0-6)、间接(7)或双重间接(8)逻辑块号, zone
                              // 可译成区段，或逻辑块
};
...
struct super_block {
    ...
    struct buffer_head
        *s_imap[8]; // i节点位图缓冲块指针数组(占用8块，可表示64M)
