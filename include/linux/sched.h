...
struct task_struct {
    ...
    struct task_struct
        *p_ysptr; // Younger Sibling, 指向比自己后创建的相邻进程的指针
    struct task_struct
        *p_osptr; // Older Sibling, 指向比自己早创建的相邻进程的指针
...
#define CURRENT_TIME                                                           \
    (startup_time + (jiffies + jiffies_offset) / HZ) // 当前时间(秒数)
...
#define _TSS(n)                                                                \
    ((((unsigned long)n) << 4) +                                               \
     (FIRST_TSS_ENTRY << 3)) // 第 n 个任务的 TSS 选择子
#define _LDT(n)                                                                \
    ((((unsigned long)n) << 4) +                                               \
     (FIRST_LDT_ENTRY << 3)) // 第 n 个任务的 LDT 选择子
#define ltr(n)                                                                 \
