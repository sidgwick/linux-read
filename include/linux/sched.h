...
#define CT_TO_USECS(x)	(((x) % HZ) * 1000000/HZ) /* 滴答数对应的不足 1s 的毫秒数. 取余是求出不足 1s 的滴答数, 把它乘以 1_000_000 / HZ 得到对应的毫秒数  */
...
/* various fields */
	...
	long session; // 会话号
	long leader; // 会话首领
	int	groups[NGROUPS]; /* 进程所属组号. 一个进程可属于多个组. 虽然是 int 类型, 但是 group id 都是 short 的 */
	...
	unsigned long alarm; // 报警定时值(滴答数) ~ 无须担心溢出, 最多可以表示 58494241735 年
    ...
    unsigned int flags;	/* per process flags, defined below */ // 各进程的标志, 在下面第149行开始定义(还未使用)
...
p_dpl_s_type = fa = 1111_1010 = 存在, dpl=3, s=1, XCRA=1010 可执行非依从可读代码段
p_dpl_s_type = f2 = 1111_0010 = 存在, dpl=3, s=1, XERA=0010 不可执行正向生长可写数据段
*/
 ...
 * 第4个独立的段中但后来并没有那样做, 于是就一直把GDT表中第4个描述符项(上面syscall项)闲置在一旁
 *
 ...
 *
 * TODO: 研究一下为啥任务切换回来之后, %ecx 里面是什么, 以及它和 last_task_used_math 的关系
 *
...
// 更新位于地址 addr 处描述符的 limit 字段, 注意 limit 在高位字节部分是 (BBRL-RRBB)
// 假如 limit = %edx = 000F_CDAB
