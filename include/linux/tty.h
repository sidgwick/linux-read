...
#define ASYNC_HUP_NOTIFY                                                                           \
    0x0001                         /* Notify getty on hangups and closes
                   on the callout port */
...
struct tty_ldisc {
    ...
    int (*write)(struct tty_struct *tty, struct file *file, unsigned char *buf, unsigned int nr);
    int (*ioctl)(struct tty_struct *tty, struct file *file, unsigned int cmd, unsigned long arg);
    int (*select)(struct tty_struct *tty, struct inode *inode, struct file *file, int sel_type,
