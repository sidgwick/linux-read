...
2. 开始往里面让字符, 比如放了 A/B/C 三个字符, 此时内存变为
    ...
    把这个式子看成 SIZE 进制的处理, 那么如果做 `&(SIZE-1)` 相当于我们只有一个 SIZE 进制数位可以用,
    多出来的数位都要丢掉, 于是上面的式子里面 (SIZE-1) + 1  + (X/SIZE) 因为都需要用多出来的数位表示,
    他们都被丢掉, 于是这种情况下出现了 (SIZE-1) + X = (X%SIZE) - 1, 进一步得到 SIZE + X = (X % SIZE)
...
3. 如果队列因为读写操作, 变成了下面这样
    ...
    如此, 求队列空闲字符数, 就是 R = tail - head - 1, 在数值山与 `(tail - head - 1) & (SIZE-1)` 也是一样的
*/

// a 是指向缓冲区的索引指针, INC 把索引往后移动一字节, 若已超出缓冲区右侧, 则指针循环
...
// 缓冲区还可存放字符的长度(空闲区长度). 这里的 -1 可能是 buf 故意不放满, 以免区别不出来真满和空状态
