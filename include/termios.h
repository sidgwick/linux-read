...
// Terminal Control GET Settings	获取终端设置(termios结构体, 参见 tcgetattr)
#define TCGETS		0x5401

// Terminal Control SET Settings	立即设置终端设置(参见 tcsetattr - TCSANOW 选项)
#define TCSETS		0x5402

// Terminal Control SET Settings Wait	在设置终端termios的信息之前，需要先等待输出队列中所有数据处理完(耗尽)。对于修改参数会影响输出的情况，就需要使用这种形式(参见tcsetattr()，TCSADRAIN选项)。
#define TCSETSW		0x5403

// Terminal Control SET Settings Flush	在设置 termios 的信息之前, 需要先等待输出队列中所有数据处理完, 并且刷新(清空)输入队列再设置(参见tcsetattr()，TCSAFLUSH选项)
#define TCSETSF		0x5404

// Terminal Control GET Attributes	获取终端属性(旧版 termio结构体, 参见tcgetattr)
#define TCGETA		0x5405


// Terminal Control SET Attributes	立即设置终端属性(参见tcsetattr()，TCSANOW选项)
#define TCSETA		0x5406

// Terminal Control SET Attributes Wait	在设置终端termio的信息之前，需要先等待输出队列中所有数据处理完(耗尽)。对于修改参数会影响输出的情况，就需要使用这种形式(参见tcsetattr()，TCSADRAIN选项)。
#define TCSETAW		0x5407


// Terminal Control SET Attributes Flush	设置终端属性，在设置termio的信息之前，需要先等待输出队列中所有数据处理完，并且刷新(清空)输入队列。 再设置(参见tcsetattr()，TCSAFLUSH选项)。
#define TCSETAF		0x5408

// Terminal Control Send Break	等待输出队列处理完毕(空)，若参数值是0，则发送一个break(参见tcsendbreak()，tcdrain())。
#define TCSBRK		0x5409

// Terminal Control XON/XOFF Control	开始/停止控制。如果参数值是0，则挂起输出；如果是1，则重新开启挂起的输出；如果是2，则挂起输入；如果是3，则重新开启挂起的输入(参见tcflow())。
#define TCXONC		0x540A

// Terminal Control FLUSH	刷新已写输出但还没发送或已收但还没有读数据。如果参数是0，则刷新(清空)输入队列；如果是1，则刷新输出队列；如果是2，则刷新输入和输出队列(参见tcflush())。
#define TCFLSH		0x540B

// Terminal IO Control EXCLusive	设置终端串行线路专用模式(独占模式)
#define TIOCEXCL	0x540C

// Terminal IO Control Not EXCLusive	复位终端串行线路专用模式(取消终端独占模式)
#define TIOCNXCL	0x540D

// Terminal IO Control Set ConTro1 TTY	设置控制终端(使当前进程成为终端的控制进程)
#define TIOCSCTTY	0x540E

// Terminal IO Control Get Process GRouP	获取前台进程组 ID, 读取指定终端设备进程的组id，参见tcgetpgrp()
#define TIOCGPGRP	0x540F

// Terminal IO Control Set Process GRouP	设置前台进程组 ID, 设置指定终端设备进程的组id(参见tcsetpgrp())
#define TIOCSPGRP	0x5410

// Terminal IO Control OUTput Queue	返回输出队列中还未送出的字符数
#define TIOCOUTQ	0x5411

// Terminal IO Control Simulate Terminal Input	模拟终端输入(将数据插入输入队列), 该命令以一个指向字符的指针作为参数，并假装该字符是在终端上键入的。用户必须在该控制终端上具有超级用户权限或具有读许可权限
...
// Terminal IO Control Modem SET	设置调制解调器状态位(覆盖全部, 如果某一比特位置位, 则modem对应的状态引线将置为有效)
#define TIOCMSET	0x5418

// Terminal IO Control Get Software Carrier	获取软件载波检测(Software Carrier Detect)设置(已废弃), (1 - 开启；0 - 关闭)。对于本地连接的终端或其他设备，软件载波标志是开启的，对于使用modem线路的终端或设备则是关闭的。为了能使用这两个 ioctl调用，tty线路应该是以 O_NDELAY 方式打开的，这样open()就不会等待载波
#define TIOCGSOFTCAR	0x5419

// Terminal IO Control Set Software Carrier	设置软件载波检测(已废弃), (1 - 开启；0 - 关闭)
...
#define ECHOCTL	0001000 // 若设置了ECHO，则除TAB、NL、START和STOP以外的ASCII控制信号将被回显成象^X式样，X值是控制符+0x40
