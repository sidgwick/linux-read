...
/* 系统调用以及很多库函数返回一个特殊的值以表示操作失败或出错, 这个值通常选择 -1
 * 或者 其他一些特定的值来表示. 但是这个返回值仅说明错误发生了.
 * 如果需要知道出错的类型, 就需要查看表示系统出错号的变量 errno, 该变量即在
 * errno.h 文件中声明 在程序开始执行时该变量值被初始化为 0 */
extern int errno;
