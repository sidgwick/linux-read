...
#define _set_seg_desc(gate_addr, type, dpl, base, limit)                        \
    {                                                                           \
        ...
        *((gate_addr) + 1) =                                                    \
            (((base) & 0x0000ffff) << 16) | ((limit) & 0x0ffff);                \
    }

// 在全局表中设置任务状态段/局部表描述符. 状态段和局部表描述符都被设置为 104
// 字节 TSS 描述符或者 LDT 描述符, 他们的组成如下: BBRL-RRBB BBBB-LLLL
// TODO-DONE: D/B 字段为啥没有置 1 呢?
///     可能是这里只考虑设置 TSS 或者 LDT 描述符, D/B 字段和这两个都没啥关系,
///     D/B 字段控制的是
//          1. 代码段: 操作数位数和有效地址长度
//          1. 数据段: 栈操作的长度和栈的边界
