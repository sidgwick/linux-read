// 转到用户空间执行
// 参考 X86 汇编语言 -- 346 页: 当切换特权级别的中断发生的时候, 会依次入栈 ss, sp, eflags, cs, ip
// 下面几条 push 指令就是在手动构造中断出现时候栈状态
...
__asm__ ("movw %%dx,%%ax\n\t" /* EAX = 0x0008_OOOO, SSSS=0x0008 = 1# 描述符, GDT, dpl=0 */ \
	"movw %0,%%dx\n\t" /* EDX = OOOO_ABCD */ \
	"movl %%eax,%1\n\t" /* 装到 gate_addr */ \
...
// 在全局表中设置任务状态段/局部表描述符. 状态段和局部表描述符都被设置为 104 字节
// TSS 描述符或者 LDT 描述符, 他们的组成如下:
// BBRL-RRBB
// BBBB-LLLL
// TODO: D/B 字段为啥没有置 1 呢?
