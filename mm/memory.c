...
// 物理内存映射字节图(1字节代表1页内存)
// 每个页面对应的字节用于标志页面当前被引用(占用)次数, 使用 PAGING_PAGES 最大可以映射 15Mb 的内存空间
// 在初始化函数 mem_init 中, 对于不能用作主内存区页面的位置均都预先被设置成 USED
...
void free_page(unsigned long addr)
{
    ...
    // 如果物理地址 addr 大于等于系统所含物理内存最高端, 则显示出错信息并且内核停止工作
	if (addr >= HIGH_MEMORY)
...
int free_page_tables(unsigned long from,unsigned long size)
{
    ...
    /* 因为每个页表最多映射 4M 的内存, 所以 from 表示的线性基地址, 必然是 4M 对齐的 */
	if (from & 0x3fffff)
    ...
    /* 因为 page directory 在内存地址 0 处, 这里直接算出 from 所属的页目录项(高 10 位)
     * 然后从此页目录项开始遍历页面簇
     * 没有 `>> 20` 而不是 22 是因为还要考虑每个 PDE 占据 4 字节 */
	...
	for( ; size-->0 ; from_dir++,to_dir++) {
        ...
        *to_dir = ((unsigned long) to_page_table) | 7;
		...
		for ( ; nr-- > 0 ; from_page_table++,to_page_table++) {
            ...
            /* 页面清理掉写标记, 主要是如果有页面是公用的, 就可以实现 copy-on-write 操作 */
 ...
 * 把物理内存页面映射到线性地址空间指定处
 *
 * 参数 page 是分配的主内存区中某一页面(页帧/页框)的指针, address 是对应的线性地址 */
 ...
 * 函数参数 error_code 和 address 是进程在写写保护页面时由 CPU产生异常而自动生成的
 *
...
void do_wp_page(unsigned long error_code,unsigned long address)
{
    ...
    // 如果 address 小于 TASK_SIZE=0x4000000(即64MB), 表示异常页面位置在内核或任务 0
    // 和任务 1 所处的线性地址范围内, 于是发出警告信息“内核范围内存被写保护”
	if (address < TASK_SIZE)
		printk("\n\rBAD! KERNEL MEMORY WP-ERR!\n\r");

    // 如果 (address–当前进程代码起始地址) 大于一个进程的长度(64MB), 表示 address
    // 所指的线性地址不在引起异常的进程线性地址空间范围内, 则在发出出错信息后退出
    // TODO: 在了解一下进程内存分布
 ...
 * 如果 p 进程 address 处的页面存在并且没有被修改过的话, 就让当前进程与 p 进程共享之.
...
static int try_to_share(unsigned long address, struct task_struct * p)
{
     ...
     * 下面用 start_code 在计算一次 PDE, 这次算出来的是进程空间在 4GB 空间上的偏移
     * 因此两者相加, 就是 address 在 4GB 空间上的页表项
     *
     * TODO: 确认这部分分析 */
 ...
 * 的 executable 字段(或 library 字段), 该字段指向进程正在执行程序(或使用的库文件)
 * 在内存中的 i 节点. 根据该 i 节点的引用次数 i_count 我们可以进行这种判断
 ...
 *      address 是进程中的逻辑地址, 即是当前进程欲与 p 进程共享页面的逻辑页面地址
 * 返回: 1-共享操作成功, 0-失败 */
static int share_page(struct m_inode * inode, unsigned long address)
{
    ...
    /* 遍历进程列表, 找找是那个进程和当前进程共用了 inode, 然后试着共享这个进程的页面 */
...
void do_no_page(unsigned long error_code,unsigned long address)
{
     ...
     * 逻辑块号(保存在 nr[] 数组中), 利用 bread_page 即可把这 4 个逻辑块读入到
     * 物理页面 page 中 */
     ...
     * 文件尾不到 1 个页面的长度, 因此可能读入一些无用的信息. 下面的操作就是把这部分超
     * 出执行文件 end_data 以后的部分进行清零处理.
     * 当然, 若该页面离末端超过 1 页, 说明不是从执行文件映像中读取的页面, 而是从库文件中读取的
     * 因此不用执行清零操作
     *
     * tmp 表示从进程代码段起始地址到发生缺页地址的偏移量, current->end_data 则是
     * 进程数据段的结束地址, 这里加 4096 是因为一个内存页的大小是 4096 字节,
     * 所以 `tmp + 4096` 计算的是从缺页地址开始的整个页面的结束位置
     * TODO: 没看懂这里和 4095 比较是啥意思, 先接受结果: i 算出来是多读进来的数据
     */
...
void show_mem(void)
{
	...
	printk("Mem-info:\n\r");
	for(i=0 ; i<PAGING_PAGES ; i++) {
		if (mem_map[i] == USED) /* 参看 mem_init, 如果内存不属于主存储区, 它们的标记就是 USED */
			continue;
	...
	for(i=4 ; i<1024 ;) {
		...
		if (1&pg_dir[i]) {
             ...
             * 则把一个进程占用的物理内存页统计值 k 增 1. 把系统占用的所有物理内存页
             * 统计值 free 增 1. 然后取对应页表地址 pg_tbl, 并对该页表中所有页表项进行统计.
             *
         ...
         *
         * 当 i <= 15 的时候, (i&15) 恒真, 当 16 < i <= 31 的时候, (i&15) 恒真, 此后的数据类似
         * 因此 (!(i&15)) 意思就是每次 i 是 16 的倍数的时候, 就真一次
