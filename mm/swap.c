...
#define bitop(name, op)                                                                   \
    ...
    {                                                                                     \
        ...
        __asm__ __volatile__(                                                             \
            "bt" op                                                                       \
            " %1,%2; adcl $0,%0" /* adc 用于将 CR 标记体现到最终结果里面 */ \
            : "=g"(__res)                                                                 \
            : "r"(nr), "m"(*(addr)), "0"(0));                                             \
        return __res;                                                                     \
    }
...
bitop(bit, "")         // bt
    ...
    /* swap_bitmap 是管理 SWAP 空间的位图, 相关的位置 1, 说明对应的页空间可用, 0
       表示不可用 */
...
int swap_out(void)
{
    static int dir_entry =
        FIRST_VM_PAGE >> 10; /* 即任务 1 的第 1 个 PDT 索引 */
    ...
    while (counter > 0) {
        ...
        counter -=
            1024; /* 一个 PDT 已经检查完, 相当于一次性查找了 1024 个 PTE */
        dir_entry++;
