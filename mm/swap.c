...
{ \
int __res; \
__asm__ __volatile__("bt" op " %1,%2; adcl $0,%0" /* adc 用于将 CR 标记体现到最终结果里面 */ \
:"=g" (__res) \
:"r" (nr),"m" (*(addr)),"0" (0)); \
return __res; \
}
 ...
 * 中读入, 可以直接释放掉相应物理页面了事
 * 否则就申请一个交换页面号, 然后把页面交换出去. 此时交换页面号要保存在对应页表项中,
 * 并且仍需要保持页表项存在位 P = 0
 *
...
int swap_out(void)
{
    ...
    /* 在取得二级页表页地址之后, 针对该页表页中的所有 1024 个页面, 逐一调用交换函数
...
void init_swapping(void)
{
     ...
     * TODO: blk_size 在磁盘或者软盘程序里面设置好的, 这里先用, 了解完块设备再回来看 */
