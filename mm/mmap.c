...
static int anon_map(struct inode *, struct file *, unsigned long, size_t, int, unsigned long);
/*
...
#define CODE_SPACE(addr) (PAGE_ALIGN(addr) < current->start_code + current->end_code)

int do_mmap(struct file *file, unsigned long addr, unsigned long len, unsigned long prot,
...
asmlinkage int sys_mmap(unsigned long *buffer)
{
    ...
    return do_mmap(file, get_fs_long(buffer), get_fs_long(buffer + 1), get_fs_long(buffer + 2),
                   flags, get_fs_long(buffer + 5));
}
...
void unmap_fixup(struct vm_area_struct *area, unsigned long addr, size_t len)
{
    ...
    if (addr < area->vm_start || addr >= area->vm_end || end <= area->vm_start ||
        end > area->vm_end || end < addr) {
        printk("unmap_fixup: area=%lx-%lx, unmap %lx-%lx!!\n", area->vm_start, area->vm_end, addr,
               end);
        return;
    }
...
int generic_mmap(struct inode *inode, struct file *file, unsigned long addr, size_t len, int prot,
                 unsigned long off)
{
    struct vm_area_struct *mpnt;
    ...
    for (mpnt = t->mmap; mpnt != NULL; mpnt = mpnt->vm_next) {
        ...
        if ((vmp->vm_start >= mpnt->vm_start && vmp->vm_start < mpnt->vm_end) ||
            ...
            printk("insert_vm_struct: ins area %lx-%lx in area %lx-%lx\n", vmp->vm_start,
                   vmp->vm_end, mpnt->vm_start, vmp->vm_end);
    }
...
void merge_segments(struct vm_area_struct *mpnt, map_mergep_fnp mergep, void *mpd)
{
    ...
    for (prev = mpnt, mpnt = mpnt->vm_next; mpnt != NULL; prev = mpnt, mpnt = next) {
        int mp;
         ...
         */
        if (prev->vm_ops != mpnt->vm_ops || prev->vm_page_prot != mpnt->vm_page_prot ||
            prev->vm_inode != mpnt->vm_inode || prev->vm_end != mpnt->vm_start || !mp ||
 ...
 */
static int anon_map(struct inode *ino, struct file *file, unsigned long addr, size_t len, int mask,
                    unsigned long off)
{
    struct vm_area_struct *mpnt;
...
int ignoff_mergep(const struct vm_area_struct *m1, const struct vm_area_struct *m2, void *data)
{
