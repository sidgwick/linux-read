 ...
 */
static int ext2_match(int len, const char *const name, struct ext2_dir_entry *de)
{
    unsigned char same;
     ...
     */
    if (!len && de->name_len == 1 && (de->name[0] == '.') && (de->name[1] == '\0'))
        return 1;
    if (len != de->name_len)
        return 0;
    __asm__("cld\n\t"
 ...
 */
static struct buffer_head *ext2_find_entry(struct inode *dir, const char *const name, int namelen,
                                           struct ext2_dir_entry **res_dir)
{
    ...
    *res_dir = NULL;
    ...
    while (offset < dir->i_size) {
        ...
        if (!bh)
            ext2_panic(sb, "ext2_find_entry", "buffer head pointer is NULL");
        wait_on_buffer(bh);
        if (!bh->b_uptodate) {
            /*
        ...
        brelse(bh);
        if (((block + NAMEI_RA_SIZE) << EXT2_BLOCK_SIZE_BITS(sb)) >= dir->i_size)
            bh = NULL;
        else
...
int ext2_lookup(struct inode *dir, const char *name, int len, struct inode **result)
{
    ...
    *result = NULL;
    ...
    if (!(ino = ext2_dcache_lookup(dir->i_dev, dir->i_ino, name, len))) {
        ...
        ino = de->inode;
#ifndef DONT_USE_DCACHE
        ext2_dcache_add(dir->i_dev, dir->i_ino, de->name, de->name_len, ino);
#endif
        brelse(bh);
#ifndef DONT_USE_DCACHE
    }
 ...
 */
static struct buffer_head *ext2_add_entry(struct inode *dir, const char *name, int namelen,
                                          struct ext2_dir_entry **res_dir, int *err)
{
    unsigned long offset;
    ...
    *err = -ENOSPC;
    while (1) {
        ...
        if (!ext2_check_dir_entry("ext2_add_entry", dir, de, bh, offset)) {
            *err = -ENOENT;
            brelse(bh);
            return NULL;
        }
...
static int ext2_delete_entry(struct ext2_dir_entry *dir, struct buffer_head *bh)
{
    ...
    de = (struct ext2_dir_entry *)bh->b_data;
    while (i < bh->b_size) {
        if (!ext2_check_dir_entry("ext2_delete_entry", NULL, de, bh, i))
            return -EIO;
        if (de == dir) {
            if (pde)
...
#ifndef DONT_USE_DCACHE
    ext2_dcache_add(dir->i_dev, dir->i_ino, de->name, de->name_len, de->inode);
#endif
    bh->b_dirt = 1;
    if (IS_SYNC(dir)) {
...
static int empty_dir(struct inode *inode)
{
    ...
    if (inode->i_size < EXT2_DIR_REC_LEN(1) + EXT2_DIR_REC_LEN(2) ||
        ...
        ext2_warning(inode->i_sb, "empty_dir", "bad directory (dir %lu)", inode->i_ino);
        return 1;
    }
    de = (struct ext2_dir_entry *)bh->b_data;
    de1 = (struct ext2_dir_entry *)((char *)de + de->rec_len);
    if (de->inode != inode->i_ino || !de1->inode || strcmp(".", de->name) ||
        strcmp("..", de1->name)) {
        ext2_warning(inode->i_sb, "empty_dir", "bad directory (dir %lu)", inode->i_ino);
        return 1;
    }
    ...
    while (offset < inode->i_size) {
        ...
        if (!ext2_check_dir_entry("empty_dir", inode, de, bh, offset)) {
            brelse(bh);
            return 1;
        }
        if (de->inode) {
...
#endif
    if (inode->i_nlink != 2)
        ext2_warning(inode->i_sb, "ext2_rmdir", "empty directory has nlink!=2 (%d)",
                     inode->i_nlink);
#ifndef DONT_USE_DCACHE
...
repeat:
    ...
    if ((dir->i_mode & S_ISVTX) && !suser() && current->euid != inode->i_uid &&
        ...
        goto end_unlink;
    if (!inode->i_nlink) {
        ext2_warning(inode->i_sb, "ext2_unlink", "Deleting nonexistent file (%lu), %d",
...
#define PARENT_INO(buffer)                                                                         \
    ((struct ext2_dir_entry *)((char *)buffer + ((struct ext2_dir_entry *)buffer)->rec_len))->inode

#define PARENT_NAME(buffer)                                                                        \
    ((struct ext2_dir_entry *)((char *)buffer + ((struct ext2_dir_entry *)buffer)->rec_len))->name
 ...
 */
static int do_ext2_rename(struct inode *old_dir, const char *old_name, int old_len,
...
start_up:
    ...
    if (!old_inode)
        goto end_rename;
    retval = -EPERM;
    if ((old_dir->i_mode & S_ISVTX) && current->euid != old_inode->i_uid &&
    ...
    retval = -EPERM;
    if (new_inode && (new_dir->i_mode & S_ISVTX) && current->euid != new_inode->i_uid &&
    ...
    if (!new_bh)
        new_bh = ext2_add_entry(new_dir, new_name, new_len, &new_de, &retval);
    if (!new_bh)
        goto end_rename;
    /*
...
#ifndef DONT_USE_DCACHE
    ext2_dcache_remove(old_dir->i_dev, old_dir->i_ino, old_de->name, old_de->name_len);
    ext2_dcache_add(new_dir->i_dev, new_dir->i_ino, new_de->name, new_de->name_len, new_de->inode);
#endif
 ...
 */
int ext2_rename(struct inode *old_dir, const char *old_name, int old_len, struct inode *new_dir,
                const char *new_name, int new_len)
{
    ...
    result = do_ext2_rename(old_dir, old_name, old_len, new_dir, new_name, new_len);
