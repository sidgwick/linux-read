...
static void read_inode_bitmap(struct super_block *sb, unsigned long block_group,
 ...
 */
static int load_inode_bitmap(struct super_block *sb, unsigned int block_group)
{
    int i, j;
    ...
    if (sb->u.ext2_sb.s_groups_count <= EXT2_MAX_GROUP_LOADED) {
        if (sb->u.ext2_sb.s_inode_bitmap[block_group]) {
            if (sb->u.ext2_sb.s_inode_bitmap_number[block_group] != block_group)
                ext2_panic(sb, "load_inode_bitmap", "block_group != inode_bitmap_number");
            else
                return block_group;
        } else {
    ...
    if (i < sb->u.ext2_sb.s_loaded_inode_bitmaps &&
        ...
        for (j = i; j > 0; j--) {
            sb->u.ext2_sb.s_inode_bitmap_number[j] = sb->u.ext2_sb.s_inode_bitmap_number[j - 1];
            sb->u.ext2_sb.s_inode_bitmap[j] = sb->u.ext2_sb.s_inode_bitmap[j - 1];
        }
 ...
 */
static void set_inode_dtime(struct inode *inode, struct ext2_group_desc *gdp)
{
    unsigned long inode_block;
    struct buffer_head *bh;
    struct ext2_inode *raw_inode;

    inode_block = gdp->bg_inode_table + (((inode->i_ino - 1) % EXT2_INODES_PER_GROUP(inode->i_sb)) /
...
void ext2_free_inode(struct inode *inode)
{
    ...
    sb = inode->i_sb;
    lock_super(sb);
    if (inode->i_ino < EXT2_FIRST_INO || inode->i_ino > sb->u.ext2_sb.s_es->s_inodes_count) {
        ext2_error(sb, "free_inode", "reserved inode or nonexistent inode");
        unlock_super(sb);
        return;
    }
    ...
    if (!clear_bit(bit, bh->b_data))
        ext2_warning(sb, "ext2_free_inode", "bit already cleared for inode %lu", inode->i_ino);
    else {
 ...
 */
static void inc_inode_version(struct inode *inode, struct ext2_group_desc *gdp, int mode)
{
    unsigned long inode_block;
    struct buffer_head *bh;
    struct ext2_inode *raw_inode;
...
repeat:
    ...
    if (S_ISDIR(mode)) {
        avefreei = es->s_free_inodes_count / sb->u.ext2_sb.s_groups_count;
...
*/
        if (!gdp) {
            for (j = 0; j < sb->u.ext2_sb.s_groups_count; j++) {
                ...
                if (tmp->bg_free_inodes_count && tmp->bg_free_inodes_count >= avefreei) {
                    if (!gdp || (tmp->bg_free_blocks_count > gdp->bg_free_blocks_count)) {
                        i = j;
                        gdp = tmp;
                    }
                }
    ...
    bh = sb->u.ext2_sb.s_inode_bitmap[bitmap_nr];
    if ((j = find_first_zero_bit((unsigned long *)bh->b_data, EXT2_INODES_PER_GROUP(sb))) <
        EXT2_INODES_PER_GROUP(sb)) {
        if (set_bit(j, bh->b_data)) {
            ext2_warning(sb, "ext2_new_inode", "bit already set for inode %d", j);
            goto repeat;
        }
        bh->b_dirt = 1;
        if (sb->s_flags & MS_SYNC) {
        ...
        if (gdp->bg_free_inodes_count != 0) {
            ext2_error(sb, "ext2_new_inode", "Free inodes count corrupted in group %d", i);
            unlock_super(sb);
            iput(inode);
            return NULL;
        }
...
unsigned long ext2_count_free_inodes(struct super_block *sb)
{
    ...
    for (i = 0; i < sb->u.ext2_sb.s_groups_count; i++) {
        ...
        bitmap_nr = load_inode_bitmap(sb, i);
        x = ext2_count_free(sb->u.ext2_sb.s_inode_bitmap[bitmap_nr], EXT2_INODES_PER_GROUP(sb) / 8);
