 ...
 */
static struct dir_cache_entry *find_name(int queue, unsigned short dev, unsigned long dir,
                                         const char *name, int len)
{
    struct dir_cache_entry *p;

    for (p = queue_head[queue]; p != NULL && (p->dev != dev || p->dir != dir || p->len != len ||
 ...
 */
unsigned long ext2_dcache_lookup(unsigned short dev, unsigned long dir, const char *name, int len)
{
 ...
 */
void ext2_dcache_add(unsigned short dev, unsigned long dir, const char *name, int len,
                     unsigned long ino)
{
    struct dir_cache_entry *p;
    int queue;
 ...
 */
void ext2_dcache_remove(unsigned short dev, unsigned long dir, const char *name, int len)
{
    struct dir_cache_entry *p;
    int queue;
