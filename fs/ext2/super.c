...
void ext2_error(struct super_block *sb, const char *function, const char *fmt, ...)
{
    ...
    va_end(args);
    if (test_opt(sb, ERRORS_PANIC) || (sb->u.ext2_sb.s_es->s_errors == EXT2_ERRORS_PANIC &&
                                       !test_opt(sb, ERRORS_CONT) && !test_opt(sb, ERRORS_RO)))
        panic("EXT2-fs panic (device %d/%d): %s: %s\n", MAJOR(sb->s_dev), MINOR(sb->s_dev),
              function, buf);
    printk(KERN_CRIT "EXT2-fs error (device %d/%d): %s: %s\n", MAJOR(sb->s_dev), MINOR(sb->s_dev),
           function, buf);
    if (test_opt(sb, ERRORS_RO) || (sb->u.ext2_sb.s_es->s_errors == EXT2_ERRORS_RO &&
...
NORET_TYPE void ext2_panic(struct super_block *sb, const char *function, const char *fmt, ...)
{
    char buf[1024];
    va_list args;
    ...
    vsprintf(buf, fmt, args);
    va_end(args);
    panic("EXT2-fs panic (device %d/%d): %s: %s\n", MAJOR(sb->s_dev), MINOR(sb->s_dev), function,
          buf);
}
...
static struct super_operations ext2_sops = {ext2_read_inode, NULL,           ext2_write_inode,
                                            ext2_put_inode,  ext2_put_super, ext2_write_super,
                                            ext2_statfs,     ext2_remount};

#ifdef EXT2FS_PRE_02B_COMPAT

static int convert_pre_02b_fs(struct super_block *sb, struct buffer_head *bh)
{
    ...
    groups_count = (sb->u.ext2_sb.s_blocks_count - sb->u.ext2_sb.s_first_data_block +
 ...
 */
static int parse_options(char *options, unsigned long *sb_block, unsigned long *mount_options)
{
    char *this_char;
    char *value;
    ...
    for (this_char = strtok(options, ","); this_char != NULL; this_char = strtok(NULL, ",")) {
        ...
        } else if (!strcmp(this_char, "grpid") || !strcmp(this_char, "bsdgroups"))
            set_opt(*mount_options, GRPID);
        else if (!strcmp(this_char, "nocheck")) {
            clear_opt(*mount_options, CHECK_NORMAL);
            clear_opt(*mount_options, CHECK_STRICT);
        } else if (!strcmp(this_char, "nogrpid") || !strcmp(this_char, "sysvgroups"))
...
static void ext2_setup_super(struct super_block *sb, struct ext2_super_block *es)
{
    if (!(sb->s_flags & MS_RDONLY)) {
        if (!(sb->u.ext2_sb.s_mount_state & EXT2_VALID_FS))
            ...
            printk("EXT2-fs warning: mounting fs with errors, "
                   "running e2fsck is recommended\n");
        else if (es->s_max_mnt_count >= 0 && es->s_mnt_count >= (unsigned short)es->s_max_mnt_count)
            printk("EXT2-fs warning: maximal mount count reached, "
                   "running e2fsck is recommended\n");
        else if (es->s_checkinterval && (es->s_lastcheck + es->s_checkinterval <= CURRENT_TIME))
        ...
        if (test_opt(sb, DEBUG))
            printk("[EXT II FS %s, %s, bs=%lu, fs=%lu, gc=%lu, "
                   ...
                   EXT2FS_VERSION, EXT2FS_DATE, sb->s_blocksize, sb->u.ext2_sb.s_frag_size,
                   sb->u.ext2_sb.s_groups_count, EXT2_BLOCKS_PER_GROUP(sb),
                   EXT2_INODES_PER_GROUP(sb), sb->u.ext2_sb.s_mount_opt);
...
static int ext2_check_descriptors(struct super_block *sb)
{
    ...
    for (i = 0; i < sb->u.ext2_sb.s_groups_count; i++) {
        ...
        if (gdp->bg_inode_table < block || gdp->bg_inode_table + sb->u.ext2_sb.s_itb_per_group >=
...
struct super_block *ext2_read_super(struct super_block *sb, void *data, int silent)
{
    ...
    if (!parse_options((char *)data, &sb_block, &sb->u.ext2_sb.s_mount_opt)) {
        sb->s_dev = 0;
        return NULL;
    }
    ...
    if (sb->s_blocksize != BLOCK_SIZE &&
        (sb->s_blocksize == 1024 || sb->s_blocksize == 2048 || sb->s_blocksize == 4096)) {
    ...
    sb->u.ext2_sb.s_itb_per_group =
        sb->u.ext2_sb.s_inodes_per_group / sb->u.ext2_sb.s_inodes_per_block;
    ...
    sb->u.ext2_sb.s_groups_count =
        (es->s_blocks_count - es->s_first_data_block + EXT2_BLOCKS_PER_GROUP(sb) - 1) /
    ...
    for (i = 0; i < bh_count; i++) {
        sb->u.ext2_sb.s_group_desc[i] = bread(dev, logic_sb_block + i + 1, sb->s_blocksize);
        if (!sb->u.ext2_sb.s_group_desc[i]) {
            sb->s_dev = 0;
            unlock_super(sb);
            for (j = 0; j < i; j++)
...
static void ext2_commit_super(struct super_block *sb, struct ext2_super_block *es)
{
...
int ext2_remount(struct super_block *sb, int *flags, char *data)
{
    ...
    if ((*flags & MS_RDONLY) == (sb->s_flags & MS_RDONLY))
        return 0;
    if (*flags & MS_RDONLY) {
        if (es->s_state & EXT2_VALID_FS || !(sb->u.ext2_sb.s_mount_state & EXT2_VALID_FS))
            return 0;
        /*
...
void ext2_statfs(struct super_block *sb, struct statfs *buf)
{
    ...
    if (tmp >= sb->u.ext2_sb.s_es->s_r_blocks_count)
        put_fs_long(tmp - sb->u.ext2_sb.s_es->s_r_blocks_count, &buf->f_bavail);
    else
