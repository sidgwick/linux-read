...
#define DIRECT_BLOCK ((inode->i_size + inode->i_sb->s_blocksize - 1) / inode->i_sb->s_blocksize)
...
repeat:
    for (i = direct_block; i < EXT2_NDIR_BLOCKS; i++) {
        ...
        *p = 0;
        ...
        inode->i_dirt = 1;
        if (inode->u.ext2_i.i_flags & EXT2_SECRM_FL) {
            clear_block(bh->b_data, inode->i_sb->s_blocksize, RANDOM_INT);
            bh->b_dirt = 1;
        }
        brelse(bh);
        if (free_count == 0) {
...
repeat:
    for (i = dindirect_block; i < addr_per_block; i++) {
        ...
        if (!tmp)
            continue;
        retry |= trunc_indirect(inode, offset + (i * addr_per_block), dind);
        dind_bh->b_dirt = 1;
    }
...
static int trunc_tindirect(struct inode *inode)
{
    ...
    int addr_per_block = EXT2_ADDR_PER_BLOCK(inode->i_sb);
    int blocks = inode->i_sb->s_blocksize / 512;
#define TINDIRECT_BLOCK                                                                            \
    (((int)DIRECT_BLOCK - (addr_per_block * addr_per_block + addr_per_block + EXT2_NDIR_BLOCKS)) / \
...
repeat:
    for (i = tindirect_block; i < addr_per_block; i++) {
        ...
        retry |= trunc_dindirect(
            inode, EXT2_NDIR_BLOCKS + addr_per_block + (i + 1) * addr_per_block * addr_per_block,
            tind);
        tind_bh->b_dirt = 1;
    }
...
void ext2_truncate(struct inode *inode)
{
    ...
    while (1) {
        ...
        retry |= trunc_dindirect(inode, EXT2_IND_BLOCK + EXT2_ADDR_PER_BLOCK(inode->i_sb),
