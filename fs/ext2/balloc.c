...
static inline int find_first_zero_bit(unsigned long *addr, unsigned size)
{
    ...
    __asm__("
                ...
                1,
            % % eax repe;
            scasl je 1f subl $4, % % edi movl(% % edi), % % eax notl % % eax bsfl % % eax,
            % % edx jmp 2f 1
            : xorl % % edx, % % edx 2
            : subl % % ebx, % % edi shll $3, % % edi addl % % edi, % % edx "
            : "=d"(res)
...
static inline int find_next_zero_bit(unsigned long *addr, int size, int offset)
{
...
static struct ext2_group_desc *get_group_desc(struct super_block *sb, unsigned int block_group,
...
static void read_block_bitmap(struct super_block *sb, unsigned int block_group,
...
static int load__block_bitmap(struct super_block *sb, unsigned int block_group)
{
    ...
    if (sb->u.ext2_sb.s_groups_count <= EXT2_MAX_GROUP_LOADED) {
        if (sb->u.ext2_sb.s_block_bitmap[block_group]) {
            if (sb->u.ext2_sb.s_block_bitmap_number[block_group] != block_group)
                ext2_panic(sb, "load_block_bitmap", "block_group != block_bitmap_number");
            else
                return block_group;
        } else {
    ...
    if (i < sb->u.ext2_sb.s_loaded_block_bitmaps &&
        ...
        for (j = i; j > 0; j--) {
            sb->u.ext2_sb.s_block_bitmap_number[j] = sb->u.ext2_sb.s_block_bitmap_number[j - 1];
            sb->u.ext2_sb.s_block_bitmap[j] = sb->u.ext2_sb.s_block_bitmap[j - 1];
        }
...
void ext2_free_blocks(struct super_block *sb, unsigned long block, unsigned long count)
{
    struct buffer_head *bh;
    ...
    if (test_opt(sb, CHECK_STRICT) &&
        (in_range(gdp->bg_block_bitmap, block, count) ||
         ...
         in_range(block, gdp->bg_inode_table, sb->u.ext2_sb.s_itb_per_group) ||
         in_range(block + count - 1, gdp->bg_inode_table, sb->u.ext2_sb.s_itb_per_group)))
        ext2_panic(sb, "ext2_free_blocks",
    ...
    for (i = 0; i < count; i++) {
        if (!clear_bit(bit + i, bh->b_data))
            ext2_warning(sb, "ext2_free_blocks", "bit already cleared for block %lu", block);
        else {
 ...
 */
int ext2_new_block(struct super_block *sb, unsigned long goal, unsigned long *prealloc_count,
...
repeat:
    ...
    if (gdp->bg_free_blocks_count > 0) {
        ...
        if (j) {
             ...
             */
            lmap = ((((unsigned long *)bh->b_data)[j >> 5]) >> ((j & 31) + 1));
            if (j < EXT2_BLOCKS_PER_GROUP(sb) - 32)
                lmap |= (((unsigned long *)bh->b_data)[(j >> 5) + 1]) << (31 - (j & 31));
            else
                lmap |= 0xffffffff << (31 - (j & 31));
            if (lmap != 0xffffffffl) {
                __asm__("bsfl %1,%0" : "=r"(k) : "r"(~lmap));
                k++;
        ...
        r = find_first_zero_byte(p, (EXT2_BLOCKS_PER_GROUP(sb) - j + 7) >> 3);
        k = (r - ((char *)bh->b_data)) << 3;
        if (k < EXT2_BLOCKS_PER_GROUP(sb)) {
            j = k;
            goto search_back;
        }
        k = find_next_zero_bit((unsigned long *)bh->b_data, EXT2_BLOCKS_PER_GROUP(sb), j);
        if (k < EXT2_BLOCKS_PER_GROUP(sb)) {
            j = k;
            goto got_block;
        }
    ...
    bitmap_nr = load_block_bitmap(sb, i);
    bh = sb->u.ext2_sb.s_block_bitmap[bitmap_nr];
    r = find_first_zero_byte(bh->b_data, EXT2_BLOCKS_PER_GROUP(sb) >> 3);
    j = (r - bh->b_data) << 3;
    if (j < EXT2_BLOCKS_PER_GROUP(sb))
        goto search_back;
    else
        j = find_first_zero_bit((unsigned long *)bh->b_data, EXT2_BLOCKS_PER_GROUP(sb));
    if (j >= EXT2_BLOCKS_PER_GROUP(sb)) {
        ext2_error(sb, "ext2_new_block", "Free blocks count corrupted for block group %d", i);
        unlock_super(sb);
        return 0;
    }
...
got_block:
    ...
    if (test_opt(sb, CHECK_STRICT) &&
        (tmp == gdp->bg_block_bitmap || tmp == gdp->bg_inode_bitmap ||
    ...
    if (set_bit(j, bh->b_data)) {
        ext2_warning(sb, "ext2_new_block", "bit already set for block %d", j);
        goto repeat;
    }
...
unsigned long ext2_count_free_blocks(struct super_block *sb)
{
    ...
    for (i = 0; i < sb->u.ext2_sb.s_groups_count; i++) {
        ...
        bitmap_nr = load_block_bitmap(sb, i);
        x = ext2_count_free(sb->u.ext2_sb.s_block_bitmap[bitmap_nr], sb->s_blocksize);
...
static inline int block_in_use(unsigned long block, struct super_block *sb, unsigned char *map)
{
    return test_bit((block - sb->u.ext2_sb.s_es->s_first_data_block) % EXT2_BLOCKS_PER_GROUP(sb),
                    map);
}
...
void ext2_check_blocks_bitmap(struct super_block *sb)
{
    ...
    gdp = NULL;
    desc_blocks =
        (sb->u.ext2_sb.s_groups_count + EXT2_DESC_PER_BLOCK(sb) - 1) / EXT2_DESC_PER_BLOCK(sb);
    for (i = 0; i < sb->u.ext2_sb.s_groups_count; i++) {
        gdp = get_group_desc(sb, i, NULL);
        desc_count += gdp->bg_free_blocks_count;
        bitmap_nr = load_block_bitmap(sb, i);
        ...
        if (!test_bit(0, bh->b_data))
            ext2_error(sb, "ext2_check_blocks_bitmap", "Superblock in group %d is marked free", i);
        ...
        if (!block_in_use(gdp->bg_block_bitmap, sb, bh->b_data))
            ext2_error(sb, "ext2_check_blocks_bitmap", "Block bitmap for group %d is marked free",
                       i);

        if (!block_in_use(gdp->bg_inode_bitmap, sb, bh->b_data))
            ext2_error(sb, "ext2_check_blocks_bitmap", "Inode bitmap for group %d is marked free",
                       i);
