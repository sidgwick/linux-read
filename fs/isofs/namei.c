 ...
 */
static struct buffer_head *isofs_find_entry(struct inode *dir, const char *name, int namelen,
                                            int *ino, int *ino_back)
{
    ...
    *ino = 0;
    ...
    if (!(block = dir->u.isofs_i.i_first_extent))
        return NULL;
    ...
    while (f_pos < dir->i_size) {
        ...
        if (offset >= bufsize) {
            ...
            de = (struct iso_directory_record *)((char *)cpnt + old_offset);
            brelse(bh);
            offset = f_pos & (bufsize - 1);
            block = isofs_bmap(dir, f_pos >> bufbits);
            if (!block || !(bh = bread(dir->i_dev, block, bufsize))) {
        ...
        if (de->name[0] == 1 && de->name_len[0] == 1) {
            ...
            printk("Doing .. (%d %d)",
                   ...
                   dir->i_ino);
#endif
            if ((dir->i_sb->u.isofs_sb.s_firstdatazone << bufbits) != dir->i_ino)
        ...
        if (match) {
            if (inode_number == -1) {
                ...
                inode_number = isofs_lookup_grandparent(dir, find_rock_ridge_relocation(de, dir));
                if (inode_number == -1) {
                    /* Should never happen */
...
int isofs_lookup(struct inode *dir, const char *name, int len, struct inode **result)
{
    ...
    *result = NULL;
    ...
    ino = 0;
    if (dir->i_dev == cache.dev && dir->i_ino == cache.dir && len == cache.dlen &&
        ...
        if (cache.dlen == 1 && cache.filename[0] == '.')
            ino = 0;
        if (cache.dlen == 2 && cache.filename[0] == '.' && cache.filename[1] == '.')
            ino = 0;
    };
