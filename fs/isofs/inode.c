...
static struct super_operations isofs_sops = {isofs_read_inode,
                                             NULL, /* notify_change */
                                             NULL, /* write_inode */
                                             NULL, /* put_inode */
                                             isofs_put_super,
...
struct super_block *isofs_read_super(struct super_block *s, void *data, int silent)
{
...
void isofs_read_inode(struct inode *inode)
{
   ...
   it impossible for a file to be > 16Mb on the CDROM!!!*/
    ...
    if (inode->i_sb->u.isofs_sb.s_cruft == 'y' && inode->i_size & 0xff000000) {
    ...
    if ((raw_inode->flags[-high_sierra] & ~2) != 0) {
        printk("Unusual flag settings for ISO file (%ld %x).\n", inode->i_ino,
               raw_inode->flags[-high_sierra]);
    }

#ifdef DEBUG
    printk("Get inode %d: %d %d: %d\n", inode->i_ino, block, ((int)pnt) & 0x3ff, inode->i_size);
#endif

    inode->i_mtime = inode->i_atime = inode->i_ctime = iso_date(raw_inode->date, high_sierra);
