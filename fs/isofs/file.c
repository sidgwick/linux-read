...
static inline void unixify_text_buffer(char *buffer, int chars, int mode)
{
    while (chars--) {
        if (*buffer == 0x1a)
            *buffer = 0x0a;
        if (*buffer == 0x0d) {
            if (mode == ISOFS_FILE_TEXT_M)
                *buffer = 0x0a;
            if (mode == ISOFS_FILE_TEXT)
                *buffer = ' ';
        }
        buffer++;
    }
...
static void isofs_determine_filetype(struct inode *inode)
{
    ...
    if (block && (bh = bread(inode->i_dev, block, ISOFS_BUFFER_SIZE(inode)))) {
        ...
        for (i = 0; i < (inode->i_size < ISOFS_BUFFER_SIZE(inode) ? inode->i_size
            ...
            if (*pnt >= 0x20 || *pnt == 0x1a)
                continue;
            if (*pnt == 0x0a) {
                result = ISOFS_FILE_TEXT;
                continue;
            };
            if (*pnt >= 0x9 && *pnt <= 0x0d)
                continue;
        ...
        do {            /* Finish off all I/O that has actually completed */
            ...
            if (*bhe) {
                if (inode->u.isofs_i.i_file_format == ISOFS_FILE_TEXT ||
                    ...
                    unixify_text_buffer(offset + (*bhe)->b_data, chars,
            ...
            if (++bhe == &buflist[NBUF])
                bhe = buflist;
        } while (bhe != bhb && (*bhe == 0 || !(*bhe)->b_lock) && (left > 0));
    } while (left > 0);
