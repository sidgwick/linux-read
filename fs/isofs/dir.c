...
static int isofs_readdir(struct inode *inode, struct file *filp, struct dirent *dirent, int count)
{
    ...
    while (filp->f_pos < inode->i_size) {
        ...
        /* Handle the case of the '..' directory */
            ...
            i = 2;
            dpnt = "..";
            if ((inode->i_sb->u.isofs_sb.s_firstdatazone << bufbits) != inode->i_ino)
            ...
            if (inode_number == -1) {
                inode_number =
                    isofs_lookup_grandparent(inode, find_rock_ridge_relocation(de, inode));
