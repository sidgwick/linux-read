...
int find_rock_ridge_relocation(struct iso_directory_record *de, struct inode *inode)
{
    int flag;
    int len;
    int retval;
...
repeat: {
    ...
    while (len > 1) { /* There may be one byte for padding somewhere */
        ...
        case SIG('R', 'R'):
            ...
            if (flag && !(rrflag & RR_PL))
                goto out;
            if (!flag && !(rrflag & RR_CL))
                goto out;
            break;
        case SIG('S', 'P'):
            CHECK_SP(goto out);
...
int get_rock_ridge_filename(struct iso_directory_record *de, char **name, int *namlen,
                            struct inode *inode)
{
    int len;
    unsigned char *chr;
...
repeat: {
    ...
    while (len > 1) { /* There may be one byte for padding somewhere */
        ...
        switch (sig) {
        case SIG('R', 'R'):
            if ((rr->u.RR.flags[0] & (RR_PX | RR_TF | RR_SL | RR_CL)) == 0)
                goto out;
            break;
        case SIG('S', 'P'):
            CHECK_SP(goto out);
