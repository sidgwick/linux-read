 ...
 */
static int dir_namei(const char *pathname, int *namelen, const char **name, struct inode *base,
                     struct inode **res_inode)
{
    char c;
    const char *thisname;
...
static int do_rename(const char *oldname, const char *newname)
{
    ...
    if (!old_len ||
        (old_base[0] == '.' && (old_len == 1 || (old_base[1] == '.' && old_len == 2)))) {
        iput(old_dir);
        return -EPERM;
    }
    ...
    if (!new_len ||
        (new_base[0] == '.' && (new_len == 1 || (new_base[1] == '.' && new_len == 2)))) {
        iput(old_dir);
        iput(new_dir);
        return -EPERM;
    }
