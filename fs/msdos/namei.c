...
static char *reserved_names[] = {"CON     ", "PRN     ", "NUL     ", "AUX     ", "LPT1    ",
                                 "LPT2    ", "LPT3    ", "LPT4    ", "COM1    ", "COM2    ",
                                 "COM3    ", "COM4    ", NULL};
...
static int msdos_format_name(char conv, const char *name, int len, char *res, int dot_dirs)
{
    char *walk, **reserved;
    unsigned char c;
    int space;
...
static int msdos_find(struct inode *dir, const char *name, int len, struct buffer_head **bh,
                      struct msdos_dir_entry **de, int *ino)
{
    char msdos_name[MSDOS_NAME];
    int res;

    if ((res = msdos_format_name(MSDOS_SB(dir->i_sb)->name_check, name, len, msdos_name, 1)) < 0)
        return res;
    return msdos_scan(dir, msdos_name, bh, de, ino);
}

int msdos_lookup(struct inode *dir, const char *name, int len, struct inode **result)
{
    int ino, res;
...
static int msdos_create_entry(struct inode *dir, char *name, int is_dir, struct inode **result)
{
    struct buffer_head *bh;
    struct msdos_dir_entry *de;
    int res, ino;
    ...
    if (!*result)
        return -EIO;
    (*result)->i_mtime = (*result)->i_atime = (*result)->i_ctime = CURRENT_TIME;
    (*result)->i_dirt = 1;
    return 0;
}

int msdos_create(struct inode *dir, const char *name, int len, int mode, struct inode **result)
{
    struct buffer_head *bh;
    struct msdos_dir_entry *de;
    char msdos_name[MSDOS_NAME];
    ...
    if (!dir)
        return -ENOENT;
    if ((res = msdos_format_name(MSDOS_SB(dir->i_sb)->name_check, name, len, msdos_name, 0)) < 0) {
        iput(dir);
        return res;
    }
    lock_creation();
...
int msdos_mkdir(struct inode *dir, const char *name, int len, int mode)
{
    ...
    if ((res = msdos_add_cluster(inode)) < 0)
        goto mkdir_error;
...
static int msdos_empty(struct inode *dir)
{
    ...
    if (MSDOS_I(dir)->i_start) { /* may be zero in mkdir */
        ...
        while (msdos_get_entry(dir, &pos, &bh, &de) > -1)
            if (!IS_FREE(de->name) && strncmp(de->name, MSDOS_DOT, MSDOS_NAME) &&
...
static int rename_same_dir(struct inode *old_dir, char *old_name, struct inode *new_dir,
                           char *new_name, struct buffer_head *old_bh,
                           struct msdos_dir_entry *old_de, int old_ino)
{
    ...
    if (exists) {
        ...
        error = S_ISDIR(new_inode->i_mode)
                    ? (old_de->attr & ATTR_DIR) ? msdos_empty(new_inode) : -EPERM
                : (old_de->attr & ATTR_DIR) ? -EPERM
                                            : 0;
        if (error) {
            iput(new_inode);
            brelse(new_bh);
...
static int rename_diff_dir(struct inode *old_dir, char *old_name, struct inode *new_dir,
                           char *new_name, struct buffer_head *old_bh,
                           struct msdos_dir_entry *old_de, int old_ino)
{
    ...
    if (old_ino == new_dir->i_ino)
        return -EINVAL;
    if (!(walk = iget(new_dir->i_sb, new_dir->i_ino)))
        return -EIO;
    ...
    iput(walk);
    while ((error = msdos_scan(new_dir, NULL, &free_bh, &free_de, &free_ino)) < 0) {
        if (error != -ENOENT)
            return error;
    ...
    if (S_ISDIR(old_inode->i_mode)) {
        if ((error = msdos_scan(old_inode, MSDOS_DOTDOT, &dotdot_bh, &dotdot_de, &dotdot_ino)) < 0)
            goto rename_done;
        ...
        dotdot_de->start = MSDOS_I(dotdot_inode)->i_start = MSDOS_I(new_dir)->i_start;
...
int msdos_rename(struct inode *old_dir, const char *old_name, int old_len, struct inode *new_dir,
                 const char *new_name, int new_len)
{
    ...
    if ((error = msdos_format_name(MSDOS_SB(old_dir->i_sb)->name_check, old_name, old_len,
                                   old_msdos_name, 1)) < 0)
        goto rename_done;
    if ((error = msdos_format_name(MSDOS_SB(new_dir->i_sb)->name_check, new_name, new_len,
                                   new_msdos_name, 0)) < 0)
        goto rename_done;
    if ((error = msdos_scan(old_dir, old_msdos_name, &old_bh, &old_de, &old_ino)) < 0)
        goto rename_done;
    lock_creation();
    if (old_dir == new_dir)
        error = rename_same_dir(old_dir, old_msdos_name, new_dir, new_msdos_name, old_bh, old_de,
                                old_ino);
    else
        error = rename_diff_dir(old_dir, old_msdos_name, new_dir, new_msdos_name, old_bh, old_de,
                                old_ino);
    unlock_creation();
    brelse(old_bh);
rename_done:
