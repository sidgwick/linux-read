...
static struct super_operations msdos_sops = {msdos_read_inode,  msdos_notify_change,
                                             msdos_write_inode, msdos_put_inode,
                                             msdos_put_super,   NULL, /* added in 0.96c */
                                             msdos_statfs,      NULL};

static int parse_options(char *options, char *check, char *conversion, uid_t *uid, gid_t *gid,
                         int *umask, int *debug, int *fat, int *quiet)
{
...
struct super_block *msdos_read_super(struct super_block *s, void *data, int silent)
{
    ...
    if (!parse_options((char *)data, &check, &conversion, &uid, &gid, &umask, &debug, &fat,
                       &quiet)) {
        s->s_dev = 0;
        return NULL;
    }
...
#define ROUND_TO_MULTIPLE(n, m) ((n) && (m) ? (n) + (m) - 1 - ((n) - 1) % (m) : 0)
    ...
    MSDOS_SB(s)->dir_start =
        (CF_LE_W(b->reserved) + b->fats * CF_LE_W(b->fat_length)) * sector_mult;
    MSDOS_SB(s)->dir_entries = CF_LE_W(*((unsigned short *)&b->dir_entries));
    MSDOS_SB(s)->data_start =
        MSDOS_SB(s)->dir_start +
        ROUND_TO_MULTIPLE((MSDOS_SB(s)->dir_entries << MSDOS_DIR_BITS) >> SECTOR_BITS, sector_mult);
    data_sectors =
    ...
    if (!error) {
        MSDOS_SB(s)->clusters = b->cluster_size ? data_sectors / b->cluster_size / sector_mult : 0;
        MSDOS_SB(s)->fat_bits = fat ? fat : MSDOS_SB(s)->clusters > MSDOS_FAT12 ? 16 : 12;
        error = !MSDOS_SB(s)->fats || (MSDOS_SB(s)->dir_entries & (MSDOS_DPS - 1)) ||
                MSDOS_SB(s)->clusters + 2 >
                    MSDOS_SB(s)->fat_length * SECTOR_SIZE * 8 / MSDOS_SB(s)->fat_bits ||
                (logical_sector_size & (SECTOR_SIZE - 1)) || !b->secs_track || !b->heads;
    }
    brelse(bh);
    if (error || debug) {
        printk("[MS-DOS FS Rel. 12,FAT %d,check=%c,conv=%c,"
               ...
               MSDOS_CAN_BMAP(MSDOS_SB(s)) ? ",bmap" : "");
        printk("[me=0x%x,cs=%d,#f=%d,fs=%d,fl=%d,ds=%d,de=%d,data=%d,"
               ...
               MSDOS_SB(s)->data_start, CF_LE_W(*(unsigned short *)&b->sectors), b->total_sect,
               logical_sector_size);
    }
    if (error) {
        if (!silent)
...
int msdos_bmap(struct inode *inode, int block)
{
    ...
    if ((sb->cluster_size & 1) || (sb->data_start & 1))
        return 0;
...
void msdos_read_inode(struct inode *inode)
{
    ...
    if (inode->i_ino == MSDOS_ROOT_INO) {
        inode->i_mode = (S_IRWXUGO & ~MSDOS_SB(inode->i_sb)->fs_umask) | S_IFDIR;
        ...
        inode->i_blocks = (inode->i_size + inode->i_blksize - 1) / inode->i_blksize *
    ...
    if ((raw_entry->attr & ATTR_DIR) && !IS_FREE(raw_entry->name)) {
        inode->i_mode =
            MSDOS_MKMODE(raw_entry->attr, S_IRWXUGO & ~MSDOS_SB(inode->i_sb)->fs_umask) | S_IFDIR;
        inode->i_op = &msdos_dir_inode_operations;
        ...
        inode->i_mode =
            MSDOS_MKMODE(raw_entry->attr, (IS_NOEXEC(inode) ? S_IRUGO | S_IWUGO : S_IRWXUGO) &
                                              ~MSDOS_SB(inode->i_sb)->fs_umask) |
            S_IFREG;
        inode->i_op = MSDOS_CAN_BMAP(MSDOS_SB(inode->i_sb)) ? &msdos_file_inode_operations
                                                            : &msdos_file_inode_operations_no_bmap;
        MSDOS_I(inode)->i_start = CF_LE_W(raw_entry->start);
        inode->i_nlink = 1;
        inode->i_size = CF_LE_L(raw_entry->size);
    }
    MSDOS_I(inode)->i_binary = is_binary(MSDOS_SB(inode->i_sb)->conversion, raw_entry->ext);
...
void msdos_write_inode(struct inode *inode)
{
    ...
    inode->i_dirt = 0;
    if (inode->i_ino == MSDOS_ROOT_INO || !inode->i_nlink)
        return;
    if (!(bh = bread(inode->i_dev, inode->i_ino >> MSDOS_DPB_BITS, BLOCK_SIZE))) {
        printk("dev = 0x%04X, ino = %ld\n", inode->i_dev, inode->i_ino);
    ...
    raw_entry->attr |= MSDOS_MKATTR(inode->i_mode) | MSDOS_I(inode)->i_attrs;
...
int msdos_notify_change(int flags, struct inode *inode)
{
    ...
    error = 0;
    if ((flags & NOTIFY_UIDGID) && (inode->i_uid != MSDOS_SB(inode->i_sb)->fs_uid ||
                                    inode->i_gid != MSDOS_SB(inode->i_sb)->fs_gid)) {
        inode->i_uid = MSDOS_SB(inode->i_sb)->fs_uid;
        inode->i_gid = MSDOS_SB(inode->i_sb)->fs_gid;
        error = -EPERM;
    ...
    inode->i_mode =
        ((inode->i_mode & S_IFMT) |
         ((((inode->i_mode & S_IRWXU & ~MSDOS_SB(inode->i_sb)->fs_umask) | S_IRUSR) >> 6) *
          S_IXUGO)) &
        ~MSDOS_SB(inode->i_sb)->fs_umask;
    return MSDOS_SB(inode->i_sb)->quiet ? 0 : error;
}
