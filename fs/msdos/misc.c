...
static char bin_extensions[] = "EXECOMBINAPPSYSDRVOVLOVROBJLIBDLLPIF" /* program code */
...
void fs_panic(struct super_block *s, char *msg)
{
    ...
    printk("Filesystem panic (dev 0x%04X, mounted on 0x%04X:%ld)\n  %s\n", s->s_dev,
           s->s_covered->i_dev, s->s_covered->i_ino, msg);
    if (not_ro)
...
{
    while (MSDOS_SB(sb)->fat_lock)
        sleep_on(&MSDOS_SB(sb)->fat_wait);
...
int msdos_add_cluster(struct inode *inode)
{
    ...
    if (inode->i_ino == MSDOS_ROOT_INO)
        return -ENOSPC;
    if (!MSDOS_SB(inode->i_sb)->free_clusters)
        return -ENOSPC;
    lock_fat(inode->i_sb);
    ...
    for (count = 0; count < limit; count++) {
        ...
        if (fat_access(inode->i_sb, nr, -1) == 0)
            break;
    }
#ifdef DEBUG
    printk("free cluster: %d\n", nr);
#endif
    MSDOS_SB(inode->i_sb)->prev_free = (count + MSDOS_SB(inode->i_sb)->prev_free + 1) % limit;
    if (count >= limit) {
        MSDOS_SB(inode->i_sb)->free_clusters = 0;
        unlock_fat(inode->i_sb);
        return -ENOSPC;
    }
    fat_access(inode->i_sb, nr, MSDOS_SB(inode->i_sb)->fat_bits == 12 ? 0xff8 : 0xfff8);
    ...
    if ((current = MSDOS_I(inode)->i_start) != 0) {
        ...
        while (current && current != -1)
            if (!(current = fat_access(inode->i_sb, last = current, -1))) {
    ...
    for (current = 0; current < MSDOS_SB(inode->i_sb)->cluster_size; current++) {
        sector = MSDOS_SB(inode->i_sb)->data_start +
                 (nr - 2) * MSDOS_SB(inode->i_sb)->cluster_size + current;
#ifdef DEBUG
    ...
    if (S_ISDIR(inode->i_mode)) {
        if (inode->i_size & (SECTOR_SIZE - 1)) {
            ...
            inode->i_size = (inode->i_size + SECTOR_SIZE) & ~(SECTOR_SIZE - 1);
        }
...
void date_unix2dos(int unix_date, unsigned short *time, unsigned short *date)
{
    ...
    *time =
        (unix_date % 60) / 2 + (((unix_date / 60) % 60) << 5) + (((unix_date / 3600) % 24) << 11);
                    ...
                    struct msdos_dir_entry **de)
{
    ...
    while (1) {
        ...
        *de = (struct msdos_dir_entry *)(data + (offset & (SECTOR_SIZE - 1)));
...
#define RSS_NAME /* search for name */                                                             \
    done = !strncmp(data[entry].name, name, MSDOS_NAME) && !(data[entry].attr & ATTR_VOLUME);
...
static int raw_scan_sector(struct super_block *sb, int sector, char *name, int *number, int *ino,
                         ...
                         struct buffer_head **res_bh, struct msdos_dir_entry **res_de)
{
    ...
    for (count = 0; count < MSDOS_SB(sb)->dir_entries / MSDOS_DPS; count++) {
        if ((cluster = raw_scan_sector(sb, MSDOS_SB(sb)->dir_start + count, name, number, ino,
                                       res_bh, res_de)) >= 0)
            return cluster;
    }
    return -ENOENT;
...
static int raw_scan_nonroot(struct super_block *sb, int start, char *name, int *number, int *ino,
                            struct buffer_head **res_bh, struct msdos_dir_entry **res_de)
{
    int count, cluster;
    ...
    do {
        for (count = 0; count < MSDOS_SB(sb)->cluster_size; count++) {
            if ((cluster = raw_scan_sector(sb,
                                           (start - 2) * MSDOS_SB(sb)->cluster_size +
                                               MSDOS_SB(sb)->data_start + count,
...
static int raw_scan(struct super_block *sb, int start, char *name, int *number, int *ino,
                    struct buffer_head **res_bh, struct msdos_dir_entry **res_de)
{
    if (start)
...
int msdos_parent_ino(struct inode *dir, int locked)
{
    ...
    if (dir->i_ino == MSDOS_ROOT_INO)
        return dir->i_ino;
    if (!locked)
        lock_creation(); /* prevent renames */
    if ((current = raw_scan(dir->i_sb, MSDOS_I(dir)->i_start, MSDOS_DOTDOT, &zero, NULL, NULL,
                            NULL)) < 0) {
        if (!locked)
            unlock_creation();
        return current;
    ...
    if (!current)
        ...
        if ((prev = raw_scan(dir->i_sb, current, MSDOS_DOTDOT, &zero, NULL, NULL, NULL)) < 0) {
            if (!locked)
                unlock_creation();
            return prev;
        }
...
int msdos_subdirs(struct inode *dir)
{
    ...
    if (dir->i_ino == MSDOS_ROOT_INO)
        ...
        if (!MSDOS_I(dir)->i_start)
            return 0; /* in mkdir */
        else
            (void)raw_scan_nonroot(dir->i_sb, MSDOS_I(dir)->i_start, NULL, &count, NULL, NULL,
                                   NULL);
    }
    return count;
}
               ...
               struct msdos_dir_entry **res_de, int *ino)
{
    ...
    if (name)
        res = raw_scan(dir->i_sb, MSDOS_I(dir)->i_start, name, NULL, ino, res_bh, res_de);
    else
        res = raw_scan(dir->i_sb, MSDOS_I(dir)->i_start, NULL, NULL, ino, res_bh, res_de);
    return res < 0 ? res : 0;
}
