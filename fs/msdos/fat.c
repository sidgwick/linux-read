...
int fat_access(struct super_block *sb, int nr, int new_value)
{
    ...
    if (!(bh = msdos_sread(sb->s_dev, MSDOS_SB(sb)->fat_start + (first >> SECTOR_BITS), &data))) {
    ...
    if ((first >> SECTOR_BITS) == (last >> SECTOR_BITS)) {
        ...
        if (!(bh2 = msdos_sread(sb->s_dev, MSDOS_SB(sb)->fat_start + (last >> SECTOR_BITS),
                                &data2))) {
            brelse(bh);
            printk("bread in fat_access failed\n");
            return 0;
    ...
    if (MSDOS_SB(sb)->fat_bits == 16) {
        ...
        next = CF_LE_W(((unsigned short *)data)[(first & (SECTOR_SIZE - 1)) >> 1]);
        if (next >= 0xfff7)
            next = -1;
    } else {
    ...
    if (new_value != -1) {
        if (MSDOS_SB(sb)->fat_bits == 16)
            ((unsigned short *)data)[(first & (SECTOR_SIZE - 1)) >> 1] = CT_LE_W(new_value);
        else {
            if (nr & 1) {
        ...
        for (copy = 1; copy < MSDOS_SB(sb)->fats; copy++) {
            if (!(c_bh = msdos_sread(sb->s_dev,
                                     MSDOS_SB(sb)->fat_start + (first >> SECTOR_BITS) +
...
#ifdef DEBUG
    printk("cache lookup: <%d,%d> %d (%d,%d) -> ", inode->i_dev, inode->i_ino, cluster, *f_clu,
           *d_clu);
#endif
...
static void list_cache(void)
{
    ...
    for (walk = fat_cache; walk; walk = walk->next) {
        if (walk->device)
            printk("<%d,%d>(%d,%d) ", walk->device, walk->ino, walk->file_cluster,
                   walk->disk_cluster);
        else
            printk("-- ");
    }
...
int get_cluster(struct inode *inode, int cluster)
{
    ...
    if (!(nr = MSDOS_I(inode)->i_start))
        return 0;
    if (!cluster)
        return nr;
    count = 0;
    for (cache_lookup(inode, cluster, &count, &nr); count < cluster; count++) {
        if ((nr = fat_access(inode->i_sb, nr, -1)) == -1)
            return 0;
        if (!nr)
            return 0;
    }
...
int msdos_smap(struct inode *inode, int sector)
{
    ...
    if (inode->i_ino == MSDOS_ROOT_INO || (S_ISDIR(inode->i_mode) && !MSDOS_I(inode)->i_start)) {
...
int fat_free(struct inode *inode, int skip)
{
    ...
    if (last)
        fat_access(inode->i_sb, last, MSDOS_SB(inode->i_sb)->fat_bits == 12 ? 0xff8 : 0xfff8);
    else {
