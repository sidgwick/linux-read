...
static int msdos_file_read(struct inode *inode, struct file *filp, char *buf, int count);
static int msdos_file_write(struct inode *inode, struct file *filp, char *buf, int count);
 ...
 */
    ...
    for (start = buf; count || carry; count -= size) {
        while (!(sector = msdos_smap(inode, filp->f_pos >> SECTOR_BITS)))
            if ((error = msdos_add_cluster(inode)) < 0)
                break;
        if (error) {
        ...
        if (MSDOS_I(inode)->i_binary) {
            memcpy_fromfs(data + (filp->f_pos & (SECTOR_SIZE - 1)), buf, written = size);
            buf += size;
        } else {
