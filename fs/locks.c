...
static int copy_flock(struct file *filp, struct file_lock *fl, struct flock *l, unsigned int fd);
...
static struct file_lock *alloc_lock(struct file_lock **pos, struct file_lock *fl, unsigned int fd);
...
void fcntl_remove_locks(struct task_struct *task, struct file *filp, unsigned int fd)
{
    struct file_lock *fl;
