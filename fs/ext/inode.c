...
static struct super_operations ext_sops = {ext_read_inode, NULL,          ext_write_inode,
                                           ext_put_inode,  ext_put_super, ext_write_super,
                                           ext_statfs,     NULL};
...
struct super_block *ext_read_super(struct super_block *s, void *data, int silent)
{
    ...
    else if (!(s->u.ext_sb.s_firstfreeblock =
                   bread(dev, s->u.ext_sb.s_firstfreeblocknumber, BLOCK_SIZE))) {
...
void ext_read_inode(struct inode *inode)
{
    ...
    raw_inode = ((struct ext_inode *)bh->b_data) + (inode->i_ino - 1) % EXT_INODES_PER_BLOCK;
