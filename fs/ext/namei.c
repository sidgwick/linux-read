 ...
 */
static struct buffer_head *ext_find_entry(struct inode *dir, const char *name, int namelen,
                                          struct ext_dir_entry **res_dir,
                                          struct ext_dir_entry **prev_dir,
                                          struct ext_dir_entry **next_dir)
{
    long offset;
    ...
    *res_dir = NULL;
    ...
    while (offset < dir->i_size) {
        ...
        if (de->rec_len < 8 || de->rec_len % 8 != 0 || de->rec_len < de->name_len + 8 ||
            ...
            *res_dir = de;
            if (next_dir)
                if (offset + de->rec_len < dir->i_size &&
                    ...
                    *next_dir = (struct ext_dir_entry *)((char *)de + de->rec_len);
                else
                    *next_dir = NULL;
            return bh;
        }
 ...
 */
static struct buffer_head *ext_add_entry(struct inode *dir, const char *name, int namelen,
                                         struct ext_dir_entry **res_dir)
{
    int i;
    long offset;
    ...
    *res_dir = NULL;
    ...
    while (1) {
        ...
        if (offset >= dir->i_size) {
            ...
            if (offset % BLOCK_SIZE == 0 || (BLOCK_SIZE - (offset % BLOCK_SIZE)) < rec_len) {
                if ((offset % BLOCK_SIZE) != 0) {
                    /* If the entry does not fit in the
                       block, the remainder of the block
                       becomes an unused entry */
                    de->inode = 0;
                    de->rec_len = BLOCK_SIZE - (offset & (BLOCK_SIZE - 1));
                    de->name_len = 0;
                    offset += de->rec_len;
        ...
        if (de->rec_len < 8 || de->rec_len % 4 != 0 || de->rec_len < de->name_len + 8 ||
            (((char *)de) + de->rec_len - 1 >= BLOCK_SIZE + bh->b_data)) {
        ...
        if (!de->inode && de->rec_len >= rec_len) {
            if (de->rec_len > rec_len && de->rec_len - rec_len >= EXT_DIR_MIN_SIZE) {
...
static int empty_dir(struct inode *inode)
{
    ...
    while (offset < inode->i_size) {
        ...
        if (de->rec_len < 8 || de->rec_len % 4 != 0 || de->rec_len < de->name_len + 8) {
            ...
            printk("dev=%d, dir=%d, offset=%d, rec_len=%d, name_len=%d\n", inode->i_dev,
                   inode->i_ino, offset, de->rec_len, de->name_len);
            brelse(bh);
            return 1;
        }
        if (de->inode) {
...
static inline void ext_merge_entries(struct ext_dir_entry *de, struct ext_dir_entry *pde,
                                     struct ext_dir_entry *nde)
{
    if (nde && !nde->inode)
...
#define PARENT_INO(buffer)                                                                         \
    ((struct ext_dir_entry *)((char *)buffer + ((struct ext_dir_entry *)buffer)->rec_len))->inode

#define PARENT_NAME(buffer)                                                                        \
    ((struct ext_dir_entry *)((char *)buffer + ((struct ext_dir_entry *)buffer)->rec_len))->name
 ...
 */
int ext_rename(struct inode *old_dir, const char *old_name, int old_len, struct inode *new_dir,
               const char *new_name, int new_len)
{
    ...
    while (lock)
        sleep_on(&wait);
    lock = 1;
    result = do_ext_rename(old_dir, old_name, old_len, new_dir, new_name, new_len);
    lock = 0;
    wake_up(&wait);
    return result;
}
