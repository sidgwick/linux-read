...
#define get_imap_zone(sb, bit_nr, not_que)                                                         \
    get__map_zone((sb), (sb)->u.xiafs_sb.s_imap_buf, (sb)->u.xiafs_sb.s_imap_iznr,                 \
                  (sb)->u.xiafs_sb.s_imap_cached, 1, (sb)->u.xiafs_sb.s_imap_zones,                \
                  _XIAFS_IMAP_SLOTS, bit_nr, not_que)

#define get_zmap_zone(sb, bit_nr, not_que)                                                         \
    get__map_zone((sb), (sb)->u.xiafs_sb.s_zmap_buf, (sb)->u.xiafs_sb.s_zmap_zznr,                 \
                  (sb)->u.xiafs_sb.s_zmap_cached, 1 + (sb)->u.xiafs_sb.s_imap_zones,               \
                  (sb)->u.xiafs_sb.s_zmap_zones, _XIAFS_ZMAP_SLOTS, bit_nr, not_que)

static struct buffer_head *get__map_zone(struct super_block *sb, struct buffer_head *bmap_buf[],
                                         int bznr[], u_char cache, int first_zone, int bmap_zones,
                                         int slots, u_long bit_nr, int *not_que)
{
...
#define xiafs_unlock_super(sb, cache)                                                              \
    if (cache)                                                                                     \
        unlock_super(sb);

#define get_free_ibit(sb, prev_bit)                                                                \
    get_free__bit(sb, sb->u.xiafs_sb.s_imap_buf, sb->u.xiafs_sb.s_imap_iznr,                       \
                  sb->u.xiafs_sb.s_imap_cached, 1, sb->u.xiafs_sb.s_imap_zones, _XIAFS_IMAP_SLOTS, \
                  prev_bit);

#define get_free_zbit(sb, prev_bit)                                                                \
    get_free__bit(sb, sb->u.xiafs_sb.s_zmap_buf, sb->u.xiafs_sb.s_zmap_zznr,                       \
                  sb->u.xiafs_sb.s_zmap_cached, 1 + sb->u.xiafs_sb.s_imap_zones,                   \
                  sb->u.xiafs_sb.s_zmap_zones, _XIAFS_ZMAP_SLOTS, prev_bit);

static u_long get_free__bit(struct super_block *sb, struct buffer_head *bmap_buf[], int bznr[],
                            u_char cache, int first_zone, int bmap_zones, int slots,
                            u_long prev_bit)
{
    ...
    do {
        ...
        bh = get__map_zone(sb, bmap_buf, bznr, cache, first_zone, bmap_zones, slots, pos, &z_nr);
        if (!bh)
            return 0;
...
void xiafs_free_zone(struct super_block *sb, int d_addr)
{
    ...
    if (d_addr < sb->u.xiafs_sb.s_firstdatazone || d_addr >= sb->u.xiafs_sb.s_nzones) {
...
int xiafs_new_zone(struct super_block *sb, u_long prev_addr)
{
    ...
    if (prev_addr < sb->u.xiafs_sb.s_firstdatazone || prev_addr >= sb->u.xiafs_sb.s_nzones) {
