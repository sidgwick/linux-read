...
#define DT_ZONE ((inode->i_size + XIAFS_ZSIZE(inode->i_sb) - 1) >> XIAFS_ZSIZE_BITS(inode->i_sb))
...
repeat:
    for (i = DINDT_ZONE < 0 ? 0 : DINDT_ZONE; i < XIAFS_ADDRS_PER_Z(inode->i_sb); i++) {
        ...
        retry |= trunc_indirect(inode, 8 + ((1 + i) << XIAFS_ADDRS_PER_Z_BITS(inode->i_sb)), dindp);
        dind_bh->b_dirt = 1;
    }
