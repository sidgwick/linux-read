...
static struct super_operations xiafs_sops = {
    xiafs_read_inode, NULL, xiafs_write_inode, xiafs_put_inode,
    xiafs_put_super,  NULL, xiafs_statfs,      NULL};

struct super_block *xiafs_read_super(struct super_block *s, void *data, int silent)
{
    ...
    if (s->s_blocksize != BLOCK_SIZE &&
        (s->s_blocksize == 1024 || s->s_blocksize == 2048 || s->s_blocksize == 4096)) {
        brelse(bh);
...
static struct buffer_head *dt_getblk(struct inode *inode, u_long *lp, int create, u_long prev_addr)
{
    int tmp;
...
static struct buffer_head *indt_getblk(struct inode *inode, struct buffer_head *bh, int nr,
                                       int create, u_long prev_addr)
{
    int tmp;
    u_long *lp;
    struct buffer_head *result;
...
struct buffer_head *xiafs_getblk(struct inode *inode, int zone, int create)
{
    ...
    bh = indt_getblk(inode, bh, zone >> XIAFS_ADDRS_PER_Z_BITS(inode->i_sb), create, prev_addr);
    bh = indt_getblk(inode, bh, zone & (XIAFS_ADDRS_PER_Z(inode->i_sb) - 1), create, prev_addr);
    return bh;
}
...
void xiafs_read_inode(struct inode *inode)
{
    ...
    zone = 1 + inode->i_sb->u.xiafs_sb.s_imap_zones + inode->i_sb->u.xiafs_sb.s_zmap_zones +
    ...
    raw_inode =
        ((struct xiafs_inode *)bh->b_data) + ((ino - 1) & (XIAFS_INODES_PER_Z(inode->i_sb) - 1));
...
static struct buffer_head *xiafs_update_inode(struct inode *inode)
{
    ...
    if (S_ISCHR(inode->i_mode) || S_ISBLK(inode->i_mode))
        ...
        for (zone = 0; zone < 8; zone++)
            raw_inode->i_zone[zone] =
                (raw_inode->i_zone[zone] & 0xff000000) | (inode->u.xiafs_i.i_zone[zone] & 0xffffff);
        raw_inode->i_ind_zone =
            (raw_inode->i_ind_zone & 0xff000000) | (inode->u.xiafs_i.i_ind_zone & 0xffffff);
        raw_inode->i_dind_zone =
            (raw_inode->i_dind_zone & 0xff000000) | (inode->u.xiafs_i.i_dind_zone & 0xffffff);
    }
    inode->i_dirt = 0;
    bh->b_dirt = 1;
    return bh;
