...
static int xiafs_dir_read(struct inode *inode, struct file *filp, char *buf, int count)
{
    return -EISDIR;
}

static int xiafs_readdir(struct inode *inode, struct file *filp, struct dirent *dirent, int count)
{
    u_int offset, i;
    ...
    while (filp->f_pos < inode->i_size) {
        ...
        while (offset < XIAFS_ZSIZE(inode->i_sb) && filp->f_pos < inode->i_size) {
            if (de->d_ino > inode->i_sb->u.xiafs_sb.s_ninodes || de->d_rec_len < 12 ||
                ...
                de->d_name_len > _XIAFS_NAME_LEN || de->d_name[de->d_name_len]) {
