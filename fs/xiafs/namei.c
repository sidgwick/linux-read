 ...
 */
static struct buffer_head *xiafs_find_entry(struct inode *inode, const char *name, int namelen,
                                            struct xiafs_direct **res_dir,
                                            struct xiafs_direct **res_pre)
{
    ...
    *res_dir = NULL;
    ...
    for (i = 0; i < zones; i++) {
        ...
        if (!i && (dep->d_rec_len != 12 || !dep->d_ino || dep->d_name_len != 1 ||
                   strcmp(dep->d_name, "."))) {
        ...
        while (pos < XIAFS_ZSIZE(inode->i_sb)) {
            if (dep->d_ino > inode->i_sb->u.xiafs_sb.s_ninodes || dep->d_rec_len < 12 ||
...
int xiafs_lookup(struct inode *dir, const char *name, int len, struct inode **result)
{
    int ino;
 ...
 */
static struct buffer_head *xiafs_add_entry(struct inode *dir, const char *name, int namelen,
                                           struct xiafs_direct **res_dir,
                                           struct xiafs_direct **res_pre)
{
    int i, pos, offset;
    ...
    for (;;) {
        ...
        while (offset < XIAFS_ZSIZE(dir->i_sb)) {
            if (pos >= dir->i_size) {
                ...
                if (de->d_ino > dir->i_sb->u.xiafs_sb.s_ninodes || de->d_rec_len < 12 ||
                    (char *)de + de->d_rec_len > bh->b_data + XIAFS_ZSIZE(dir->i_sb) ||
                    de->d_name_len + 8 > de->d_rec_len || de->d_name[de->d_name_len]) {
...
int xiafs_create(struct inode *dir, const char *name, int len, int mode, struct inode **result)
{
    struct inode *inode;
    struct buffer_head *bh;
...
static int empty_dir(struct inode *inode)
{
    ...
    for (i = 0; i < zones; i++) {
        ...
        if (!i) {
            ...
            if (de->d_ino != inode->i_ino || strcmp(".", de->d_name) || de->d_rec_len != 12) {
                printk("XIA-FS: bad directory (%s %d)\n", WHERE_ERR);
                brelse(bh);
                return 1;
            }
        ...
        while (offset < XIAFS_ZSIZE(inode->i_sb)) {
            de = (struct xiafs_direct *)(bh->b_data + offset);
            if (de->d_ino > inode->i_sb->u.xiafs_sb.s_ninodes || de->d_rec_len < 12 ||
...
int xiafs_symlink(struct inode *dir, const char *name, int len, const char *symname)
{
    struct xiafs_direct *de;
...
int xiafs_link(struct inode *oldinode, struct inode *dir, const char *name, int len)
{
    struct xiafs_direct *de;
    struct buffer_head *bh;
...
#define PARENT_INO(buffer) (((struct xiafs_direct *)((u_char *)(buffer) + 12))->d_ino)
 ...
 */
static int do_xiafs_rename(struct inode *old_dir, const char *old_name, int old_len,
...
try_again:
    ...
    if (!new_bh)
        goto end_rename;
    /* sanity checking */
    if ((new_inode && (new_de->d_ino != new_inode->i_ino)) || (new_de->d_ino && !new_inode) ||
 ...
 */
int xiafs_rename(struct inode *old_dir, const char *old_name, int old_len, struct inode *new_dir,
                 const char *new_name, int new_len)
{
    ...
    while (lock)
        sleep_on(&wait);
    lock = 1;
    result = do_xiafs_rename(old_dir, old_name, old_len, new_dir, new_name, new_len);
    lock = 0;
    wake_up(&wait);
    return result;
}
