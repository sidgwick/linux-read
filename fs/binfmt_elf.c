...
unsigned long *create_elf_tables(char *p, int argc, int envc, struct elfhdr *exec,
                                 unsigned int load_addr, int ibcs)
{
    ...
    if (exec) { /* Put this here for an ELF program interpreter */
        ...
        put_fs_long(3, dlinfo++);
        put_fs_long(load_addr + exec->e_phoff, dlinfo++);
        put_fs_long(4, dlinfo++);
        put_fs_long(sizeof(struct elf_phdr), dlinfo++);
        put_fs_long(5, dlinfo++);
        put_fs_long(exec->e_phnum, dlinfo++);
        put_fs_long(9, dlinfo++);
        put_fs_long((unsigned long)exec->e_entry, dlinfo++);
        put_fs_long(7, dlinfo++);
        put_fs_long(SHM_RANGE_START, dlinfo++);
        put_fs_long(8, dlinfo++);
        put_fs_long(0, dlinfo++);
        put_fs_long(6, dlinfo++);
        put_fs_long(PAGE_SIZE, dlinfo++);
        put_fs_long(0, dlinfo++);
        put_fs_long(0, dlinfo++);
    };
...
static unsigned int load_elf_interp(struct elfhdr *interp_elf_ex, struct inode *interpreter_inode)
{
    ...
    elf_phdata =
        (struct elf_phdr *)kmalloc(sizeof(struct elf_phdr) * interp_elf_ex->e_phnum, GFP_KERNEL);
    if (!elf_phdata)
        return 0xffffffff;
    ...
    for (i = 0; i < interp_elf_ex->e_phnum; i++, eppnt++)
        if (eppnt->p_type == PT_LOAD) {
            error = do_mmap(file, eppnt->p_vaddr & 0xfffff000,
    ...
    if (last_bss > len)
        do_mmap(NULL, len, last_bss - len, PROT_READ | PROT_WRITE | PROT_EXEC,
...
static unsigned int load_aout_interp(struct exec *interp_ex, struct inode *interpreter_inode)
{
    int retval;
    unsigned int elf_entry;

    current->brk = interp_ex->a_bss + (current->end_data = interp_ex->a_data +
                                                           (current->end_code = interp_ex->a_text));
    elf_entry = interp_ex->a_entry;

    if (N_MAGIC(*interp_ex) == OMAGIC) {
        do_mmap(NULL, 0, interp_ex->a_text + interp_ex->a_data, PROT_READ | PROT_WRITE | PROT_EXEC,
                MAP_FIXED | MAP_PRIVATE, 0);
        retval = read_exec(interpreter_inode, 32, (char *)0, interp_ex->a_text + interp_ex->a_data);
    } else if (N_MAGIC(*interp_ex) == ZMAGIC || N_MAGIC(*interp_ex) == QMAGIC) {
        do_mmap(NULL, 0, interp_ex->a_text + interp_ex->a_data, PROT_READ | PROT_WRITE | PROT_EXEC,
                MAP_FIXED | MAP_PRIVATE, 0);
        retval = read_exec(interpreter_inode, N_TXTOFF(*interp_ex), (char *)N_TXTADDR(*interp_ex),
                           interp_ex->a_text + interp_ex->a_data);
    } else
        retval = -1;

    if (retval >= 0)
        do_mmap(NULL, (interp_ex->a_text + interp_ex->a_data + 0xfff) & 0xfffff000,
                interp_ex->a_bss, PROT_READ | PROT_WRITE | PROT_EXEC, MAP_FIXED | MAP_PRIVATE, 0);
    if (retval < 0)
        return 0xffffffff;
    return elf_entry;
}
...
int load_elf_binary(struct linux_binprm *bprm, struct pt_regs *regs)
{
    ...
    /* First of all, some simple consistency checks */
    if (elf_ex.e_type != ET_EXEC || (elf_ex.e_machine != EM_386 && elf_ex.e_machine != EM_486) ||
    ...
    for (i = 0; i < elf_ex.e_phnum; i++) {
        if (elf_ppnt->p_type == PT_INTERP) {
            ...
            retval =
                read_exec(bprm->inode, elf_ppnt->p_offset, elf_interpreter, elf_ppnt->p_filesz);
#if 0
    ...
    if (elf_interpreter) {
        ...
        if (interp_elf_ex.e_ident[0] != 0x7f || strncmp(&interp_elf_ex.e_ident[1], "ELF", 3) != 0)
            interpreter_type &= ~INTERPRETER_ELF;
    ...
    for (i = 0; i < elf_ex.e_phnum; i++) {
        ...
        if (elf_ppnt->p_type == PT_INTERP) {
            ...
            if (interpreter_type & 1)
                elf_entry = load_aout_interp(&interp_ex, interpreter_inode);
        ...
        if (elf_ppnt->p_type == PT_LOAD) {
            error = do_mmap(file, elf_ppnt->p_vaddr & 0xfffff000,
    ...
    bprm->p =
        (unsigned long)create_elf_tables((char *)bprm->p, bprm->argc, bprm->envc,
                                         (interpreter_type == INTERPRETER_ELF ? &elf_ex : NULL),
                                         load_addr, (interpreter_type == INTERPRETER_AOUT ? 0 : 1));
    if (interpreter_type == INTERPRETER_AOUT)
    ...
    error = do_mmap(NULL, 0, 4096, PROT_READ | PROT_EXEC, MAP_FIXED | MAP_PRIVATE, 0);
...
int load_elf_library(int fd)
{
    ...
    if (elf_ex.e_type != ET_EXEC || elf_ex.e_phnum > 2 ||
        ...
        (!inode->i_op || !inode->i_op->bmap || !inode->i_op->default_file_ops->mmap)) {
        return -ENOEXEC;
    };
    ...
    elf_phdata = (struct elf_phdr *)kmalloc(sizeof(struct elf_phdr) * elf_ex.e_phnum, GFP_KERNEL);
    ...
    /* Now use mmap to map the library into memory. */
    error = do_mmap(file, elf_phdata->p_vaddr & 0xfffff000,
    ...
    if (bss > len)
        do_mmap(NULL, len, bss - len, PROT_READ | PROT_WRITE | PROT_EXEC, MAP_FIXED | MAP_PRIVATE,
                0);
    kfree(elf_phdata);
    return 0;
}
