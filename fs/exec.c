...
#define DUMP_WRITE(addr, nr)                                                                       \
    while (file.f_op->write(inode, &file, (char *)(addr), (nr)) != (nr))                           \
    goto close_coredump
...
int core_dump(long signr, struct pt_regs *regs)
{
   ...
   if we wrote the stack, but not the data area.  */
    if ((dump.u_dsize + dump.u_ssize + 1) * PAGE_SIZE > current->rlim[RLIMIT_CORE].rlim_cur)
        dump.u_dsize = 0;
    /* Make sure we have enough room to write the stack and data areas. */
    if ((dump.u_ssize + 1) * PAGE_SIZE > current->rlim[RLIMIT_CORE].rlim_cur)
        dump.u_ssize = 0;
 ...
 */
unsigned long copy_strings(int argc, char **argv, unsigned long *page, unsigned long p,
                           int from_kmem)
{
    char *tmp, *pag = NULL;
    int len, offset = 0;
...
extern int load_aout_binary(struct linux_binprm *, struct pt_regs *regs);
extern int load_aout_library(int fd);

#ifdef CONFIG_BINFMT_ELF
extern int load_elf_binary(struct linux_binprm *, struct pt_regs *regs);
extern int load_elf_library(int fd);
#endif

#ifdef CONFIG_BINFMT_COFF
extern int load_coff_binary(struct linux_binprm *, struct pt_regs *regs);
extern int load_coff_library(int fd);
#endif

/* Here are the actual binaries that will be accepted  */
struct linux_binfmt formats[] = {{load_aout_binary, load_aout_library},
#ifdef CONFIG_BINFMT_ELF
...
int load_aout_binary(struct linux_binprm *bprm, struct pt_regs *regs)
{
    ...
    if ((N_MAGIC(ex) != ZMAGIC && N_MAGIC(ex) != OMAGIC && N_MAGIC(ex) != QMAGIC) || ex.a_trsize ||
        ex.a_drsize || bprm->inode->i_size < ex.a_text + ex.a_data + ex.a_syms + N_TXTOFF(ex)) {
        return -ENOEXEC;
    }

    if (N_MAGIC(ex) == ZMAGIC && (N_TXTOFF(ex) < bprm->inode->i_sb->s_blocksize)) {
    ...
    if (N_MAGIC(ex) == OMAGIC) {
        do_mmap(NULL, 0, ex.a_text + ex.a_data, PROT_READ | PROT_WRITE | PROT_EXEC,
        ...
        if (!file->f_op || !file->f_op->mmap) {
            ...
            do_mmap(NULL, 0, ex.a_text + ex.a_data, PROT_READ | PROT_WRITE | PROT_EXEC,
                    MAP_FIXED | MAP_PRIVATE, 0);
            read_exec(bprm->inode, N_TXTOFF(ex), (char *)N_TXTADDR(ex), ex.a_text + ex.a_data);
            goto beyond_if;
        }
        error = do_mmap(file, N_TXTADDR(ex), ex.a_text, PROT_READ | PROT_EXEC,
        ...
        error =
            do_mmap(file, N_TXTADDR(ex) + ex.a_text, ex.a_data, PROT_READ | PROT_WRITE | PROT_EXEC,
...
int load_aout_library(int fd)
{
    ...
    if ((N_MAGIC(ex) != ZMAGIC && N_MAGIC(ex) != QMAGIC) || ex.a_trsize || ex.a_drsize ||
        ((ex.a_entry & 0xfff) && N_MAGIC(ex) == ZMAGIC) ||
        inode->i_size < ex.a_text + ex.a_data + ex.a_syms + N_TXTOFF(ex)) {
        return -ENOEXEC;
    }
    if (N_MAGIC(ex) == ZMAGIC && N_TXTOFF(ex) && (N_TXTOFF(ex) < inode->i_sb->s_blocksize)) {
    ...
    error = do_mmap(file, start_addr, ex.a_text + ex.a_data, PROT_READ | PROT_WRITE | PROT_EXEC,
    ...
    if (bss > len)
        do_mmap(NULL, start_addr + len, bss - len, PROT_READ | PROT_WRITE | PROT_EXEC,
