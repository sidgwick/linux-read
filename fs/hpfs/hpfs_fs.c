...
static const struct inode_operations hpfs_file_iops = {
    ...
    NULL,                                       /* readlink */
    NULL,                                       /* follow_link */
    (int (*)(struct inode *, int)) & hpfs_bmap, /* bmap */
    NULL,                                       /* truncate */
    NULL,                                       /* permission */
};
...
static int hpfs_dir_read(struct inode *inode, struct file *filp, char *buf, int count);
static int hpfs_readdir(struct inode *inode, struct file *filp, struct dirent *dirent, int count);
...
static int parse_opts(char *opts, uid_t *uid, gid_t *gid, umode_t *umask, int *lowercase,
                      int *conv);
static int check_warn(int not_ok, const char *p1, const char *p2, const char *p3);
static int zerop(void *addr, unsigned len);
static void count_dnodes(struct inode *inode, dnode_secno dno, unsigned *n_dnodes,
                         unsigned *n_subdirs);
static unsigned count_bitmap(struct super_block *s);
static unsigned count_one_bitmap(dev_t dev, secno secno);
static secno bplus_lookup(struct inode *inode, struct bplus_header *b, secno file_secno,
...
static struct hpfs_dirent *map_pos_dirent(struct inode *inode, off_t *posp,
                                          struct quad_buffer_head *qbh);
static void write_one_dirent(struct dirent *dirent, const unsigned char *name, unsigned namelen,
                             ino_t ino, int lowercase);
static dnode_secno dir_subdno(struct inode *inode, unsigned pos);
static struct hpfs_dirent *map_nth_dirent(dev_t dev, dnode_secno dno, int n,
                                          struct quad_buffer_head *qbh);
static unsigned choose_conv(unsigned char *p, unsigned len);
static unsigned convcpy_tofs(unsigned char *out, unsigned char *in, unsigned len);
static dnode_secno fnode_dno(dev_t dev, ino_t ino);
static struct fnode *map_fnode(dev_t dev, ino_t ino, struct buffer_head **bhp);
static struct anode *map_anode(dev_t dev, unsigned secno, struct buffer_head **bhp);
static struct dnode *map_dnode(dev_t dev, unsigned secno, struct quad_buffer_head *qbh);
static void *map_sector(dev_t dev, unsigned secno, struct buffer_head **bhp);
static void *map_4sectors(dev_t dev, unsigned secno, struct quad_buffer_head *qbh);
...
struct super_block *hpfs_read_super(struct super_block *s, void *options, int silent)
{
    ...
    if (bootblock->magic != 0xaa55 || superblock->magic != SB_MAGIC ||
        spareblock->magic != SP_MAGIC || bootblock->sig_28h != 0x28 ||
    ...
    dubious |=
        check_warn(spareblock->dirty != 0, "`Improperly stopped'", "flag is set", "run CHKDSK");
    dubious |=
        check_warn(spareblock->n_spares_used != 0, "Spare blocks", "may be in use", "run CHKDSK");
    ...
    dubious |= check_warn((spareblock->n_dnode_spares != spareblock->n_dnode_spares_free),
                          "Spare dnodes", "may be in use", "run CHKDSK");
    dubious |= check_warn(superblock->zero1 != 0, "#1", "unknown word nonzero", "investigate");
    dubious |= check_warn(superblock->zero3 != 0, "#3", "unknown word nonzero", "investigate");
    dubious |= check_warn(superblock->zero4 != 0, "#4", "unknown word nonzero", "investigate");
    dubious |= check_warn(!zerop(superblock->zero5, sizeof superblock->zero5), "#5",
                          "unknown word nonzero", "investigate");
    dubious |= check_warn(!zerop(superblock->zero6, sizeof superblock->zero6), "#6",
                          "unknown word nonzero", "investigate");
...
static int parse_opts(char *opts, uid_t *uid, gid_t *gid, umode_t *umask, int *lowercase, int *conv)
{
    char *p, *rhs;
...
static void hpfs_read_inode(struct inode *inode)
{
    ...
    if (inode->i_ino == 0 || ino_secno(inode->i_ino) >= inode->i_sb->s_hpfs_fs_size) {
...
static int hpfs_file_read(struct inode *inode, struct file *filp, char *buf, int count)
{
    unsigned q, r, n, n0;
...
static int hpfs_lookup(struct inode *dir, const char *name, int len, struct inode **result)
{
    struct quad_buffer_head qbh;
    struct hpfs_dirent *de;
    struct inode *inode;
    ...
    *result = 0;
    ...
    if (name[0] == '.' && len == 1)
        ...
        de = map_dirent(dir, fnode_dno(dir->i_dev, dir->i_hpfs_parent_dir), "\001\001", 2, &qbh);
    else
...
static inline int memcasecmp(const unsigned char *s1, const unsigned char *s2, unsigned n)
{
    int t;
...
static int hpfs_readdir(struct inode *inode, struct file *filp, struct dirent *dirent,
                        int likely_story)
{
    struct quad_buffer_head qbh;
    struct hpfs_dirent *de;
    ...
    if (inode == 0 || inode->i_sb == 0 || !S_ISDIR(inode->i_mode))
        return -EBADF;
