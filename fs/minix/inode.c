...
static void minix_commit_super(struct super_block *sb, struct minix_super_block *ms)
{
...
static struct super_operations minix_sops = {
    minix_read_inode,  NULL,         minix_write_inode, minix_put_inode, minix_put_super,
    minix_write_super, minix_statfs, minix_remount};
...
int minix_remount(struct super_block *sb, int *flags, char *data)
{
    ...
    if ((*flags & MS_RDONLY) == (sb->s_flags & MS_RDONLY))
        return 0;
    if (*flags & MS_RDONLY) {
        if (ms->s_state & MINIX_VALID_FS || !(sb->u.minix_sb.s_mount_state & MINIX_VALID_FS))
            return 0;
...
void minix_read_inode(struct inode *inode)
{
    ...
    raw_inode = ((struct minix_inode *)bh->b_data) + (ino - 1) % MINIX_INODES_PER_BLOCK;
