...
static int sync_iblock(struct inode *inode, unsigned short *iblock, struct buffer_head **bh,
                       int wait)
{
    int rc;
    unsigned short tmp;
...
static int sync_indirect(struct inode *inode, unsigned short *iblock, int wait)
{
    ...
    for (i = 0; i < addr_per_block; i++) {
        rc = sync_block(inode, ((unsigned short *)ind_bh->b_data) + i, wait);
        if (rc > 0)
            break;
        if (rc)
            err = rc;
...
static int sync_dindirect(struct inode *inode, unsigned short *diblock, int wait)
{
    int i;
    ...
    for (i = 0; i < addr_per_block; i++) {
        rc = sync_indirect(inode, ((unsigned short *)dind_bh->b_data) + i, wait);
        if (rc > 0)
            break;
        if (rc)
            err = rc;
