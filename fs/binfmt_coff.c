...
static int preload_library(struct linux_binprm *exe_bprm, COFF_SCNHDR *sect, struct file *fp);

static int load_object(struct linux_binprm *bprm, struct pt_regs *regs, int lib_ok);
 ...
 */
            ...
            bprm->p = (unsigned long)create_tables((char *)bprm->p, bprm->argc, bprm->envc, 1);
            /*
 ...
 */
            ...
            current->start_brk = current->brk = bss_vaddr + bss_size;
            ...
            regs->esp = current->start_stack = bprm->p;
        }
        /*
 *   Map the text pages
 */
...
#ifdef COFF_DEBUG
        printk(".text: vaddr = %d, size = %d, scnptr = %d\n", text_vaddr, text_size, text_scnptr);
#endif
        status = do_mmap(fp, text_vaddr & PAGE_MASK, text_size + (text_vaddr & ~PAGE_MASK),
                         PROT_READ | PROT_EXEC, MAP_FIXED | MAP_SHARED, text_scnptr & PAGE_MASK);
 ...
 */
        if (status >= 0 && data_size != 0) {
            ...
            printk(".data: vaddr = %d, size = %d, scnptr = %d\n", data_vaddr, data_size,
                   data_scnptr);
#endif
            status = do_mmap(fp, data_vaddr & PAGE_MASK, data_size + (data_vaddr & ~PAGE_MASK),
...
#endif
            zeromap_page_range(PAGE_ALIGN(bss_vaddr), PAGE_ALIGN(bss_size), PAGE_COPY);
...
static int preload_this_library(struct linux_binprm *exe_bprm, char *lib_name)
{
    int status;
    int old_fs = get_fs();
/*
 ...
 */
            if ((unsigned)header_size >= (unsigned)nbytes || entry_size <= 0 ||
 ...
 */
                ...
                status = preload_this_library(exe_bprm, &((char *)phdr)[header_size]);
#ifdef COFF_DEBUG
 ...
 */
    bprm = (struct linux_binprm *)kmalloc(sizeof(struct linux_binprm), GFP_KERNEL);
    if (0 == bprm) {
#ifdef COFF_DEBUG
        printk("kmalloc failed\n");
#endif
