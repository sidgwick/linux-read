...
static int nfs_dir_read(struct inode *, struct file *filp, char *buf, int count);
static int nfs_readdir(struct inode *, struct file *, struct dirent *, int);
static int nfs_lookup(struct inode *dir, const char *name, int len, struct inode **result);
...
static int nfs_symlink(struct inode *inode, const char *name, int len, const char *symname);
static int nfs_link(struct inode *oldinode, struct inode *dir, const char *name, int len);
static int nfs_mknod(struct inode *dir, const char *name, int len, int mode, int rdev);
...
static int nfs_dir_read(struct inode *inode, struct file *filp, char *buf, int count)
{
    return -EISDIR;
}
...
static int nfs_readdir(struct inode *inode, struct file *filp, struct dirent *dirent, int count)
{
    static int c_dev = 0;
    static int c_ino;
    static int c_size;
    ...
    if (c_entry == NULL) {
        ...
        for (i = 0; i < NFS_READDIR_CACHE_SIZE; i++) {
            c_entry[i].name = (char *)kmalloc(NFS_MAXNAMLEN + 1, GFP_KERNEL);
        }
    }
    entry = NULL;
    ...
    if (!entry) {
        result = nfs_proc_readdir(NFS_SERVER(inode), NFS_FH(inode), filp->f_pos,
                                  NFS_READDIR_CACHE_SIZE, c_entry);
        if (result < 0) {
            c_dev = 0;
            return result;
        }
...
static int nfs_lookup_cache_lookup(struct inode *dir, const char *filename, struct nfs_fh *fhandle,
                                   struct nfs_fattr *fattr)
{
...
static void nfs_lookup_cache_add(struct inode *dir, const char *filename, struct nfs_fh *fhandle,
                                 struct nfs_fattr *fattr)
{
    ...
    if (fattr->size == -1 || fattr->uid == -1 || fattr->gid == -1 || fattr->atime.seconds == -1 ||
        fattr->mtime.seconds == -1)
        return;
    if (!(entry = nfs_lookup_cache_index(dir, filename))) {
    ...
    entry->expiration_date =
        jiffies + (S_ISDIR(fattr->mode) ? NFS_SERVER(dir)->acdirmax : NFS_SERVER(dir)->acregmax);
}

static void nfs_lookup_cache_remove(struct inode *dir, struct inode *inode, const char *filename)
{
    struct nfs_lookup_cache_entry *entry;
    int dev;
    int fileid;
...
static void nfs_lookup_cache_refresh(struct inode *file, struct nfs_fattr *fattr)
{
    struct nfs_lookup_cache_entry *entry;
    int dev = file->i_dev;
...
static int nfs_lookup(struct inode *dir, const char *__name, int len, struct inode **result)
{
    struct nfs_fh fhandle;
    struct nfs_fattr fattr;
    ...
    *result = NULL;
    ...
    if ((NFS_SERVER(dir)->flags & NFS_MOUNT_NOAC) ||
        ...
        if ((error = nfs_proc_lookup(NFS_SERVER(dir), NFS_FH(dir), name, &fhandle, &fattr))) {
            iput(dir);
            return error;
        }
...
static int nfs_create(struct inode *dir, const char *name, int len, int mode, struct inode **result)
{
    ...
    *result = NULL;
    ...
    if ((error = nfs_proc_create(NFS_SERVER(dir), NFS_FH(dir), name, &sattr, &fhandle, &fattr))) {
        iput(dir);
        return error;
    }
    if (!(*result = nfs_fhget(dir->i_sb, &fhandle, &fattr))) {
...
static int nfs_mknod(struct inode *dir, const char *name, int len, int mode, int rdev)
{
    ...
    sattr.atime.seconds = sattr.mtime.seconds = (unsigned)-1;
    error = nfs_proc_create(NFS_SERVER(dir), NFS_FH(dir), name, &sattr, &fhandle, &fattr);
    if (!error)
        nfs_lookup_cache_add(dir, name, &fhandle, &fattr);
    iput(dir);
    return error;
...
static int nfs_mkdir(struct inode *dir, const char *name, int len, int mode)
{
    ...
    sattr.uid = sattr.gid = sattr.size = (unsigned)-1;
    sattr.atime.seconds = sattr.mtime.seconds = (unsigned)-1;
    error = nfs_proc_mkdir(NFS_SERVER(dir), NFS_FH(dir), name, &sattr, &fhandle, &fattr);
    if (!error)
        nfs_lookup_cache_add(dir, name, &fhandle, &fattr);
    iput(dir);
    return error;
...
static int nfs_symlink(struct inode *dir, const char *name, int len, const char *symname)
{
    struct nfs_sattr sattr;
    int error;
    ...
    sattr.uid = sattr.gid = sattr.size = (unsigned)-1;
    sattr.atime.seconds = sattr.mtime.seconds = (unsigned)-1;
    error = nfs_proc_symlink(NFS_SERVER(dir), NFS_FH(dir), name, symname, &sattr);
    iput(dir);
    return error;
}
...
static int nfs_link(struct inode *oldinode, struct inode *dir, const char *name, int len)
{
    ...
    error = nfs_proc_link(NFS_SERVER(oldinode), NFS_FH(oldinode), NFS_FH(dir), name);
    if (!error)
                      ...
                      struct inode *new_dir, const char *new_name, int new_len)
{
    ...
    error =
        nfs_proc_rename(NFS_SERVER(old_dir), NFS_FH(old_dir), old_name, NFS_FH(new_dir), new_name);
    if (!error) {
