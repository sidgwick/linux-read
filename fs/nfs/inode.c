...
static struct super_operations nfs_sops = {NULL,              /* read inode */
...
void nfs_statfs(struct super_block *sb, struct statfs *buf)
{
    ...
    error = nfs_proc_statfs(&sb->u.nfs_sb.s_server, &sb->u.nfs_sb.s_root, &res);
    if (error) {
...
struct inode *nfs_fhget(struct super_block *sb, struct nfs_fh *fhandle, struct nfs_fattr *fattr)
{
    ...
    if (!fattr) {
        error = nfs_proc_getattr(&sb->u.nfs_sb.s_server, fhandle, &newfattr);
        if (error) {
...
int nfs_notify_change(int flags, struct inode *inode)
{
    ...
    error = nfs_proc_setattr(NFS_SERVER(inode), NFS_FH(inode), &sattr, &fattr);
    if (!error)
        nfs_refresh_inode(inode, &fattr);
    inode->i_dirt = 0;
    return error;
