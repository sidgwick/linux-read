...
static int nfs_file_read(struct inode *inode, struct file *file, char *buf, int count)
{
    int result;
    int hunk;
    int i;
    ...
    for (i = 0; i < count; i += n) {
        ...
        if (hunk > n)
            hunk = n;
        result = nfs_proc_read(NFS_SERVER(inode), NFS_FH(inode), pos, hunk, data, &fattr);
        if (result < 0) {
            kfree_s(data, n);
            return result;
        }
...
static int nfs_file_write(struct inode *inode, struct file *file, char *buf, int count)
{
    int result;
    int hunk;
    int i;
    ...
    for (i = 0; i < count; i += n) {
        ...
        result = nfs_proc_write(NFS_SERVER(inode), NFS_FH(inode), pos, hunk, data, &fattr);
        if (result < 0) {
            kfree_s(data, n);
            return result;
        }
