...
static int do_nfs_rpc_call(struct nfs_server *server, int *start, int *end)
{
    ...
    current->blocked |=
        ...
        | _S(SIGSTOP)
          ...
          | ((server->flags & NFS_MOUNT_INTR)
                 ? ((current->sigaction[SIGINT - 1].sa_handler == SIG_DFL ? _S(SIGINT) : 0) |
                    (current->sigaction[SIGQUIT - 1].sa_handler == SIG_DFL ? _S(SIGQUIT) : 0))
                 : 0));
    fs = get_fs();
    set_fs(get_ds());
    ...
    re_select:
        ...
        if (!select(inode, file, SEL_IN, &wait_table) && !select(inode, file, SEL_IN, NULL)) {
        ...
        current->state = TASK_RUNNING;
        addrlen = 0;
        result = sock->ops->recvfrom(sock, (void *)start, PAGE_SIZE, 1, 0, NULL, &addrlen);
        if (result < 0) {
            if (result == -EAGAIN) {
#if 0
