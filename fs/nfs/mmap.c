...
extern int share_page(struct vm_area_struct *area, struct task_struct *tsk, struct inode *inode,
                      unsigned long address, unsigned long error_code, unsigned long newpage);

extern unsigned long put_page(struct task_struct *tsk, unsigned long page, unsigned long address,
                              int prot);
...
int nfs_mmap(struct inode *inode, struct file *file, unsigned long addr, size_t len, int prot,
             unsigned long off)
{
...
static void nfs_file_mmap_nopage(int error_code, struct vm_area_struct *area, unsigned long address)
{
    ...
    for (i = 0; i < (PAGE_SIZE - clear); i += n) {
        ...
        if (hunk > n)
            hunk = n;
        result =
            nfs_proc_read(NFS_SERVER(inode), NFS_FH(inode), pos, hunk, (char *)(page + i), &fattr);
        if (result < 0)
            break;
        pos += result;
        if (result < n) {
