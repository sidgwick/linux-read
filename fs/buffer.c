...
repeat:
    ...
    for (i = nr_buffers * 2; i-- > 0; bh = bh->b_next_free) {
        if (dev && bh->b_dev != dev)
            continue;
#ifdef NO_COMPILE /* Disable bad-block debugging code */
        if (bh->b_req && !bh->b_lock && !bh->b_dirt && !bh->b_uptodate)
...
static unsigned long check_aligned(struct buffer_head *first, unsigned long address, dev_t dev,
                                   int *b, int size)
{
...
static unsigned long try_to_load_aligned(unsigned long address, dev_t dev, int b[], int size)
{
...
int shrink_buffers(unsigned int priority)
{
    ...
    for (; i-- > 0; bh = bh->b_next_free) {
        if (bh->b_count || (priority >= 5 && mem_map[MAP_NR((unsigned long)bh->b_data)] > 1)) {
            put_last_free(bh);
            continue;
        }
        if (!bh->b_this_page)
