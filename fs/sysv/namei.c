 ...
 */
static struct buffer_head *sysv_find_entry(struct inode *dir, const char *name, int namelen,
                                           struct sysv_dir_entry **res_dir)
{
    ...
    *res_dir = NULL;
    ...
    while (pos < dir->i_size) {
        ...
        if (sysv_match(namelen, name, *res_dir = (struct sysv_dir_entry *)(bh_data + offset)))
            return bh;
        pos += SYSV_DIRSIZE;
        offset += SYSV_DIRSIZE;
...
int sysv_lookup(struct inode *dir, const char *name, int len, struct inode **result)
{
    int ino;
 ...
 */
static int sysv_add_entry(struct inode *dir, const char *name, int namelen,
                          struct buffer_head **res_buf, struct sysv_dir_entry **res_dir)
{
    struct super_block *sb;
    int i;
    unsigned long pos, block, offset; /* pos = block * block_size + offset */
...
int sysv_create(struct inode *dir, const char *name, int len, int mode, struct inode **result)
{
    int error;
    struct inode *inode;
    struct buffer_head *bh;
...
#define PARENT_INO(buffer) (((struct sysv_dir_entry *)((buffer) + 1 * SYSV_DIRSIZE))->inode)
 ...
 */
int sysv_rename(struct inode *old_dir, const char *old_name, int old_len, struct inode *new_dir,
                const char *new_name, int new_len)
{
    ...
    while (lock)
        sleep_on(&wait);
    lock = 1;
    result = do_sysv_rename(old_dir, old_name, old_len, new_dir, new_name, new_len);
    lock = 0;
    wake_up(&wait);
    return result;
}
