...
void sysv_free_block(struct super_block *sb, unsigned int block)
{
    ...
    if (*sb->sv_sb_flc_count == sb->sv_flc_size) {
        ...
        if (sb->sv_block_size_ratio_bits > 0) /* block_size < BLOCK_SIZE ? */
            bh = bread(sb->s_dev, (block >> sb->sv_block_size_ratio_bits) + sb->sv_block_base,
                       BLOCK_SIZE);
        else
            bh = getblk(sb->s_dev, (block >> sb->sv_block_size_ratio_bits) + sb->sv_block_base,
                        BLOCK_SIZE);
        if (!bh) {
        ...
        case FSTYPE_COH:
            ...
            break;
        default:
            panic("sysv_free_block: invalid fs type\n");
        }
     ...
     */
        if (*sb->sv_sb_flc_count == 0) {          /* Applies only to Coherent FS */
            ...
            if (sb->sv_block_size_ratio_bits == 0) { /* block_size == BLOCK_SIZE ? */
                ...
                bh = get_hash_table(sb->s_dev,
                                    (block >> sb->sv_block_size_ratio_bits) + sb->sv_block_base,
                                    BLOCK_SIZE);
                if (bh)
                    bh->b_dirt = 0;
                brelse(bh);
...
int sysv_new_block(struct super_block *sb)
{
    ...
    if (*sb->sv_sb_flc_count == 0) { /* the last block continues the free list */
        ...
        case FSTYPE_COH:
            ...
            break;
        default:
            panic("sysv_new_block: invalid fs type\n");
        }
    ...
    if (*sb->sv_sb_flc_count > 0) {
        ...
        while (1) {
            ...
            case FSTYPE_COH:
                ...
                break;
            default:
                panic("sysv_count_free_blocks: invalid fs type\n");
            }
            if (*flc_count > sb->sv_flc_size) {
