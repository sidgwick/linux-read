...
void sysv_free_inode(struct inode *inode)
{
    ...
    if (!(bh = sysv_bread(sb, inode->i_dev,
                          sb->sv_firstinodezone + ((ino - 1) >> sb->sv_inodes_per_block_bits),
                          &bh_data))) {
...
struct inode *sysv_new_inode(const struct inode *dir)
{
    ...
    if ((*sb->sv_sb_fic_count == 0) ||
        ...
        for (i = 0, ino = SYSV_ROOT_INO + 1, block = sb->sv_firstinodezone, j = SYSV_ROOT_INO;
             i < sb->sv_fic_size && block < sb->sv_firstdatazone; block++, j = 0) {
...
unsigned long sysv_count_free_inodes(struct super_block *sb)
{
    ...
    /* bh : buffer for block                 */
    /* raw_inode : pointer to inode ino in the block     */
    for (block = sb->sv_firstinodezone, j = SYSV_ROOT_INO; block < sb->sv_firstdatazone;
         block++, j = 0) {
        if (!(bh = sysv_bread(sb, sb->s_dev, block, &bh_data))) {
