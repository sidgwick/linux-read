...
static int sync_block(struct inode *inode, unsigned long *blockp, int convert, int wait)
{
    ...
    if (!block)
        return 0;
    sb = inode->i_sb;
    bh = get_hash_table(inode->i_dev, (block >> sb->sv_block_size_ratio_bits) + sb->sv_block_base,
                        BLOCK_SIZE);
    if (!bh)
        return 0;
    if (*blockp != tmp) {
...
static int sync_indirect(struct inode *inode, unsigned long *iblockp, int convert, int wait)
{
    ...
    for (i = 0; i < sb->sv_ind_per_block; i++) {
        rc = sync_block(inode, ((unsigned long *)ind_bh_data) + i, sb->sv_convert, wait);
        if (rc > 0)
            break;
        if (rc)
            err = rc;
...
static int sync_dindirect(struct inode *inode, unsigned long *diblockp, int convert, int wait)
{
    int i;
    ...
    sb = inode->i_sb;
    for (i = 0; i < sb->sv_ind_per_block; i++) {
        rc = sync_indirect(inode, ((unsigned long *)dind_bh_data) + i, sb->sv_convert, wait);
        if (rc > 0)
            break;
        if (rc)
            err = rc;
...
static int sync_tindirect(struct inode *inode, unsigned long *tiblockp, int convert, int wait)
{
    int i;
    ...
    sb = inode->i_sb;
    for (i = 0; i < sb->sv_ind_per_block; i++) {
        rc = sync_dindirect(inode, ((unsigned long *)tind_bh_data) + i, sb->sv_convert, wait);
        if (rc > 0)
            break;
        if (rc)
            err = rc;
