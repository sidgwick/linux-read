...
static struct super_operations sysv_sops = {
    sysv_read_inode, sysv_notify_change, sysv_write_inode, sysv_put_inode,
    sysv_put_super,  sysv_write_super,   sysv_statfs,      NULL};
...
static void detected_bs512(struct super_block *sb)
{
    ...
    sb->sv_toobig_block = 10 + (sb->sv_ind_per_block = 512 / 4) +
    ...
    sb->sv_ind_per_block_2_bits = 2 * (sb->sv_ind_per_block_bits = 9 - 2);
}
...
static void detected_bs1024(struct super_block *sb)
{
    ...
    sb->sv_toobig_block = 10 + (sb->sv_ind_per_block = 1024 / 4) +
    ...
    sb->sv_ind_per_block_2_bits = 2 * (sb->sv_ind_per_block_bits = 10 - 2);
}
...
struct super_block *sysv_read_super(struct super_block *sb, void *data, int silent)
{
    ...
    if ((512 != sizeof(struct sysv4_super_block)) || (512 != sizeof(struct sysv2_super_block)))
...
repeat:
    tmp = *p;
    if (tmp) {
        result = getblk(inode->i_dev, (tmp >> sb->sv_block_size_ratio_bits) + sb->sv_block_base,
                        BLOCK_SIZE);
        if (tmp == *p) {
    ...
    if (!tmp)
        return NULL;
    result =
        getblk(inode->i_dev, (tmp >> sb->sv_block_size_ratio_bits) + sb->sv_block_base, BLOCK_SIZE);
    if (*p) {
...
repeat:
    ...
    if (tmp) {
        result = getblk(bh->b_dev, (block >> sb->sv_block_size_ratio_bits) + sb->sv_block_base,
                        BLOCK_SIZE);
        if (tmp == *p) {
            brelse(bh);
            *start =
                result->b_data + ((block & sb->sv_block_size_ratio_1) << sb->sv_block_size_bits);
            return result;
        }
        brelse(result);
        goto repeat;
    ...
    result =
        getblk(bh->b_dev, (block >> sb->sv_block_size_ratio_bits) + sb->sv_block_base, BLOCK_SIZE);
    if (*p) {
...
struct buffer_head *sysv_getblk(struct inode *inode, unsigned int block, int create, char **start)
{
    struct super_block *sb = inode->i_sb;
    struct buffer_head *bh;
    ...
    if (block < sb->sv_ind_per_block_3) {
        ...
        bh = block_getblk(inode, bh, (block >> sb->sv_ind_per_block_bits) & sb->sv_ind_per_block_1,
                          create, start);
        return block_getblk(inode, bh, block & sb->sv_ind_per_block_1, create, start);
    }
    if ((int)block < 0) {
...
{
    return (unsigned long)(*(unsigned short *)p) | (unsigned long)(*(unsigned char *)(p + 2)) << 16;
}
...
{
    return (unsigned long)(*(unsigned char *)p) << 16 | (unsigned long)(*(unsigned short *)(p + 1));
}
...
void sysv_read_inode(struct inode *inode)
{
    ...
    else if (sb->sv_convert)
        for (block = 0; block < 10 + 1 + 1 + 1; block++)
            inode->u.sysv_i.i_data[block] = coh_read3byte(&raw_inode->i_a.i_addb[3 * block]);
    else
        for (block = 0; block < 10 + 1 + 1 + 1; block++)
            inode->u.sysv_i.i_data[block] = read3byte(&raw_inode->i_a.i_addb[3 * block]);
    brelse(bh);
