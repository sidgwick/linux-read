...
static int sysv_readdir(struct inode *inode, struct file *filp, struct dirent *dirent, int count)
{
            ...
            if (de->inode) {
                ...
                if (i) {
                    if (de->inode > inode->i_sb->sv_ninodes)
                        printk("sysv_readdir: Bad inode number on dev 0x%04x, ino %ld, offset "
                               "0x%04lx: %d is out of range\n",
