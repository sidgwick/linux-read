 ...
 */
static int coh_trunc_indirect(struct inode *inode, unsigned long blocks, unsigned long *p,
                              int convert, unsigned char *dirt)
{
 ...
 * from_coh_ulong(*p) if convert=1, *p if convert=0.
 */
static int coh_trunc_dindirect(struct inode *inode, unsigned long blocks, unsigned long *p,
                               int convert, unsigned char *dirt)
{
    struct super_block *sb = inode->i_sb;
...
static int trunc_indirect(struct inode *inode, unsigned long offset, unsigned long *p, int convert,
                          unsigned char *dirt)
{
...
static int trunc_dindirect(struct inode *inode, unsigned long offset, unsigned long *p, int convert,
                           unsigned char *dirt)
{
    unsigned long indtmp, indblock;
    struct super_block *sb;
    ...
    if (inode->i_size < offset)
        ...
        i = (inode->i_size - offset + IND_PER_BLOCK * BLOCK_SIZE - 1) /
            (IND_PER_BLOCK * BLOCK_SIZE);
    for (; i < IND_PER_BLOCK; i++) {
        ind = ((sysv_zone_t *)indbh->b_data) + i;
        block = tmp = *ind;
        if (sb->sv_convert)
        ...
        if (!block)
            continue;
        retry |=
            trunc_indirect(inode, offset + i * IND_PER_BLOCK, ind, sb->sv_convert, &indbh->b_dirt);
    }
    for (i = 0; i < IND_PER_BLOCK; i++)
        if (((sysv_zone_t *)indbh->b_data)[i])
            goto done;
...
static int trunc_tindirect(struct inode *inode, unsigned long offset, unsigned long *p, int convert,
                           unsigned char *dirt)
{
    unsigned long indtmp, indblock;
    struct super_block *sb;
    ...
    if (inode->i_size < offset)
        ...
        i = (inode->i_size - offset + IND_PER_BLOCK * IND_PER_BLOCK * BLOCK_SIZE - 1) /
            (IND_PER_BLOCK * IND_PER_BLOCK * BLOCK_SIZE);
    for (; i < IND_PER_BLOCK; i++) {
        ind = ((sysv_zone_t *)indbh->b_data) + i;
        block = tmp = *ind;
        if (sb->sv_convert)
        ...
        if (!block)
            continue;
        retry |= trunc_dindirect(inode, offset + i * IND_PER_BLOCK * IND_PER_BLOCK, ind,
                                 sb->sv_convert, &indbh->b_dirt);
    }
    for (i = 0; i < IND_PER_BLOCK; i++)
        if (((sysv_zone_t *)indbh->b_data)[i])
            goto done;
...
static int trunc_all(struct inode *inode)
{
    return trunc_direct(inode) |
           ...
           trunc_dindirect(inode, (10 + IND_PER_BLOCK) * BLOCK_SIZE, &inode->u.sysv_i.i_data[11], 0,
                           &inode->i_dirt) |
           trunc_tindirect(inode, (10 + IND_PER_BLOCK + IND_PER_BLOCK * IND_PER_BLOCK) * BLOCK_SIZE,
                           &inode->u.sysv_i.i_data[12], 0, &inode->i_dirt);
}
