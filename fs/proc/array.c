...
static int get_loadavg(char *buffer)
{
    ...
    return sprintf(buffer, "%d.%02d %d.%02d %d.%02d\n", LOAD_INT(a), LOAD_FRAC(a), LOAD_INT(b),
                   LOAD_FRAC(b), LOAD_INT(c), LOAD_FRAC(c));
}
...
static int get_kstat(char *buffer)
{
    return sprintf(buffer,
                   ...
                   kstat.pgpgin, kstat.pgpgout, kstat.pswpin, kstat.pswpout, kstat.interrupts,
                   kstat.context_swtch, xtime.tv_sec - jiffies / HZ);
}
...
static unsigned long get_wchan(struct task_struct *p)
{
    ...
    do {
        ...
        if ((void *)eip != sleep_on && (void *)eip != interruptible_sleep_on)
            return eip;
...
static int get_stat(int pid, char *buffer)
{
    ...
    if (tty_pgrp > 0 && tty_table[tty_pgrp])
        ...
        tty_pgrp = -1;
    return sprintf(
        buffer, "%d (%s) %c %d %d %d %d %d %lu %lu \
%lu %lu %lu %ld %ld %ld %ld %ld %ld %lu %lu %ld %lu %u %u %lu %lu %lu %lu %lu %lu \
%lu %lu %lu %lu\n",
        pid, (*p)->comm, state, (*p)->p_pptr->pid, (*p)->pgrp, (*p)->session, (*p)->tty, tty_pgrp,
        (*p)->flags, (*p)->min_flt, (*p)->cmin_flt, (*p)->maj_flt, (*p)->cmaj_flt, (*p)->utime,
        (*p)->stime, (*p)->cutime, (*p)->cstime, (*p)->counter, /* this is the kernel priority ---
                   subtract 30 in your user-level program. */
        (*p)->priority,                                         /* this is the nice value ---
                   subtract 15 in your user-level program. */
        (*p)->timeout, (*p)->it_real_value, (*p)->start_time, vsize,
        (*p)->rss, /* you might want to shift this left 3 */
        (*p)->rlim[RLIMIT_RSS].rlim_cur, (*p)->start_code, (*p)->end_code, (*p)->start_stack, esp,
        eip, (*p)->signal, (*p)->blocked, sigignore, sigcatch, wchan);
}
    ...
    for (map = (*p)->mmap; map != NULL; map = map->vm_next) {
        ...
        *cp++ = 0;
        ...
        sz += sprintf(buf + sz, "%08lx-%08lx %s %08lx %02x:%02x %lu\n", map->vm_start, map->vm_end,
                      str, map->vm_offset, MAJOR(dev), MINOR(dev), ino);
        if (sz > end) {
