...
/* forward references */
static int proc_readnet(struct inode *inode, struct file *file, char *buf, int count);
static int proc_readnetdir(struct inode *, struct file *, struct dirent *, int);
...
static struct proc_dir_entry net_dir[] = {{1, 2, ".."},
                                          {8, 1, "."},
                                          {128, 4, "unix"}
#ifdef CONFIG_INET
                                          ,
...
static int proc_lookupnet(struct inode *dir, const char *name, int len, struct inode **result)
{
    unsigned int ino;
    int i;
...
static int proc_readnetdir(struct inode *inode, struct file *filp, struct dirent *dirent, int count)
{
