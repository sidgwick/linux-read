...
static int proc_lookupfd(struct inode *dir, const char *name, int len, struct inode **result)
{
    ...
    *result = NULL;
    ...
    if (!len || (name[0] == '.' && (len == 1 || (name[1] == '.' && len == 2)))) {
        if (len < 2) {
            *result = dir;
            return 0;
        }
...
static int proc_readfd(struct inode *inode, struct file *filp, struct dirent *dirent, int count)
{
    struct task_struct *p;
