...
int sys_shmctl(int shmid, int cmd, struct shmid_ds *buf)
{
    ...
    case IPC_STAT:
        ...
        break;
    case IPC_SET:
        if (suser() || current->euid == shp->shm_perm.uid || current->euid == shp->shm_perm.cuid) {
            ipcp->uid = tbuf.shm_perm.uid;
            ipcp->gid = tbuf.shm_perm.gid;
            ipcp->mode = (ipcp->mode & ~S_IRWXUGO) | (tbuf.shm_perm.mode & S_IRWXUGO);
            shp->shm_ctime = CURRENT_TIME;
            break;
        }
        return -EPERM;
...
static int shm_map(struct shm_desc *shmd, int remap)
{
    ...
    for (tmp = shmd->start; tmp < shmd->end; tmp += PAGE_SIZE, shm_sgn += (1 << SHM_IDX_SHIFT)) {
        page_table = PAGE_DIR_OFFSET(page_dir, tmp);
        page_table = (ulong *)(PAGE_MASK & *page_table);
        page_table += (tmp >> PAGE_SHIFT) & (PTRS_PER_PAGE - 1);
        *page_table = shm_sgn;
...
int sys_shmat(int shmid, char *shmaddr, int shmflg, ulong *raddr)
{
    ...
    if (shmflg & SHM_REMAP)
        for (shmd = current->shm; shmd; shmd = shmd->task_next) {
            if (addr >= shmd->start && addr < shmd->end)
                return -EINVAL;
            if (addr + shp->shm_segsz >= shmd->start && addr + shp->shm_segsz < shmd->end)
                return -EINVAL;
        }
    ...
    shmd->shm_sgn =
        (SHM_SWP_TYPE << 1) | (id << SHM_ID_SHIFT) | (shmflg & SHM_RDONLY ? SHM_READ_ONLY : 0);
    shmd->start = addr;
