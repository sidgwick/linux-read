...
no_error_code:
    ...
    push %es
    push %fs
    pushl $0        # 造一个 "error code"
    lea 44(%esp),%edx # EDX = addr(EIP), 这里也是中断发生的时刻原始的栈顶(即当时 ESP 的值)
    pushl %edx # 原始 ESP 入栈
    movl $0x10,%edx # DS=ES=FS = 4GB 数据段
    mov %dx,%ds
    mov %dx,%es
    mov %dx,%fs
    call *%eax # 调用真正的处理函数(注意看栈上面: EIP, 0)
    addl $8,%esp # 恢复栈
    pop %fs
    pop %es
    pop %ds
...
# 时钟中断 int 0x20(timer_interrupt)在kernel/sys_call.s
# 系统调用 int 0x80(system_call)在kernel/sys_call.s
