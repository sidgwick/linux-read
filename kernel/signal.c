...
int sys_sigsuspend(int restart, unsigned long old_mask, unsigned long set)
{
    ...
    * 并把进程的阻塞码替换成 set. 然后调用 pause 让进程睡眠, 等待信号的到来. 当进程
    * 收到一个信号时, pause 就会返回, 并且进程会去执行信号处理函数, 然后本调用
    * 返回 -ERESTARTNOINTR 码退出, 这个返回码说明在处理完信号后要求返回到本系统
    * 调用中继续运行, 即本系统调用不会被中断 */
 ...
 *                后恢复系统调用返回时几个寄存器的原有值以及系统调用的返回值, 就好
 *                象系统调用没有执行过信号处理程序而直接返回到用户程序一样.
 * 函数返回原信号句柄 */
int sys_signal(int signum, long handler, long restorer)
{
	struct sigaction tmp;

    /* 首先验证信号值在有效范围(1-32)内, 并且不得是信号 SIGKILL 或者 SIGSTOP, 因为这
     * 两个信号不能被进程捕获 */
	...
	unsigned long * esp, long ss)
{
	...
	long old_eip=eip; /* 这个 EIP 是在 int0x80 的时候压栈的那个 EIP, 是用户任务的下一条指令 */
     ...
     * 函数 fs/pipe.c 中管道当前读数据但没有读到任何数据等情况下, 进程收到了任何一个
     * 非阻塞的信号, 则都会以 -ERESTARTSYS 返回值返回. 它表示进程可以被中断, 但是在
     * 继续执行后会重新启动系统调用.
     *
     * 返回码 -ERESTARTNOINTR 说明在处理完信号后要求返回到原系统调用中继续运行, 即系统
     * 调用不会被中断
     *
     ...
     *
     * sa_handler 执行的时候, 特权级 3 栈上的参数是下面几个 put_fs_long
     * 放上去的内容, 他们分别是:
     *      - sa->sa_restorer
     *      - signr
     *      - current->blocked
	...
	*(&esp) -= longs;
    ...
    // 在用户堆栈中从下到上存放 sa_restorer, 信号 signr, 屏蔽码 blocked (如果SA_NOMASK
    // 置位), eax, ecx, edx, eflags 和用户程序原代码指针
	tmp_esp=esp;
	put_fs_long((long) sa->sa_restorer,tmp_esp++);
	put_fs_long(signr,tmp_esp++);

    /* 如果允许信号自己的处理句柄程序收到信号自己, 则需要将进程的信号阻塞码压入堆栈*/
	...
	current->blocked |= sa->sa_mask; /* 信号处理过程中, 不会继续处理 sa_mask 指定的那部分信号 */
