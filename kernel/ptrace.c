 ...
 * data space.
 */
static inline int put_stack_long(struct task_struct *task, int offset, unsigned long data)
{
    unsigned char *stack;
...
asmlinkage int sys_ptrace(long request, long pid, long addr, long data)
{
    ...
    if (request == PTRACE_ATTACH) {
        if (child == current)
            return -EPERM;
        if ((!child->dumpable || (current->uid != child->euid) || (current->gid != child->egid)) &&
            !suser())
            return -EPERM;
    ...
    case PTRACE_PEEKUSR: {
        ...
        if (addr < 17 * sizeof(long)) {
            ...
            if (addr == DS || addr == ES || addr == FS || addr == GS || addr == CS || addr == SS)
                tmp &= 0xffff;
        };
        if (addr >= (long)&dummy->u_debugreg[0] && addr <= (long)&dummy->u_debugreg[7]) {
    ...
    case PTRACE_POKEUSR: /* write the word at location addr in the USER area */
        ...
        if (addr >= (long)&dummy->u_debugreg[0] && addr <= (long)&dummy->u_debugreg[7]) {
            ...
            if (addr == (long)&dummy->u_debugreg[5])
                return -EIO;
            if (addr < (long)&dummy->u_debugreg[4] && ((unsigned long)data) >= 0xbffffffd)
                return -EIO;
