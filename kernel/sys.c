 ...
 * 设置进程组号(gid).
 *
 * 如果任务没有超级用户特权, 它可以使用 setgid 将其 effective gid 设置为成其 saved gid 或其 real gid
 ...
 * 在进入内核中运行时, 段寄存器 fs 被默认地指向当前用户数据空间. 因此该函数就可利用
 * fs 来访问用户空间中的值 */
 ...
 * 设置任务用户 ID(uid)
 *
 * 如果任务没有超级用户特权, 它可以使用setuid()将其 effective uid 设置成其 saved uid 或其 real uid
 * 如果任务有超级用户特权, 则实际的uid/有效的uid和保存的uid都会被设置成参数指定的uid */
 ...
 * 该函数设置数据段末尾为 end_data_seg 指定的值. 该值必须大于代码结尾并且要小于堆栈结尾 16384=16KB.
 ...
 *
 * 如果该函数用于将进程从一个进程组移到另一个进程组, 则这两个进程组必须属于同一个
 * 会话(session). 在这种情况下, 参数 pgid 指定了要加入的现有进程组 ID, 此时该
 * 组的会话 ID 必须与将要加入进程的相同 */
int sys_setpgid(int pid, int pgid)
{
    ...
    /* 如果参数 pgid 为 0, 则 pgid 也取值为当前进程的 pid (?? 这里与POSIX标准的描述有出入) */
    if (!pgid)
		pgid = current->pid;
    ...
    for (i=0 ; i<NR_TASKS ; i++)
        /* 如果找到了进程号是 pid 的进程, 并且该进程的父进程就是当前进程或者该进程
         * 就是当前进程. 这就是说, 自己的父进程或者自己才可以给自己设置 pgid */
 ...
 * 若某项的值是 NOGROUP, 则表示从该项开始以后所有项都空闲, 否则数组项中保存的是用户组号.
 *
 * 参数
...
int in_group_p(gid_t grp)
{
    ...
    * 若扫描到值为 NOGROUP 的项, 表示已扫描完全部有效项而没有发现匹配的组号, 函数返回 0 */
	if (grp == current->egid)
		return 1;
...
/* 取当前进程指定资源的界限值.
 *
 * 进程的任务结构中定义有一个 rlim[RLIM_NLIMITS] 数组, 用于控制进程使用系统资源的界限,
 ...
 *
 * 参数:
 *      - resource 指定我们咨询的资源名称, 实际上它是任务结构中 rlim 数组的索引值
...
/* 设置当前进程指定资源的界限值
 * 参数:
 *      - resource 指定我们设置界限的资源名称, 实际上它是任务结构中 rlim 数组的索引值
 ...
 * measuring them yet).
 *
 * 获取指定进程的资源利用信息, 本系统调用提供当前进程或其已终止的和等待着的子进程资源使用情况
 *
...
// timezone 结构含有本地距格林尼治标准时间以西的分钟数(tz_minuteswest)和夏令时间调整类型(tz_dsttime)两个字段.
