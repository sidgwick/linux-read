...
void do_int3(long *esp, long error_code, long fs, long es, long ds, long ebp, long esi, long edi,
...
/* 设置中断描述符表
 * set_trap_gate 与 set_system_gate 都使用了中断描述符表 IDT 中的陷阱门(Trap
 * Gate), 它们之间的主要区别在于前者设置的特权级为 0, 后者是 3
 * 因此断点陷阱中断int3, 溢出中断 overflow 和边界出错中断 bounds
 * 可以由任何程序调用 */
void trap_init(void)
{
    int i;
    ...
    /* 设置协处理器中断 0x2d(即45)陷阱门描述符, 并允许其产生中断请求,
     * 设置并行口中断描述符 */
