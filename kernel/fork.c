...
// 写保护标志 WP(位16), 内核可以通过设置该标志来禁止特权级 0 的代码向用户空间只读
...
void verify_area(void * addr,int size)
{
	...
	start += get_base(current->ldt[2]); /* addr 对应的 4GB 空间的线性地址对应的页面起始地址 */
	while (size>0) {
		size -= 4096;
		write_verify(start);
		start += 4096;
 ...
 *    CS, EIP    EDX, ECX, EBX */
int copy_process(
    int nr,long ebp,long edi,long esi,long gs, /* 这几个参数是在 sys_call.s 设置的 */
		...
		long eip,long cs,long eflags,long esp,long ss)
{
	...
	*p = *current;	/* NOTE! this doesn't copy the supervisor stack */
     ...
     * 汇编指令 clts 用于清除控制寄存器 CR0 中的任务已交换(TS)标志. 每当发生任务切换,
     * CPU 都会设置该标志. 该标志用于管理数学协处理器: 如果该标志置位, 那么每个 ESC 指
     * 令都会被捕获(异常7). 如果协处理器存在标志 MP 也同时置位的话, 那么 WAIT 指令也会
     * 捕获. 因此, 如果任务切换发生在一个 ESC 指令开始执行之后, 则协处理器中的内容就可能
     * 需要在执行新的 ESC 指令之前保存起来. 捕获处理句柄会保存协处理器的内容并复位 TS 标
     * 志, 指令 fnsave 用于把协处理器的所有状态保存到目的操作数指定的内存区域中(tss.i387) */
	...
	if (current->library) current->library->i_count++;
