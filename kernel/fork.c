...
void verify_area(void *addr, int size)
{
    ...
    start += get_base(
        current->ldt[2]); /* addr 对应的 4GB 空间的线性地址对应的页面起始地址 */
    while (size > 0) {
        size -= 4096;
        write_verify(start);
        start += 4096;
...
int copy_mem(int nr, struct task_struct *p)
{
    ...
    p->start_code =
        new_code_base; /* start_code 是任务在 4GB 线性空间的线性地址 */
...
int copy_process(
    ...
    long ebx, long ecx, long edx, long orig_eax, long fs, long es, long ds,
    /* 这里是不是少了个 long none ??? */
    long eip, long cs, long eflags, long esp, long ss)
{
    ...
    *p = *current; /* NOTE! this doesn't copy the supervisor stack */
    ...
    p->alarm = 0;             /* 报警定时值(嘀嗒数) */
    p->leader =
        0; /* process leadership doesn't inherit, 进程的领导权是不能继承的 */
