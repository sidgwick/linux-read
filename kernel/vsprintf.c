...
static char * number(char * str, int num, int base, int size, int precision, int type)
{
    ...
    /* i 存有数值 num 的数字个数, 若数字个数小于精度值, 则str中放入(精度值-i)个 0 */
	while(i<precision--)
		*str++ = '0';
	/* 数字本身 */
    while(i-->0)
...
int vsprintf(char *buf, const char *fmt, va_list args)
{
	...
	for (str=buf ; *fmt ; ++fmt) {
        ...
        *
        * 如果宽度域中是数值则直接取其为宽度值
        * 如果宽度域中是字符 '*', 表示下一个参数指定宽度, 因此调用 va_arg 取宽度值. 若此时宽度值小于 0,
        * 则该负数表示其带有标志域 '-' 标志(左靠齐), 因此还需在标志变量中添入该标志, 并将字段宽度值取为其绝对值 */
		...
		case 'c':
            ...
            // 此时如果标志域表明不是左靠齐, 则该字段前面放入 '宽度域值-1' 个空格字符, 然后再放入参数字符
            // 如果宽度域还大于0, 则表示为左靠齐, 则在参数字符后面添加 '宽度值-1' 个空格字符
			if (!(flags & LEFT))
				while (--field_width > 0)
					*str++ = ' ';
