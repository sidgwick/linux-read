 ...
 */
struct tty_queue *table_list[] = {con_queues + 0, con_queues + 1,
...
int tty_read(unsigned channel, char *buf, int nr)
{
    ...
    while (nr > 0) {
        ...
        do {
            ...
            if ((EOF_CHAR(tty) != _POSIX_VDISABLE && c == EOF_CHAR(tty)) ||
                c == 10)
                tty->secondary->data--;
            if ((EOF_CHAR(tty) != _POSIX_VDISABLE && c == EOF_CHAR(tty)) &&
                L_CANON(tty))
...
int tty_write(unsigned channel, char *buf, int nr)
{
    ...
    if (!(tty->write_q || tty->read_q || tty->secondary))
        return -EIO;
    if (L_TOSTOP(tty) && (current->tty == channel) &&
...
void tty_init(void)
{
    ...
    for (i = 0; i < 256; i++) {
        tty_table[i] = (struct tty_struct){
            {0, 0, 0, 0, 0, INIT_C_CC}, 0, 0, 0, NULL, NULL, NULL, NULL};
    }
    con_init();
    for (i = 0; i < NR_CONSOLES; i++) {
        con_table[i] = (struct tty_struct){
            {ICRNL,         /* change incoming CR to NL */
             ...
             0, IXON | ISIG | ICANON | ECHO | ECHOCTL | ECHOKE,
             0, /* console termio */
             INIT_C_CC},
            0, /* initial pgrp */
            0, /* initial session */
    ...
    for (i = 0; i < NR_SERIALS; i++) {
        rs_table[i] = (struct tty_struct){{0, /* no translation */
                                           0, /* no translation */
                                           B2400 | CS8, 0, 0, INIT_C_CC},
                                          0,
                                          0,
                                          0,
                                          rs_write,
    ...
    for (i = 0; i < NR_PTYS; i++) {
        mpty_table[i] = (struct tty_struct){{0, /* no translation */
                                             0, /* no translation */
                                             B9600 | CS8, 0, 0, INIT_C_CC},
                                            0,
                                            0,
                                            0,
                                            mpty_write,
                                            ...
                                            mpty_queues + 1 + i * 3,
                                            mpty_queues + 2 + i * 3};
        spty_table[i] = (struct tty_struct){{0, /* no translation */
                                             0, /* no translation */
                                             B9600 | CS8, IXON | ISIG | ICANON,
                                             0, INIT_C_CC},
                                            0,
                                            0,
                                            0,
