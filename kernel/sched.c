...
#define _BLOCKABLE (~(_S(SIGKILL) | _S(SIGSTOP))) /* SIGKILL 和 SIGSTOP 不可被 block */
...
void show_task(int nr,struct task_struct * p)
{
    ...
    //       看着似乎不是, tss.eip 偏移量是 0x3d0, 1019x4 算下来是 0xfec, 差的还挺多的
...
// 100Hz, 也即每 10ms 发出一次时钟中断. 因此这里 LATCH 是设置 8253 芯片的初值
...
// 前面的限定符 volatile, 这个限定词的含义是向编译器指明变量的内容可能会由于被其他程
// 序修改而变化, 通常在程序中申明一个变量时, 编译器会尽量把它存放在通用寄存器中, 例如
// %ebx, 以提高访问效率. 当 CPU 把其值放到 ebx 中后一般就不会再关心该变量对应内存位
// 置中的内容, 若此时其他程序(例如内核程序或一个中断过程)修改了内存中该变量的值, ebx
// 中的值并不会随之更新. 为了解决这种情况就创建了 volatile 限定符, 让代码在引用该变
// 量时一定要从指定内存位置中取得其值. 这里即是要求 gcc 不要对 jiffies 进行优化处理,
// 也不要挪动位置, 并且需要从内存中取其值. 因为时钟中断处理过程等程序会修改它的值
...
void schedule(void)
{
	...
	for(p = &LAST_TASK ; p > &FIRST_TASK ; --p)
		if (*p) {
            ...
            /* 闹钟到时间, 发送闹钟信号
             * 如果设置过任务的定时值 alarm, 并且已经过期(alarm<jiffies), 则在信号位图中置
             * SIGALRM 信号(即发送 SIGALARM 信号), 该信号的默认操作是终止进程. */
	...
	while (1) {
         ...
         * 仍然为 -1, next=0), 则退出循环执行任务切换操作. 否则就根据每个任务的优先权值, 更新每
 ...
 * 只有当捕获了一个信号, 并且信号捕获处理函数返回, pause 才会返回. 此时 pause 返回值应该是 -1,
 * 并且 errno 被置为 EINTR. 这里还没有完全实现(直到0.95版) */
int sys_pause(void)
{
 ...
 *
 * 数组 wait_motor 用于存放等待软驱马达启动到正常转速的进程指针, 数组索引 0-3 分别对应软驱 A-D
 * 数组 mon_timer 存放各软驱马达启动所需要的滴答数. 程序中默认启动时间为 50 个滴答(0.5秒)
 * 数组 moff_timer 存放各软驱在马达停转之前需维持的时间, 程序中设定为 10000 个滴答(100秒)
 *
...
int ticks_to_floppy_on(unsigned int nr)
{
	...
	if (mask != current_DOR) {
         ...
         * 上面的逻辑给 mask 的高 4 bits 可能有赋 1 操作(表示启用对应的软驱), 这里如果 if 条件为真
         * 说明 nr 对应的那个软驱, 是从关闭变为启动, 这时候就给 mon_timer 数组对应的定时器设置 0.5 秒延时
         * 否则说明 nr 对应的那个软驱已经是启动状态, 简单的设置为 2 个滴答, 以满足 do_floppy_timer 中先递
         * 减后判断的要求 */
...
void do_floppy_timer(void)
{
	...
	for (i=0 ; i<4 ; i++,mask <<= 1) {
		...
		} else if (!moff_timer[i]) { /* 如果设置了马达停转定时, 则尝试到时间关闭软驱马达 */
			current_DOR &= ~mask;
...
static struct timer_list {
	long jiffies; /* 定时器滴答数量(如果非链表头, 则表示上一个节点滴答完, 这个节点还需要多少滴答到期) */
	void (*fn)(); /* 定时器处理函数 */
...
void add_timer(long jiffies, void (*fn)(void))
{
    ...
    if (jiffies <= 0)
        ...
        // 链表项按定时值从小到大排序
        // 在排序时减去排在前面需要的滴答数, 这样在处理定时器时只要查看链表头的, 第一项的定时是否到期即可.
        // ??? 这段程序好象没有考虑周全, 如果新插入的定时器值小于原来头一个定时器值时则根本不会进入循环中,
        // 但此时还是应该将紧随其后面的一个定时器值减去新的第 1 个的定时值. 即如果第 1 个定时值 <= 第 2 个,
        // 则第 2 个定时值扣除第 1 个的值即可, 否则进入下面循环中进行处理
 ...
 * 一个进程由于执行时间片用完时, 则进行任务切换, 并执行一个计时更新工作 */
void do_timer(long cpl)
{
     ...
     * 那么若已经处于黑屏状态(blanked = 1), 则让屏幕恢复显示. 若 blankcount 计数不为零,
     * 则递减之, 并且复位黑屏标志
