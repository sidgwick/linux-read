...
void show_task(int nr, struct task_struct *p)
{
    ...
    printk("%d: pid=%d, state=%d, father=%d, child=%d, ", nr, p->pid, p->state,
           p->p_pptr->pid, p->p_cptr ? p->p_cptr->pid : -1);
...
repeat:
    ...
    // 说明 在本任务插入等待队列后还有任务进入等待队列.
    // 于是我们应该也要唤醒这个任务, 而我
    // 们自己应按顺序让这些后面进入队列的任务唤醒,
    // 因此这里将等待队列头所指任务先置为 就绪状态,
